...
class CADPCMDecoder final : public IMusicPatternDecoder
{
public:
...
class CADPCMDecoderInstance final : public IMusicPatternDecoderInstance
{
public:
	// hardcoded constants,  can be changed when using a different format
	static const unsigned int scuiBlockSize =
	    2048; // constant value in 44 KHz Microsoft format, half the size for 22KHz
	static const unsigned int scuiSamplesPerBlock =
	    2036; // constant value in 44 KHz Microsoft format, half the size for 22KHz
...
protected:
	...
	unsigned int m_uiCurrentSamplesPerBlock; // sampels per block in file(usually m_uiCurrentBlockSize-12), always
	                                         // less than static allocated buffer
...
protected:
	...
	unsigned short AdpcmReadBlock(
	    short* pDest = NULL); // decoded one block, if dest not specified, default static buffer is chosen
