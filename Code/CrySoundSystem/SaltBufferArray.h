...
template<class TSalt = unsigned short, class TIndex = unsigned short, unsigned int TSize = 0xffff - 2>
...
public:
	...
	static TIndex GetTSize() { return TSize; }

	//!
	bool IsFull() const { return m_FreeListStartIndex == (TIndex)-1; }
	...
	/*
	// to support save games compatible with patched levels (patched levels might use more EntityIDs and save game
	might conflict with dynamic ones)
	...
	*/
	...
	void Remove(const CSaltHandle<TSalt, TIndex> Handle)
	{
		...
		assert(rSalt >
		       oldSalt); // if this fails a wraparound occured (thats severe) todo: check in non debug as well
	...
	// O(1)
	// Returns:
	//   true=handle is referenceing to a valid object, false=handle is not or referencing to an object that was
	//   removed
	...
	TIndex GetMaxUsed() const { return m_maxUsed; }
...
private:
	...
	struct SSaltBufferElement
	{
		...
		TIndex m_NextIndex; //!< linked list of free or used elements, (TIndex)-1 is used as end marker and for
		                    //!< not valid elements, -2 is used for used elements
	};
	...
	TIndex m_maxUsed; //!< to enable fast iteration through the used elements - is constantly growing execpt when
	                  //!< calling Reset()
};
