...
int CPCMDecoder::ReadFile(void* pBuf, int nSize, int nCount, int* pNewFilePos)
{
	if (!m_pFile)
		return false;
	int nBytesRead = m_pPak->FReadRaw(pBuf, nSize, nCount, m_pFile);
...
bool CPCMDecoder::InitStreamWAV()
{
	...
	{
		CryLogWarning("[CPCMDecoder::InitStreamWAV()] Cannot load file %s, due to zero-data-size !",
		              m_FileInfo.sFilename.c_str());
		return false;
	}
	if (WaveHeader.wChnls != 2)
	{
		CryLogWarning("[CPCMDecoder::InitStreamWAV()] Cannot load file %s, due to unsupported channel-number "
		              "(%d channels found; 2 channels needed) !",
		              m_FileInfo.sFilename.c_str(), WaveHeader.wChnls);
		return false;
	}
	if (WaveHeader.BitsPerSample != 16)
	{
		CryLogWarning("[CPCMDecoder::InitStreamWAV()] Cannot load file %s, due to unsupported resolution (%d "
		              "bits/sample found; 16 bits/sample needed) !",
...
bool CPCMDecoder::Open(const char* pszFilename)
{
	...
	m_pFile = m_pPak->FOpen(pszFilename, "rbx");
	if (!m_pFile)
		return false;
	m_nPosBytes = 0;
...
bool CPCMDecoderInstance::Seek0(const int nDelay)
{
...
bool CPCMDecoderInstance::SeekPos(const int nSamplePos)
{
...
bool CPCMDecoderInstance::GetPCMData(signed long* pDataOut, int nSamples, bool bLoop)
{
	...
	for (;;)
	{
		...
		{
			if (!m_pDecoder->ReadFile(&(pDataOut[nOfs]), m_pDecoder->m_PCMFileInfo.nBytesPerSample,
			                          nSamplesToRead, &m_nPosBytes))
				return false;
		}
		else
		{
...
const bool CPCMDecoderInstance::FillPCMBuffer22KHz(signed long* pBuffer, int nSamples)
{
	...
	const unsigned int cuiSamplesPerBlock =
	    scuiEncodedBlockSize /
	    m_pDecoder->m_PCMFileInfo
		.nBytesPerSample; // number of samples to read per block (according to allocated static array)
	const unsigned int cuiBlocks =
	    nSamples / (cuiSamplesPerBlock * 2); // blocks to read, remember samples will be doubled
	for (unsigned int i = 0; i < cuiBlocks; i++)
	{
		if (!m_pDecoder->ReadFile(m_aEncodedBlock, m_pDecoder->m_PCMFileInfo.nBytesPerSample,
		                          cuiSamplesPerBlock, &m_nPosBytes))
			return false;
	...
	if (nSamples > 0)
	{
		...
		if (nSamples & 0x1)
		{
			...
			*pBuffer++ = m_lLastSample =
			    *pEncodedData; // remember last sample, has to start with in next request
		}
		else
		{
