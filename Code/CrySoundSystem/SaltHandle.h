...
public:
	...
	// constructor
	CSaltHandle(TSalt Salt, TIndex Index) : m_Index(Index), m_Salt(Salt) {}
	...
	// comparison operator
	bool operator!=(const CSaltHandle<TSalt, TIndex>& rhs) const { return !(*this == rhs); }

	// conversion to bool
	// e.g. if(id){ ..nil.. } else { ..valid or not valid.. }
	operator bool() const { return m_Salt != 0 || m_Index != 0; }
	...
	TSalt GetSalt() const { return m_Salt; }
