...
int CGame::FindGripIndex(std::string_view key) const
{
	const auto it = std::find_if(
		m_gripRegistry.begin(), m_gripRegistry.end(),
		[&](const HandGripInfo& e)
		{
			return e.key == key;
		});

	if (it == m_gripRegistry.end())
		return -1;

	return static_cast<int>(std::distance(m_gripRegistry.begin(), it));
}

CGame::HandGripInfo& CGame::GetOrCreateGrip(std::string_view key)
{
	int idx = FindGripIndex(key);
	if (idx < 0)
	{
		m_gripRegistry.emplace_back();
		idx = static_cast<int>(m_gripRegistry.size()) - 1;
		m_gripRegistry[idx].key.assign(key);
	}
	return m_gripRegistry[idx];
}

CGame::HandGripInfo* CGame::GetGripByKey(std::string_view key)
{
	auto it = std::find_if(
		m_gripRegistry.begin(), m_gripRegistry.end(),
		[&](const HandGripInfo& e) { return e.key == key; });

	return (it == m_gripRegistry.end()) ? nullptr : &(*it);
}

CGame::HandGripInfo* CGame::GetGripByEntity(IEntity* e)
{
	if (!e)
	{
		return nullptr;
...
void CGame::SetGripLeft(std::string_view key, const Vec3& leftEL)
{
	HandGripInfo& e = GetOrCreateGrip(key);
	e.leftEL = leftEL;
	e.hasLeft = true;
}

void CGame::SetGripRight(std::string_view key, const Vec3& rightEL)
{
	HandGripInfo& e = GetOrCreateGrip(key);
	e.rightEL = rightEL;
	e.hasRight = true;
...
void CGame::SetGripHoldObjectOffset(std::string_view key, const Vec3& fpOffset, const Vec3& tpOffset)
{
	HandGripInfo& e = GetOrCreateGrip(key);
...
bool CGame::RemoveGripByKey(std::string_view key)
{
	auto it = std::find_if(
		m_gripRegistry.begin(), m_gripRegistry.end(),
		[&](const HandGripInfo& e)
	...
	m_gripRegistry.erase(it);
	return true;
}

bool CGame::RemoveGripForEntity(IEntity* e)
{
	if (!e)
		return false;
...
void CGame::ClearGripRegistry()
{
	m_gripRegistry.clear();
}