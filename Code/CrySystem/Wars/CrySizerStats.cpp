...
void CrySizerStatsRenderer::render(bool bRefreshMark)
{
	...
	if (nNameWidth < 25)
		nNameWidth = 25;
	float fCharScaleX = 0.8f, fCharScaleY = 0.8f;
	float fCharSizeX = fCharScaleX, fCharSizeY = fCharScaleY;
	float fLeft = 0;
	float fTop  = 8;
	float fVStep = 6;
	...
	m_pRenderer->WriteXY((int)fLeft, (int)(fTop), fCharScaleX, fCharScaleY, 0.9f,0.85f,1,0.85f,
		"%-*s   total  partial  %s",nNameWidth,bRefreshMark?"Memory usage (refresh*)":"Memory usage (refresh )",szCountStr1);
	m_pRenderer->WriteXY((int)fLeft, (int)(fTop + fVStep*0.25f), fCharScaleX, fCharScaleY, 0.85f,0.9f,1,0.85f,
	...
	for (unsigned i = 0, r = 0; i < m_pStats->size(); ++i)
	{
		...
		m_pRenderer->WriteXY((int)fLeft, (int)(fTop), fCharScaleX, fCharScaleY, pColor[0],pColor[1],pColor[2],pColor[3],
	...
	fTop += 0.25f*fVStep;
	m_pRenderer->WriteXY((int)fLeft, (int)(fTop), fCharScaleX, fCharScaleY, fLightGray,fLightGray,fLightGray,1,
