...
IReadStream_AutoPtr CRefStreamEngine::StartRead (const char* szSource, const char* szFilePathPC, IStreamCallback* pCallback, StreamReadParams* pParams)
{
	...
	m_pPak->RecordFile( 0, szFilePathPC );
	...
	// at this moment the stream should self-in this engine and the stream sets should get initialized
...
unsigned CRefStreamEngine::Wait(unsigned nMilliseconds, unsigned nFlags)
{
	...
	if (!IsMainThread())
	{
		...
		if (numIOJobs(eWaiting)+numIOJobs(ePending) > 0)
		{
#ifdef REFSTREAMENGINE_USE_CryThread
			m_lockNotify.Lock();
			if (!m_flagIOExecuted)
...
{
	if (HaveIOWorkerThread() && (pJobProxy->GetParams().nFlags & SRP_FLAGS_ASYNC_CALLBACK))
...
void CRefStreamEngine::StartWorkerThread()
{
	...
	m_hIOWorker = CreateThread (NULL, 0x8000,
			reinterpret_cast<LPTHREAD_START_ROUTINE>(IOWorkerThreadProc), this, 0, &m_dwWorkerThreadId);
