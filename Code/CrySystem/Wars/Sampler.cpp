...
{
	if (m_rawSamples.size() >= m_nMaxSamples)
	{
...
void CSampler::LogSampledData()
{
	...
	float fOnePercent = nTotalSamples / 100;

	float fPercentTotal = 0;
	int nSampleSum = 0;
	for (int i = 0; i < m_functionSamples.size(); i++)
	{
		// Calculate percentage.
		float fPercent = m_functionSamples[i].nSamples / fOnePercent;
		const char *func = m_functionSamples[i].function.c_str();
...
BOOL WINAPI EnumCallcbackM( PCSTR ModuleName,DWORD64 BaseOfDll,PVOID UserContext )
{
	return TRUE;
}
...
bool CSymbolDatabase::Init()
{
	...
	SymEnumerateModules64(GetCurrentProcess(), reinterpret_cast<PSYM_ENUMMODULES_CALLBACK64>(&EnumCallcbackM), nullptr);
