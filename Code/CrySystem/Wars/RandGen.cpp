...
#define M_R             (397)                 // a period parameter
#define K_R             (0x9908B0DFU)         // a magic constant
...
{
	uint32 *p0=state, *p2=state+2, *pM=state+M_R, s0, s1;
	int    j;
	...
	for(s0=state[0], s1=state[1], j=N_RAND_STATE-M_R+1; --j; s0=s1, s1=*p2++)
			*p0++ = *pM++ ^ (mixBits(s0, s1) >> 1) ^ (loBit(s1) ? K_R : 0U);

	for(pM=state, j=M_R; --j; s0=s1, s1=*p2++)
			*p0++ = *pM++ ^ (mixBits(s0, s1) >> 1) ^ (loBit(s1) ? K_R : 0U);

	s1=state[0], *p0 = *pM ^ (mixBits(s0, s1) >> 1) ^ (loBit(s1) ? K_R : 0U);
	s1 ^= (s1 >> 11);
	s1 ^= (s1 <<  7) & 0x9D2C5680U;
