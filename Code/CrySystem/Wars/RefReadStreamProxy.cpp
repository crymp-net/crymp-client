...
#if LOG_IO
	CryLog("io:CRefReadStreamProxy %p(%s, %p)", this, szSource, pCallback);
#endif
...
CRefReadStreamProxy::~CRefReadStreamProxy ()
{
	...
	if (m_bFreeBuffer && m_pBuffer)
		g_pPakHeap->Free (m_pBuffer);
#if LOG_IO
	CryLog("io:~CRefReadStreamProxy %p(%s, %p)", this, m_strClient.c_str(), m_pCallback);
#endif
	m_pStream->Unregister(this);
}

//////////////////////////////////////////////////////////////////////////
void CRefReadStreamProxy::AddRef()
{
	CryInterlockedIncrement(&m_nRefCount);
}

//////////////////////////////////////////////////////////////////////////
void CRefReadStreamProxy::Release()
{
...
bool CRefReadStreamProxy::StartRead(unsigned nMemQuota)
{
	...
	if (!m_pBuffer)
	{
		...
		m_pBuffer = g_pPakHeap->Alloc(m_Params.nSize, "CRefReadStreamProxy::StartRead: m_pBuffer");
		if (!m_pBuffer)
		{
	...
	_smart_ptr<CRefReadStreamProxy> tempRef(this);
...
void CRefReadStreamProxy::OnFinishRead(unsigned nError)
{
	...
	{
		m_bError = true;
		CryLogAlways( "IO Error=%d",nError );
	}
