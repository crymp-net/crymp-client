...
#include "CryCommon/CryCore/crc32.h"

#include "md5.h"

//#define EXE_CRC_CHECK
#ifdef WIN32
...
bool CDataProbe::GetDataCode( char *pBuf,int nSize,SDataProbeContext &ctx )
{
	...
	unsigned int tkey[4] = {2985634234U,378634893U,387681212U,436851212U};
	TEA_ENCODE( (unsigned int*)&nCode,(unsigned int*)&nCode,8,tkey );
	ctx.nCode = nCode;
	return true;
}
...
bool CDataProbe::GetCode( SDataProbeContext &ctx )
{
	...
	// SNH: this used to attempt to load files directly, then fall back on the pak system.
	//	Better to let the pak system decide where to load from (eg depending on devmode setting). 
	//	This ensures we probe the same files that the game is loading, and prevents MP disconnects if
	//	a user uses the editor (so long as they don't overwrite game files).
	CCryFile cryfile;
	if (cryfile.Open( ctx.sFilename.c_str(),"rb" ))
	{
		...
		if (cryfile.ReadRaw( pBuf,ctx.nSize ) != ctx.nSize)
		{
			free( pBuf );
			return false;
		}
...
bool CDataProbe::GetRandomFileProbe( SDataProbeContext &ctx,bool bAtEnd )
{
	...
	if (FILE *file = fopen( ctx.sFilename.c_str(),"rb" ))
	{
	...
	if (nFileSize > 0)
	{
		...
		else
		{
			...
			if (nFileSize < MAX_PROBE_SIZE)
			{
				...
				int nOffset2 = 0;
				do {
					nOffset1 = (int)GetRandom( 1000 );
					nOffset2 = (int)GetRandom( 1000 );
		...
		ctx.nSize = (unsigned int)nSize;
		ctx.nOffset = (unsigned int)nOffset;
		return true;
	}
...
//////////////////////////////////////////////////////////////////////////
// Timur.
// This is game launcher exe authentication function, this code is not for public release!!
//////////////////////////////////////////////////////////////////////////
bool CDataProbe::CheckLoader( void *pFunc )
{
	...
	// set of keys (one for game launcher exe second for dedicated server).
	unsigned int keys[6][4] = {
		{1873613783U,235688123U,812763783U,1745863682U},
		{1897178562U,734896899U,156436554U,902793442U},
		{1178362782U,223786232U,371615531U,90884141U},
		{89158165U, 1389745433U,971685123U,785741042U},
		{389623487U,373673863U,657846392U,378467832U},
		{1982697467U,3278962783U,278963782U,287678311U},
	};

	m_rand.Seed( CryGetTicks() );
	...
	// First try... key0,key1
	//////////////////////////////////////////////////////////////////////////
	TEA_ENCODE( data1,data2,32,keys[0] );
	pCheckFunc( data2 );
	TEA_DECODE( data2,data2,32,keys[1] );
	...
	//////////////////////////////////////////////////////////////////////////
	TEA_ENCODE( data1,data2,32,keys[2] );
	pCheckFunc( data2 );
	TEA_DECODE( data2,data2,32,keys[3] );
	...
	// Third try... key4,key5
	//////////////////////////////////////////////////////////////////////////
	TEA_ENCODE( data1,data2,32,keys[4] );
	pCheckFunc( data2 );
	TEA_DECODE( data2,data2,32,keys[5] );
...
bool CDataProbe::CheckPaul()
{
	...
	// paul.dll
	string fsysname = string("p")+"a"+"u"+"l"+"."+"d"+"l"+"l";
	...
	int nLen = file.GetLength();
	char *pBuffer = new char[nLen+16];
	file.ReadRaw( pBuffer,nLen );
	Crc32Gen gen;
	uint32 nCRC = gen.GetCRC32(pBuffer,nLen,0xffffffff);
	delete []pBuffer;

	//CryLogAlways( "CRC=%X",nCRC );

	if (nCRC != SECUROM_PAUL_CRC32)
		return false;
