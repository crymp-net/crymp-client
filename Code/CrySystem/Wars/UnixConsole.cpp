...
// Helper class for cleanup performed via atexit().
// Initialized by CUNIXConsole::Init().
class CUNIXConsoleCleanup
{
	static CUNIXConsole *m_pSelf;

	void operator = (const CUNIXConsoleCleanup&);
	CUNIXConsoleCleanup();
	CUNIXConsoleCleanup(const CUNIXConsoleCleanup&);
...
public:
	static void Register(CUNIXConsole *pSelf)
	{
		if (m_pSelf != NULL)
			return;
		m_pSelf = pSelf;
...
CUNIXConsole *CUNIXConsoleCleanup::m_pSelf = NULL;

class CUNIXConsoleInputThread : public CrySimpleThread<>
{
...
void CUNIXConsole::Init(const char *headerString)
{
  ...
  keypad(stdscr, TRUE);
	...
	CUNIXConsoleCleanup::Register(this);
...
{
	static CryFastLock cleanupLock;
...
bool CUNIXConsole::FixCursorPosition()
{
	...
	if (m_CursorPosition < m_Width / 2 && m_ScrollPosition > 0)
	{
		m_ScrollPosition = 0;
		repaint = true;
	}
...
int CUNIXConsole::GetNumPlayers()
{
	...
	IGameFramework *pGameFramework = m_pSystem->GetIGame()->GetIGameFramework();
...
void CUNIXConsole::Key(int c)
{
	...
	assert(m_CursorPosition <= m_InputLine.size());
...
void CUNIXConsole::DrawStatus(int maxLines)
{
	...
	if (m_ScrollUp > 0)
	{
		...
		if (m_ScrollUp == m_LineBuffer.size() - logHeight)
			strncpy(bufferRight, "| SCROLL:TOP ", sizeof bufferRight);
		else
			snprintf(
					bufferRight,
					sizeof bufferRight,
	...
	if (leftWidth + rightWidth > m_Width)
	{
		pad = 0;
		if (rightWidth > m_Width)
		{
			leftWidth = 0;
			rightWidth = m_Width;
		}
		...
		case eCDO_PutText:
			{
				...
				if (y < 0 || y > m_Height-4)
					break;
				if (iter->x < 0 || iter->x > m_Width)
					break;
				int len = strlen(iter->text);
				if (iter->x + len > m_Width)
					len = m_Width - iter->x;
				move(y + 1, iter->x);
				for (int i=0; i<len; i++)
					addch(iter->text[i]);
...
char CUNIXConsole::Prompt(const char *promptString, const char *responseChars)
{
	...
	strncpy(m_PromptResponseChars, responseChars, sizeof m_PromptResponseChars);
	m_PromptResponseChars[sizeof m_PromptResponseChars - 1] = 0;
	DrawCmd();
	while (m_PromptResponse == 0)
		m_PromptCond.Wait(m_Lock);
...
{
	if (m_RequireDedicatedServer && !pSystem->IsDedicated())
		return;
...
void CUNIXConsole::OnUpdate()
{
	...
	if (IGameFramework *pGameFramework = m_pSystem->GetIGame()->GetIGameFramework())
...
void CUNIXConsole::PutText( int x, int y, const char * msg )
{
	...
	cmd.x = x;
	cmd.y = y;
	strncpy( cmd.text, msg, sizeof(cmd.text) );
...
void CUNIXConsoleInputThread::Run()
{
	...
	while (true)
	{
		...
		case KEY_HOME:
		case '\001': // CTRL-A
			m_UNIXConsole.KeyHome();
			break;
		case KEY_END:
		case '\005': // CTRL-E
			m_UNIXConsole.KeyEnd();
			break;
		case KEY_BACKSPACE:
		case '\010': // CTRL-H
		...
		case KEY_NPAGE:
		case '\006': // CTRL-F
			m_UNIXConsole.KeyPgDown();
			break;
		case KEY_PPAGE:
		case '\002': // CTRL-B
			m_UNIXConsole.KeyPgUp();
			break;
		default:
			if (c >= 0x20 && c <= 0xff)
				m_UNIXConsole.Key(c);
