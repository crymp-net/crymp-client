...
void update_idbest(const quotient& tsin, quotient& tbest, int newid, int bBest, int& idbest)
{
...
int box_sphere_lin_unprojection(unprojection_mode* pmode, const primitives::box* pbox, int iFeature1,
                                ...
                                geom_contact_area* parea)
{
	...
	{ // minimum unprojection dir requested
		...
		if (niz == 0)
		{
			// Vec3r csize(fabs_tpl(center.x)*size.y*size.z, fabs_tpl(center.y)*size.x*size.z,
			// fabs_tpl(center.z)*size.x*size.y);
			Vec3r csize(fabs_tpl(center.x) - size.x, fabs_tpl(center.y) - size.y,
			            fabs_tpl(center.z) - size.z);
			niz = 1;
...
int cylinder_sphere_lin_unprojection(unprojection_mode* pmode, const primitives::cylinder* pcyl, int iFeature1,
                                     const primitives::sphere* psph, int iFeature2, contact* pcontact,
                                     geom_contact_area* parea)
{
	...
	if (isnonneg(kd) & (isneg(kb) | isneg((kb * kb) - kd)))
	{
		...
		bBest = isneg(tmax - t) & bContact;
		update_idbest(t, tmax, 0x10, bBest, idbest);
	}
	...
	if (bCapped)
	{
		...
		for (icap = -1; icap <= 1; icap += 2)
		{
			...
			polynomial_tpl<real, 4> p =
			    (P2(-(dir * dir)) + P1(2 * (c * dir)) + rc * rc + rs * rs - c * c).sqr() -
			    (P2(-sqr(dir * axis)) + P1(2 * (c * axis) * (dir * axis)) + rs * rs - sqr(c * axis)) *
				(4 * rc * rc);
			for (i = p.findroots(0, pmode->tmax, roots) - 1; i >= 0; i--)
			{
				t.set(roots[i], 1);
				bContact =
				    isneg((rs * rs) + (rc * rc) - (c - dir * t.x).len2()); // checks for phantom root
				bBest = isneg(tmax - t) & bContact;
				update_idbest(t, tmax, 0x20, bBest, idbest);
			}
		}
	}
...
int default_unprojection(unprojection_mode*, const primitives::primitive*, int, const primitives::primitive*, int,
                         contact*, geom_contact_area*)
{
	return 0;
}

int tri_tri_lin_unprojection(unprojection_mode* pmode, const primitives::triangle* ptri1, int iFeature1,
                             const primitives::triangle* ptri2, int iFeature2, contact* pcontact,
                             geom_contact_area* parea)
{
	Vec3r edge0, edge1, n, dp, dir;
	quotient t, tmax(0, 1), t0, t1;
	int itri, i, j, bContact, bBest, idbest = -1, isg;
	const primitives::triangle* ptri[2] = {ptri1, ptri2};
	...
	{ // check for vertex-face contacts (itri provides face, itri^1 - vertex)
		...
		i = 0;
		mindist = (ptri[itri ^ 1]->pt[0] * ptri[itri]->n) *
		          sg; // find vertex of itri^1 that is the closest to the face of itri
		...
		t.set(((ptri[itri]->pt[0] - ptri[itri ^ 1]->pt[i]) * ptri[itri]->n) * isg, pmode->dir * ptri[itri]->n)
		    .fixsign();
		dir = pmode->dir * isg;
		...
		update_idbest(t, tmax, itri << 2 | i, bBest, idbest);
	}
	...
	{ // check for edge-edge contacts
		...
		for (j = 0; j < 3; j++)
		{
			...
			bContact = isneg(fabs_tpl((t0.x * 2) - t0.y) - t0.y) &
			           isneg(fabs_tpl((t1.x * 2) - t1.y) - t1.y); // contact point belongs to both edges
			bBest = bContact & isneg(tmax - t);
			update_idbest(t, tmax, i << 2 | j | 0x80, bBest, idbest);
		}
	}
	...
	if (idbest & 0x80)
	{
		...
		pcontact->n =
		    n * sgnnz((ptri2->n ^ edge1) * n); // sgnnz((ptri2->pt[dec_mod3[j]]-ptri1->pt[dec_mod3[i]])*n);
...
int tri_box_lin_unprojection(unprojection_mode* pmode, const primitives::triangle* ptri, int iFeature1,
                             const primitives::box* pbox, int iFeature2, contact* pcontact, geom_contact_area* parea)
{
	...
	// box vertex - triangle face, check only the box vertex that is the farthest from the triangle (along flipped
	// normal) Proof: suppose that not the farthest point is the global maximum. but then some edges from this point
	// to the farthest point will penetrate the triangle (since the box is convex), meaning this point is not the
	// global maximum
	...
	bContact =
	    1 ^ (isneg((pt[1] - pt[0] ^ ptbox - pt[0] * t.y) * n) | isneg((pt[2] - pt[1] ^ ptbox - pt[1] * t.y) * n) |
	         isneg((pt[0] - pt[2] ^ ptbox - pt[2] * t.y) * n));
	bBest = bContact & isneg(tmax - t);
	update_idbest(t, tmax, 0, bBest, idbest);

	// triangle vertex - box face
	for (j = 0; j < 3; j++)
	{ // i-triangle vertex, j-box face (with sign in direction of movement)
		i = 0;
		mindist = pt[0][j] * sgdir[j]; // find triangle vertex that is the farthest from corresponding box face
		                               // (along its inside normal)
		dist = pt[1][j] * sgdir[j];
		i += isneg(dist - mindist);
		mindist = min(dist, mindist);
		dist = pt[2][j] * sgdir[j];
		...
		ix = inc_mod3[j];
		iy = dec_mod3[j];
		t.set(pbox->size[j] - (pt[i][j] * sgdir[j]),
		      dir[j] * sgdir[j]); // check if contact point belongs to box face
		bContact = isneg(fabs_tpl((pt[i][ix] * t.y) + (dir[ix] * t.x)) - (pbox->size[ix] * t.y)) &
		           isneg(fabs_tpl((pt[i][iy] * t.y) + (dir[iy] * t.x)) - (pbox->size[iy] * t.y));
		bBest = bContact & isneg(tmax - t);
		update_idbest(t, tmax, 0x40 | (i << 2 | j), bBest, idbest);
	}
	...
	for (i = 0; i < 3; i++)
	{
		...
		for (idir = 0; idir < 3; idir++)
		{
			...
			t.y = dir * ncontact;
			t.y *= isg = sgnnz(t.y);
			// choose box edge along idir that will generate maximum t (only it can be the global maximum
			// for the entire unprojection) Proof: there's a continuous range along dir where the edge
			// intersects infinite box slab in direction idir. if touch point is the global separation
			// point, then the edge will intersect the slab, but not the box while continuing moving along
			// dir. but it cannot leave the box immediately after the touch point, because it doesn't touch
			// box feature different from slab feature (if it touches a vertex, we'll hopefully detect it as
			// a face-vertex contact) NOTE: if maximum t is generated by 2 box edges, and the triangle edge
			// crosses both of them, then which one we choose doesn't matter, and if the triangle edge
			// crosses only one, then tri vertex-box face contact will be detected (so choosing max t edge
			// only is safe)
			...
			dp_x_edge1[ix] = dp[iy];
			dp_x_edge1[iy] = -dp[ix];
			t0.set(dp_x_edge1 * ncontact,
			       ncontact.len2() * t.y); // check if t0 is in (0..1) and t1 is in (0..2*size)
	...
	switch (idbest & 0xC0)
	{
		...
		default: // triangle edge - box edge
			...
			pcontact->pt = ptri->pt[i] + (ptri->pt[inc_mod3[i]] - ptri->pt[i]) * t0.x * t0.y +
			               pmode->dir * pcontact->t;
...
int box_tri_lin_unprojection(unprojection_mode* pmode, const primitives::box* pbox, int iFeature1,
                             const primitives::triangle* ptri, int iFeature2, contact* pcontact,
                             geom_contact_area* parea)
{
	pmode->dir.Flip();
...
int tri_cylinder_lin_unprojection(unprojection_mode* pmode, const primitives::triangle* ptri, int iFeature1,
                                  ...
                                  geom_contact_area* parea)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		if (d >= 0)
		{
			...
			update_idbest(t, tmax, 0x40 | i, bBest, idbest);
		}
	}
	...
	for (i = 0; i < 3; i++)
	{
		...
		if (d >= 0)
		{
			...
			bContact =
			    isneg(fabs_tpl((t0.x * 2) - t0.y) - t0.y) & isneg(fabs_tpl(t1.x) - (t1.y * pcyl->hh));
			bBest = bContact & isneg(tmax - t) & isneg(t.x - (pmode->tmax * t.y));
			update_idbest(t, tmax, 0x60 | i, bBest, idbest);
		}
	}
	...
	if (bCapped)
	{
		...
		bBest = bContact & isneg(tmax - t) & isneg(t.x - (pmode->tmax * t.y));
		update_idbest(t, tmax, i, bBest, idbest);

		// triangle vertices - cylinder cap faces
		j = (isnonneg(pcyl->axis * pmode->dir) << 1) -
		    1; // choose cap that lies farther along unprojection direction
		i = 0;
		...
		for (i = 0; i < 3; i++)
		{
			...
			for (j = -1; j <= 1; j += 2)
			{
				...
				if (d >= 0)
				{
					d = sqrt_tpl(d);
					t.set(-b + d, a);
					t0.set(-((dp * t.y + pmode->dir * t.x) * pcyl->axis),
					       (edge * pcyl->axis) * t.y);
		...
		// we also need to process degenerate cases when contacting triangle edge lies in cap plane (edge-cap
		// edge check doesn't detect them)
		if (fabs_tpl(fabs_tpl(ptri->n * pcyl->axis) - 1.0f) < 1E-4f)
		{ // potential triangle face - cylinder cap contact
			...
			if (t > tmax && t < pmode->tmax)
			{
				...
				bContact =
				    1 ^ (isneg((ptri->pt[1] - ptri->pt[0] ^ ptcyl - ptri->pt[0] * t.y) * ptri->n) |
				         isneg((ptri->pt[2] - ptri->pt[1] ^ ptcyl - ptri->pt[1] * t.y) * ptri->n) |
				         isneg((ptri->pt[0] - ptri->pt[2] ^ ptcyl - ptri->pt[2] * t.y) * ptri->n));
				if (bContact)
				{ // cylinder cap center is inside triangle
					pcontact->t = t.val();
					pcontact->pt = center;
					pcontact->iFeature[0] = 0x40;
					return 1;
		...
		else
		{
			...
			{
				edge = ptri->pt[inc_mod3[i]] - ptri->pt[i];
				if (sqr(edge * pcyl->axis) < edge.len2() * 1E-4)
				{ // potential traingle edge - cylinder cap contact
					j = sgnnz(pcyl->axis * pmode->dir);
					center = pcyl->center + pcyl->axis * pcyl->hh * j;
					dp = center - ptri->pt[i];
					t.set(((dp * pcyl->axis) * edge.len2()) -
					          ((edge * pcyl->axis) *
					           (edge * dp)), // calculate when the closest point
					                         // on edge will touch the cap
					      ((pmode->dir * pcyl->axis) * edge.len2()) -
					          ((edge * pcyl->axis) * (edge * pmode->dir)))
					    .fixsign();
					if (t > tmax && t < pmode->tmax)
					{
						pcontact->t = t.val();
						...
						pcontact->pt = ptri->pt[i] + pmode->dir * pcontact->t;
						if (fabs_tpl((t0.x * 2) - t0.y) < t0.y &&
						    (edge * t0.x + (pcontact->pt - center) * t0.y).len2() <
						        r2 * sqr(t0.y))
						{
...
int cylinder_tri_lin_unprojection(unprojection_mode* pmode, const primitives::cylinder* pcyl, int iFeature1,
                                  const primitives::triangle* ptri, int iFeature2, contact* pcontact,
                                  geom_contact_area* parea)
{
	pmode->dir.Flip();
...
int tri_sphere_lin_unprojection(unprojection_mode* pmode, const primitives::triangle* ptri, int iFeature1,
                                const primitives::sphere* psphere, int iFeature2, contact* pcontact,
                                geom_contact_area* parea)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		if (d >= 0)
		{
			...
			bContact = isneg(fabs_tpl((t0.x * 2) - t0.y) - t0.y);
			bBest = bContact & isneg(tmax - t);
			update_idbest(t, tmax, 0x80 | i, bBest, idbest);
			t.x += d * 2;
			t0.set(((psphere->center - ptri->pt[i]) * t.y - pmode->dir * t.x) * edge, t.y * edge.len2());
			bContact = isneg(fabs_tpl((t0.x * 2) - t0.y) - t0.y);
			bBest = bContact & isneg(tmax - t);
	...
	switch (idbest & 0xC0)
	{
		...
		case 0x80:
			i = idbest & 3;
			edge = ptri->pt[inc_mod3[i]] - ptri->pt[i];
			t0.set(((psphere->center - ptri->pt[i]) * tmax.y - pmode->dir * tmax.x) * edge,
			       tmax.y * edge.len2());
			t0.y = 1.0 / t0.y;
...
int sphere_tri_lin_unprojection(unprojection_mode* pmode, const primitives::sphere* psphere, int iFeature1,
                                const primitives::triangle* ptri, int iFeature2, contact* pcontact,
                                geom_contact_area* parea)
{
	pmode->dir.Flip();
...
int tri_capsule_lin_unprojection(unprojection_mode* pmode, const primitives::triangle* ptri, int iFeature1,
                                 const primitives::capsule* pcaps, int iFeature2, contact* pcontact,
                                 geom_contact_area* parea)
{
...
int capsule_tri_lin_unprojection(unprojection_mode* pmode, const primitives::capsule* pcaps, int iFeature1,
                                 const primitives::triangle* ptri, int iFeature2, contact* pcontact,
                                 geom_contact_area* parea)
{
	pmode->dir.Flip();
...
int tri_ray_lin_unprojection(unprojection_mode* pmode, const primitives::triangle* ptri, int iFeature1,
                             const primitives::ray* pray, int iFeature2, contact* pcontact, geom_contact_area* parea)
{
...
int ray_tri_lin_unprojection(unprojection_mode* pmode, const primitives::ray* pray, int iFeature1,
                             const primitives::triangle* ptri, int iFeature2, contact* pcontact,
                             geom_contact_area* parea)
{
	pmode->dir.Flip();
...
int tri_plane_lin_unprojection(unprojection_mode* pmode, const primitives::triangle* ptri, int iFeature1,
                               const primitives::plane* pplane, int iFeature2, contact* pcontact,
                               geom_contact_area* parea)
{
	quotient t;
	real dist, mindist;
...
int plane_tri_lin_unprojection(unprojection_mode* pmode, const primitives::plane* pplane, int iFeature1,
                               const primitives::triangle* ptri, int iFeature2, contact* pcontact,
                               geom_contact_area* parea)
{
	pmode->dir.Flip();
...
int crop_polygon_with_bound(const vector2df* ptsrc, const int* idedgesrc, int nsrc, const vector2df& sz, int icode,
                            int iedge, vector2df* ptdst, int* idedgedst)
{
	...
	for (i = istart, n = ndst = 0, bInside = 1; n < nsrc; n++, i = inext)
	{
		...
		if (bSwitch)
		{
			...
			idedgedst[ndst] =
			    (idedgedst[ndst] & -bInside) |
			    (iedge & ~-bInside); // change edge to bound edge only for inside-outside switches
			++ndst;
		}
	}
	return ndst;
...
void crop_segment_with_bound(vector2df* ptseg, float bound, int ic, int* piFeature0, int* piFeature1, int iedge0,
                             int iedge1)
{
...
int edge_from_vtx(int ivtx0, int ivtx1)
{
...
int vtx_from_edges(int iedge0, int iedge1)
{
	int idir0 = iedge0 >> 2, idir1 = iedge1 >> 2;
	return (iedge0 & 1) << inc_mod3[idir0] | (iedge0 >> 1 & 1) << dec_mod3[idir0] |
	       (iedge1 >> iszero(idir0 - dec_mod3[idir1]) & 1) << idir0;
}

int box_box_lin_unprojection(unprojection_mode* pmode, const primitives::box* pbox1, int iFeature1,
                             const primitives::box* pbox2, int iFeature2, contact* pcontact, geom_contact_area* parea)
{
	const primitives::box* pbox[2] = {pbox1, pbox2};
	...
	// each contact we check guarantees that boxes are fully separated at it (while also guaranteeing that no closer
	// contact of the selected group of features can be the global separation minimum), so the contact with the
	// minimum unprojection length is the global separation minimum, and we don't even need to verify that boxes are
	// in contact at this point (i.e. check finite versions of features in addition to infinite)

	// face - the closest vertex
	for (ibox = 0; ibox < 2; ibox++)
	{ // ibox provides face, ibox^1 - vertex
		for (iz = 0; iz < 3; iz++)
		{
			if (bFindMinUnproj)
			{
			...
			pt = center[ibox] -
			     (axes[0] * (size[ibox ^ 1].x * isg1.x) + axes[1] * (size[ibox ^ 1].y * isg1.y) +
			      axes[2] * (size[ibox ^ 1].z * isg1.z)) *
			         isg0.z;
	...
	for (idir0 = 0; idir0 < 3; idir0++)
	{
		for (idir1 = 0; idir1 < 3; idir1++)
		{
			...
			// Proof of validity is similar to box-triangle: if boxes separate, but infinite slabs (along
			// idir0 and idir1) do not, then the global minimum contact should be at box feature that is not
			// a slab feature.
			...
			pt1 = center[0] - axes[idir1] * size[1][idir1] + axes[ix1] * (size[1][ix1] * isg1.x) +
			      axes[iy1] * (size[1][iy1] * isg1.y);
			dp = pt1 - pt0;
			t.x = (dp * n) * sg;
			bBest = isneg(-t.x) & isneg(t - tmin);
			update_idbest(t, tmin,
	...
	{ // face-vertex
		...
		pcontact->pt = pbox[ibox ^ 1]->center + pmode->dir * pcontact->t * ibox -
		               (Vec3(pbox[ibox ^ 1]->size.x * isg1.x, pbox[ibox ^ 1]->size.y * isg1.y,
		                     pbox[ibox ^ 1]->size.z * isg1.z) *
		                isg0.z) *
		                   pbox[ibox ^ 1]->Basis;
	...
	if (parea)
	{
		...
		for (idir1 = 0; idir1 < 3; idir1++)
		{
			if (fabs_tpl(axes[idir1].x) + fabs_tpl(axes[idir1].y) + fabs_tpl(axes[idir1].z) < (real)1.05)
			{                // axes[i] is close to some coordinate axis
				...
				diff0 =
				    fabs_tpl(center[0][iz] - (dir[0][iz] * pcontact->t)) -
				    (pbox[0]->size[iz] + fabs_tpl(axes[0][iz]) * pbox[1]->size.x +
				     fabs_tpl(axes[1][iz]) * pbox[1]->size.y + fabs_tpl(axes[2][iz]) * pbox[1]->size.z);
				diff1 = fabs_tpl(center[1][idir1] - (dir[1][idir1] * pcontact->t)) -
				        (pbox[1]->size[idir1] + fabs_tpl(axes[idir1][0]) * pbox[0]->size.x +
				         fabs_tpl(axes[idir1][1]) * pbox[0]->size.y +
				         fabs_tpl(axes[idir1][2]) * pbox[0]->size.z);
				if (min(fabs_tpl(diff0), fabs_tpl(diff1)) <
				        (pbox[0]->size[iz] + pbox[1]->size[idir1]) * 0.003f &&
				    fabs_tpl(dir[0][iz]) > 0.01f)
				{
					...
					ptc2d.set(center[0][ix0] - (dir[0][ix0] * pcontact->t),
					          center[0][iy0] - (dir[0][iy0] * pcontact->t));
					...
					for (i = 0; i < 4; i++)
					{
						...
						iy = (i ^ isg1.z) >> 1 & 1;
						ivtx0 = ix << ix1 | iy << iy1;
						pt2d[0][i] = ptc2d + axisx2d * (pbox[1]->size[ix1] * ((ix << 1) - 1)) +
						             axisy2d * (pbox[1]->size[iy1] * ((iy << 1) - 1));
						idedge[0][i] = 1 << 4 | edge_from_vtx((isg1.z & 1) << idir1 | ivtx0,
						                                      (isg1.z & 1) << idir1 | ivtx1);
					}
					parea->npt =
					    crop_polygon_with_bound(pt2d[0], idedge[0], 4, sz2d, 0,
					                            iy0 << 2 | (isg0.z & 1), pt2d[1], idedge[1]);
					parea->npt =
					    crop_polygon_with_bound(pt2d[1], idedge[1], parea->npt, sz2d, 1,
					                            ix0 << 2 | (isg0.z << 1 & 2), pt2d[0], idedge[0]);
					parea->npt =
					    crop_polygon_with_bound(pt2d[0], idedge[0], parea->npt, sz2d, 2,
					                            iy0 << 2 | 2 | (isg0.z & 1), pt2d[1], idedge[1]);
					parea->npt = crop_polygon_with_bound(pt2d[1], idedge[1], parea->npt, sz2d, 3,
					                                     ix0 << 2 | 1 | (isg0.z << 1 & 2), pt2d[0],
					                                     idedge[0]);
					for (i = 0; i < parea->npt; i++)
					{
						...
						pt3d[iy0] = pt2d[0][i].y;
						parea->pt[i] =
						    pt3d * pbox[0]->Basis + pbox[0]->center + pmode->dir * pcontact->t;
						parea->piPrim[0][i] = parea->piPrim[1][i] = 0;
						if ((idedge[0][iprev] ^ idedge[0][i]) >> 4)
						{
							parea->piFeature[idedge[0][iprev] >> 4][i] =
							    0x20 | (idedge[0][iprev] & 0xF);
							parea->piFeature[idedge[0][i] >> 4][i] =
							    0x20 | (idedge[0][i] & 0xF);
						}
						else
						{
							parea->piFeature[idedge[0][i] >> 4][i] =
							    vtx_from_edges(idedge[0][iprev] & 0xF, idedge[0][i] & 0xF);
							parea->piFeature[idedge[0][i] >> 4 ^ 1][i] =
							    iface[idedge[0][i] >> 4 ^ 1];
						}
					}
					if (parea->npt)
					{
		...
		for (int idx = 0; idx < 9; idx++)
		{
			...
			if (fabs_tpl(axes[idir1][iz]) < (real)0.02)
			{ // check all near-zeros in Basis21 buffer
				ix1 = inc_mod3[idir1];
				iy1 = dec_mod3[idir1];
				for (ibox = 0; ibox < 2; ibox++)
				{
					if (fabs_tpl(fabs_tpl(center[ibox][iz] - (dir[ibox][iz] * pcontact->t)) -
					             (pbox[ibox]->size[iz] +
					              fabs_tpl(axes[ix1][iz]) * pbox[ibox ^ 1]->size[ix1] +
					              fabs_tpl(axes[iy1][iz]) * pbox[ibox ^ 1]->size[iy1])) <
					    min(pbox[ibox]->size[iz], pbox[1]->size[ix1] + pbox[ibox ^ 1]->size[iy1]) *
					        0.01f)
					{ // confirmed face(ibox.iz) - edge(ibox^1.idir1) contact
						...
						axisx2d.set(axes[ix1][ix0], axes[ix1][iy0]) *=
						    (isg1.x = sgnnz(axes[ix1][iz]) * -isg0.z);
						axisy2d.set(axes[iy1][ix0], axes[iy1][iy0]) *=
						    (isg1.y = sgnnz(axes[iy1][iz]) * -isg0.z);
						pt2d[0].set(ptc[ix0], ptc[iy0]) += axisx2d * pbox[ibox ^ 1]->size[ix1] +
						                                   axisy2d * pbox[ibox ^ 1]->size[iy1];
						sz2d.set(axes[idir1][ix0], axes[idir1][iy0]) *=
						    pbox[ibox ^ 1]->size[idir1];
						pt2d[1] = pt2d[0] + sz2d;
						pt2d[0] -= sz2d;
						...
						if (len2(pt2d[0] - pt2d[1]) > sqr(pmode->minPtDist))
						{
							...
							for (i = 0; i < 2; i++)
							{
								pt3d[ix0] = pt2d[i].x;
								pt3d[iy0] = pt2d[i].y;
								parea->pt[i] = pt3d * pbox[ibox]->Basis +
								               pbox[ibox]->center +
								               pmode->dir * (pcontact->t * (ibox ^ 1));
								parea->piPrim[0][i] = parea->piPrim[1][i] = 0;
							}
							parea->npt = 2;
						}
...
int box_cylinder_lin_unprojection(unprojection_mode* pmode, const primitives::box* pbox, int iFeature1,
                                  const primitives::cylinder* pcyl, int iFeature2, contact* pcontact,
                                  geom_contact_area* parea)
{
	...
	for (i = 0; i < 8; i++)
	{
		...
		if (isnonneg(kd) & (isneg(kb) | isneg((kb * kb) - kd)))
		{
			...
			bSeparated = isneg((pt.x * isg.x) - e) & isneg((pt.y * isg.y) - e) &
			             isneg((pt.z * isg.z) - e); // no outgoing box edge points 'inside' the cylinder
			bBest = isneg(t - tmin) & bSeparated;
			update_idbest(t, tmin, 0x40 | i, bBest, idbest);
			dir_best = dir_best * (bBest ^ 1) + dir * bBest;
		}
	}
	...
	if (bCapped)
	{
		...
		if (bFindMinUnproj)
		{
			dir = dir_best; // finding real minimizing edge-capedge unprojection involves solving 4th order
			                // eq., not worth the effort here
		}
		tmax.set(0, 1);
			...
			if (fabs_tpl(axis[ic.z]) > 0.01)
			{
				...
				for (i = 0; i < 8; i++)
				{
					...
					if (isnonneg(kd) & (isneg(kb) | isneg((kb * kb) - kd)))
					{
						...
						bSeparated = isneg(
						    ((pt ^ axis) * cross_with_ort(axis, ic.z)) * axis[ic.z] *
						    -icap);     // intersection along the edge lies outside the cylinder
						pt = axis ^ pt; // tangential direction at cap point, neither positive
						                // nor flipped should point inside the box
		...
		update_idbest(t, tmin, i, bBest, idbest);
	}
	...
	switch (idbest & 0xE0)
	{
		case 0x00: // box.face - cyl.capedge
			...
			ic.x = inc_mod3[ic.z];
			ic.y = dec_mod3[ic.z];
			bContact = isneg(fabs_tpl(pt[ic.x] - (dir[ic.x] * pcontact->t)) - size[ic.x] - e) &
		...
		case 0x60: // box.edge - cyl.side
			...
			pt += dir * pcontact->t;
			bContact =
			    isneg(fabs_tpl(t0.x) - size[ic.z] - e) & isneg(fabs_tpl((pt - center) * axis) - hh - e);
			pcontact->pt = pt * pbox->Basis + pbox->center;
	...
	if (parea)
	{
		...
		if (fabs_tpl(axis.x) + fabs_tpl(axis.y) + fabs_tpl(axis.z) < (real)1.07)
		{                  // axis is close to some coordinate axis
			...
			if (ic.z >= 0)
			{
				...
				ic.y = dec_mod3[ic.z];
				isg.z = -sgnnz(dir[ic.z]);
				dlen = sqr(max((real)0.0, fabs_tpl(c[ic.x]) - size[ic.x])) +
				       sqr(max((real)0.0, fabs_tpl(c[ic.y]) - size[ic.y]));
				// check for box.face-cyl.cap area contact
				if (fabs_tpl(fabs_tpl(c[ic.z]) - size[ic.z]) < min(size[ic.z], hh) * tol &&
				    dlen < r * r)
				{
					if (bCapped)
					{
						...
						pt2d.set(r, 0);
						for (i = 0; i < 8; i++)
						{ // add points on cap edge with 45 degrees step (if they are inside)
						...
						for (i = 0; i < 4; i++)
						{
							if ((kd = (r * r) - sqr((pt[ic[i & 1]] =
							                             size[ic[i & 1]] * ((i & 2) - 1)) -
							                        c[ic[i & 1]])) > 0)
							{
								for (j = 0, pt[ic[(i & 1) ^ 1]] = c[ic[(i & 1) ^ 1]] -
								                                  (kd = sqrt_tpl(kd));
								     j < 2; j++, pt[ic[(i & 1) ^ 1]] += kd * 2)
								{
									if (fabs_tpl(pt[ic[(i & 1) ^ 1]]) <
									    size[ic[(i & 1) ^ 1]])
									{
										parea->pt[parea->npt] =
										    pt * pbox->Basis + pbox->center +
										    pmode->dir * pcontact->t;
										parea->piPrim[0][parea->npt] =
		...
		for (ic.z = 0; ic.z < 3; ic.z++)
		{
			if (fabs_tpl(axis[ic.z]) < (real)0.04)
			{
				...
				{ // check for box.edge-cyl.cap area contact
					...
					pt[ic.z] = 0;
					for (j = 0, i = 1; i < 4; i++)
					{ // find the edge in direction ic.z that is the closest to the cap plane
					...
					if (fabs_tpl((c - pt) * axis) < hh * tol && dlen < r * r)
					{
						...
						if (c[ic.z] - dlen < size[ic.z] || c[ic.z] + dlen > -size[ic.z])
						{
							for (i = 0; i < 2; i++)
							{
								pt[ic.z] = min(
								    size[ic.z],
								    max(-size[ic.z], c[ic.z] + (dlen * (i * 2 - 1))));
								parea->pt[i] = pt * pbox->Basis + pbox->center +
								               pmode->dir * pcontact->t;
							}
							parea->type = geom_contact_area::polyline;
...
int cylinder_box_lin_unprojection(unprojection_mode* pmode, const primitives::cylinder* pcyl, int iFeature1,
                                  const primitives::box* pbox, int iFeature2, contact* pcontact,
                                  geom_contact_area* parea)
{
	pmode->dir.Flip();
...
int sphere_box_lin_unprojection(unprojection_mode* pmode, const primitives::sphere* psph, int iFeature1,
                                const primitives::box* pbox, int iFeature2, contact* pcontact, geom_contact_area* parea)
{
	pmode->dir.Flip();
...
int box_capsule_lin_unprojection(unprojection_mode* pmode, const primitives::box* pbox, int iFeature1,
                                 const primitives::capsule* pcaps, int iFeature2, contact* pcontact,
                                 geom_contact_area* parea)
{
	float tmin0 = pmode->tmin;
...
int capsule_box_lin_unprojection(unprojection_mode* pmode, const primitives::capsule* pcaps, int iFeature1,
                                 const primitives::box* pbox, int iFeature2, contact* pcontact,
                                 geom_contact_area* parea)
{
	pmode->dir.Flip();
...
int sphere_cylinder_lin_unprojection(unprojection_mode* pmode, const primitives::sphere* psph, int iFeature1,
                                     const primitives::cylinder* pcyl, int iFeature2, contact* pcontact,
                                     geom_contact_area* parea)
{
	pmode->dir.Flip();
...
int sphere_sphere_lin_unprojection(unprojection_mode* pmode, const primitives::sphere* psph1, int iFeature1,
                                   const primitives::sphere* psph2, int iFeature2, contact* pcontact,
                                   geom_contact_area* parea)
{
	Vec3r dc, vec0, vec1;
...
int cyl_cyl_lin_unprojection(unprojection_mode* pmode, const primitives::cylinder* pcyl1, int iFeature1,
                             const primitives::cylinder* pcyl2, int iFeature2, contact* pcontact,
                             geom_contact_area* parea)
{
	...
	axisx =
	    sina > 0.001f ? (Vec3r)(axisx * (rsina = 1 / sina)) : (Vec3r)(pcyl[0]->axis.GetOrthogonal().normalized());

	// cap edge - cap face
	if (bCapped[0] & bCapped[1])
	{
		for (icyl = 0; icyl < 2; icyl++)
		{ // icyl provides cap edge, icyl^1 - cap face
			if (bFindMinUnproj)
			{
				dir = axis[1] =
				    pcyl[icyl ^ 1]->axis *
				    sgnnz(pcyl[icyl ^ 1]->axis * (pcyl[icyl]->center - pcyl[icyl ^ 1]->center));
			}
			else
			{
				axis[1] =
				    pcyl[icyl ^ 1]->axis * (sgnnz(pcyl[icyl ^ 1]->axis * pmode->dir) * (1 - icyl * 2));
				dir = pmode->dir;
			}
			dir *= 1 - (icyl * 2);
			...
			{ // in case of degenerate n (�o-axis cylinders) mask it to point to center, not to plane
				n = center[1] - center[0];
			...
			update_idbest(t, tmax, icyl, bBest, idbest);
		}
	}
	...
	{
		if (bCapped[icyl])
		{ // icyl provides edge, icyl^1 - side
			...
			for (i = -1; i <= 1; i += 2)
			{
				dc = pcyl[icyl ^ 1]->center - pcyl[icyl]->center -
				...
				if (bFindMinUnproj)
				{
					...
					do
					{
						...
						pt2d[1].set(g_costab[iter] * pt2d[3].x,
						            g_sintab[iter] * pt2d[3].y); // ellipse normal
						idx[isneg(pt2d[1] ^ c2d - pt2d[0]) ^ j1] = iter;
					}
					while (idx[1] - idx[0] > 1);

					dist = len2(pt2d[1] = pt2d[0] - c2d);
					if (pt2d[1] * pt2d[0] < 0 && dist < sqr(r1) &&
					    r1 - (dist = sqrt_tpl(dist)) < tmax)
					{
						ptbest2d.set(pt2d[0].x, g_sintab[iter] * sg.y * r0);
						dp = dc - axisx * pt2d[0].x -
						     (pcyl[icyl]->axis ^ axisx) * (ptbest2d.y * sgnnz(cosa));
						bNoContact =
						    isneg(pcyl[icyl ^ 1]->hh - fabs_tpl(dp * pcyl[icyl ^ 1]->axis));
						tmax = r1 - dist;
				...
				else
				{
					...
					else
					{
						...
						{
							// find t0,t1 where the circle enters the inner and outer
							// boundaries respectively
							denom = 1 / sqrt_tpl(
									sqr(dir2d.y) +
									sqr(dir2d.x *
							                    cosa)); // pt2d[0] - dir-ellipse touch point
							...
							pt2d[1] =
							    ...
							    (denom * r0 *
							     fabs_tpl(
								 cosa)); // pt2d[1] - dir-ellipse intersection point
							denom =
							    1 / sqrt_tpl(sqr(dir2d.x) +
							                 sqr(dir2d.y *
							                     cosa)); // pt2d[2] - dir.rot90cw()-ellipse
							                             // touch point
							...
							pt2d[3] =
							    dir2d.rot90cw() * ((dir2d.rot90cw() * pt2d[0]) * denom) +
							    pt2d[2];
							j = 0;
						}
						else
						...
						for (; j < 2; j++)
						{
							...
							t.set((r1 * len(l)) -
							          ((c2d - pt2d[j * 2] ^ l) * sgn(dir2d ^ l)),
							      fabs_tpl(dir2d ^ l));
							if (inrange(((c2d - pt2d[j * 2]) * t.y + dir2d * t.x) * l,
							            (real)0, len2(l) * t.y) &&
							    t > t0)
							{
								if (t < pmode->tmax)
					...
					if (fabs_tpl(cosa) > 0.003f)
					{
						real a0 = (r0 * r0 * (1 + sqr(cosa))) + (r1 * r1),
						     b0 = sqr(r0 * r0 * cosa) + (sqr(r1 * r0) * (1 + sqr(cosa))), cx2,
						     cy2, p, q, Q, Q0, Q1;
						const auto calcQ = [&]() {
	...
	for (i = 0; i < 2; i++, t.x += (pcyl[0]->r + pcyl[1]->r) * 2)
	{
		...
		bContact = isneg(fabs_tpl(t0.x) - (t0.y * pcyl1->hh * sina)) &
		           isneg(fabs_tpl(t1.x) - (t1.y * pcyl2->hh * sina));
		if (bFindMinUnproj)
		{
		...
		update_idbest(t, tmax, 0xC0, bBest, idbest);
	}

	// cap edge - cap edge
	if (sina * (bCapped[0] & bCapped[1]) > 0.001f)
	{
		// real edge-edge min unprojection will require iterative solving, so instead use the best unprojection
		// dir found so far
		dir = bFindMinUnproj ? (Vec3r)dir_best : (Vec3r)pmode->dir;
		...
		for (i = -1; i <= 1; i += 2)
		{
			...
			for (j = -1; j <= 1; j += 2)
			{
				...
				polynomial_tpl<real, 1> udc = P1(u * dir) + u * dc, a1dc = P1(a1 * dir) + a1 * dc,
							ldc = P1(l * dir) + l * dc;
				polynomial_tpl<real, 2> dc2 = P2(dir.len2()) + P1(dir * dc) * 2 + dc * dc;
				polynomial_tpl<real, 4> colls = (r0 * r0 - a1dc.sqr()) * ldc.sqr() * 4 -
				                                (udc * a1dc * 2 + dc2 + r0 * r0 - r1 * r1).sqr();
				if (colls.nroots(0, pmode->tmax))
				{
					...
					if (bFindMinUnproj)
					{
						for (nroots--; nroots >= 0; nroots--)
						{
							...
							bSeparated &=
							    isneg((pcyl[1]->hh * 0.999f) -
							          fabs_tpl((center[0] + dir * t.x - pcyl[1]->center) *
							                   pcyl[1]->axis));
							if (bSeparated)
							{
								break;
	...
	switch (idbest & 0xC0)
	{
		case 0: // cap edge - cap face
			...
			center[0] = axis[0] * pcyl[icyl]->hh + pcyl[icyl]->center;
			pcontact->t = tmax.val();
			pcontact->pt = center[0] + ncap[icyl] * (pcyl[icyl]->r / ncaplen[icyl]) +
			               pmode->dir * (pcontact->t * (icyl ^ 1));
		...
		case 0x40: // cap edge - side
			...
			i = (idbest * 2 & 2) - 1;
			pcontact->t = tmax.val();
			pcontact->pt = pcyl[icyl]->center + pcyl[icyl]->axis * (pcyl[icyl]->hh * i) +
			               pmode->dir * (pcontact->t * (1 - icyl)) + axisx * ptbest2d.x +
			               (pcyl[icyl]->axis ^ axisx) * (ptbest2d.y * sgnnz(cosa));
		...
		case 0x80: // cap edge - cap edge
			...
			pcontact->pt +=
			    l * (sqrt_tpl(fabs_tpl((r0 * r0) - (a * a))) * sgnnz(l * (center[1] - pcontact->pt)));
			pcontact->n =
	...
	if (parea)
	{
		...
		if (sina < 0.017f)
		{
			if (fabs_tpl(fabs_tpl(dc * pcyl[0]->axis) - (pcyl[0]->hh + pcyl[1]->hh)) <
			    min(pcyl[0]->hh, pcyl[1]->hh) * 0.05f * (bCapped[0] & bCapped[1]))
			{
				// check for cap-cap area contact
				i = -sgnnz(pcyl[0]->axis * dc);
				j = sgnnz(pcyl[1]->axis * dc);
				center[0] =
				    pcyl[0]->center + pmode->dir * pcontact->t + pcyl[0]->axis * (pcyl[0]->hh * i);
				...
				if (inrange(d, sqr(pcyl[0]->r - pcyl[1]->r) * (real)1.001f,
				            sqr(pcyl[0]->r + pcyl[1]->r) * (real)0.999f))
				{
					b = 1.0 / (d = sqrt_tpl(d));
			...
			if (fabs_tpl((dc - pcyl[0]->axis * (dc * pcyl[0]->axis)).len2() -
			             sqr(pcyl[0]->r + pcyl[1]->r)) < min(pcyl[0]->r, pcyl[1]->r) * 0.05f)
			{
				...
				for (icyl = 0; icyl < 2; icyl++)
				{
					for (i = -1; i <= 1; i += 2)
					{
						if (fabs_tpl((dc * (1 - (icyl * 2)) +
						              pcyl[icyl]->axis * (pcyl[icyl]->hh * i)) *
						             pcyl[icyl ^ 1]->axis) < pcyl[icyl ^ 1]->hh * 1.002f)
						{
							parea->pt[parea->npt] =
							    pcyl[icyl]->center +
							    pmode->dir * (pcontact->t * (1 - icyl)) +
							    pcyl[icyl]->axis * (pcyl[icyl]->hh * i) +
		...
		if (fabs_tpl(cosa) < 0.017f)
		{
			...
			{
				if (fabs_tpl((dc ^ pcyl[icyl ^ 1]->axis).len2() -
				             sqr(pcyl[icyl]->hh + pcyl[icyl ^ 1]->r)) <
				    sqr(min(pcyl[icyl]->hh, pcyl[icyl ^ 1]->r)) * 0.08f * bCapped[icyl])
				{
					...
					if (d > 0)
					{
						...
						for (j = -1; j <= 1; j += 2)
						{
							if ((dc + pcyl[icyl ^ 1]->axis * (pcyl[icyl ^ 1]->hh * j) ^
							     pcyl[icyl]->axis)
							        .len2() < sqr(pcyl[icyl]->r))
							{
								parea->pt[parea->npt] =
						...
						{
							dp =
							    pcyl[icyl]->axis * (pcyl[icyl]->axis * (dc + parea->pt[j]) -
							                        (pcyl[icyl]->hh * i));
							parea->pt[j] += pcyl[icyl ^ 1]->center +
							                pmode->dir * (pcontact->t * icyl) - dp;
						}
					}
				}
			}
...
int cylinder_capsule_lin_unprojection(unprojection_mode* pmode, const primitives::cylinder* pcyl, int iFeature1,
                                      const primitives::capsule* pcaps, int iFeature2, contact* pcontact,
                                      geom_contact_area* parea)
{
	float tmin0 = pmode->tmin;
...
int capsule_cylinder_lin_unprojection(unprojection_mode* pmode, const primitives::capsule* pcaps, int iFeature1,
                                      const primitives::cylinder* pcyl, int iFeature2, contact* pcontact,
                                      geom_contact_area* parea)
{
	pmode->dir.Flip();
...
int capsule_capsule_lin_unprojection(unprojection_mode* pmode, const primitives::capsule* pcaps1, int iFeature1,
                                     const primitives::capsule* pcaps2, int iFeature2, contact* pcontact,
                                     geom_contact_area* parea)
{
	float tmin0 = pmode->tmin;
	int bContact, bContactSph, i, icaps;
	primitives::sphere sph, sph1;
	const primitives::capsule* pcaps[2] = {pcaps1, pcaps2};
	pcontact->t = 0;
	...
	if (pmode->dir.len2() == 0)
	{
		...
		for (icaps = 0; icaps < 2; icaps++)
		{
			for (i = -1; i <= 1; i += 2)
			{
				...
				sph.r = pcaps[icaps]->r;
				pmode->dir.zero();
				if (cylinder_sphere_lin_unprojection(pmode, pcaps[icaps ^ 1], 0x43, &sph, -1, &capcont,
				                                     parea) &&
				    capcont.t > pcontact->t)
				{
					if (icaps == 0)
					{
						capcont.pt -= pmode->dir * capcont.t, capcont.n.Flip(),
						    pmode->dir.Flip();
					}
					*pcontact = capcont;
					dir_best = pmode->dir;
				...
				sph1.center = pcaps[icaps ^ 1]->center +
				              pcaps[icaps ^ 1]->axis * (pcaps[icaps ^ 1]->hh * i * (1 - icaps * 2));
				sph1.r = pcaps[icaps ^ 1]->r;
				pmode->dir.zero();
				if (sphere_sphere_lin_unprojection(pmode, &sph1, -1, &sph, -1, &capcont, parea) &&
				    (max(capcont.t - pcontact->t,
				         min(capcont.t, (real)(capcont.n * pcaps[icaps]->axis) * -i)) > 0))
				{
					if (icaps == 0)
					{
						capcont.pt -= pmode->dir * capcont.t, capcont.n.Flip(),
	...
	else
	{
		...
		for (pmode->dir.Flip(), icaps = 0; icaps < 2; icaps++, pmode->dir.Flip())
		{
			for (i = -1; i <= 1; i += 2)
			{
				sph.center = pcaps[icaps]->center + pcaps[icaps]->axis * (pcaps[icaps]->hh * i);
				sph.r = pcaps[icaps]->r;
				bContact |= (bContactSph = cylinder_sphere_lin_unprojection(
						 pmode, pcaps[icaps ^ 1], 0x43, &sph, -1, pcontact, parea));
				...
				sph1.center = pcaps[icaps ^ 1]->center +
				              pcaps[icaps ^ 1]->axis * (pcaps[icaps ^ 1]->hh * i * (1 - icaps * 2));
				sph1.r = pcaps[icaps ^ 1]->r;
				bContact |= (bContactSph = sphere_sphere_lin_unprojection(pmode, &sph1, -1, &sph, -1,
				                                                          pcontact, parea));
				pmode->tmin += (pcontact->t - pmode->tmin) * bContactSph;
				if (bContactSph & (icaps ^ 1))
				{
...
int sphere_capsule_lin_unprojection(unprojection_mode* pmode, const primitives::sphere* psph, int iFeature1,
                                    const primitives::capsule* pcaps, int iFeature2, contact* pcontact,
                                    geom_contact_area* parea)
{
	float tmin0 = pmode->tmin;
...
int capsule_sphere_lin_unprojection(unprojection_mode* pmode, const primitives::capsule* pcaps, int iFeature1,
                                    const primitives::sphere* psph, int iFeature2, contact* pcontact,
                                    geom_contact_area* parea)
{
	pmode->dir.Flip();
...
int ray_box_lin_unprojection(unprojection_mode* pmode, const primitives::ray* pray, int iFeature1,
                             const primitives::box* pbox, int iFeature2, contact* pcontact, geom_contact_area* parea)
{
	...
	// ray end - box face
	for (j = 0; j < 3; j++)
	{ // j-box face (with sign in direction of movement)
		i = 0;
		mindist = pt[0][j] * sgdir[j]; // find triangle vertex that is the farthest from corresponding box face
		                               // (along its inside normal)
		dist = pt[1][j] * sgdir[j];
...
int box_ray_lin_unprojection(unprojection_mode* pmode, const primitives::box* pbox, int iFeature1,
                             const primitives::ray* pray, int iFeature2, contact* pcontact, geom_contact_area* parea)
{
	pmode->dir.Flip();
...
int ray_cylinder_lin_unprojection(unprojection_mode* pmode, const primitives::ray* pray, int iFeature1,
                                  const primitives::cylinder* pcyl, int iFeature2, contact* pcontact,
                                  geom_contact_area* parea)
{
	Vec3r n, center, dp, edge, vec0, vec1, ptcyl;
	...
	if (d >= 0)
	{
		...
		bBest = bContact & isneg(tmax - t) & isneg(t.x - (pmode->tmax * t.y));
		update_idbest(t, tmax, 0x60, bBest, idbest);
	}
	...
	if (bCapped)
	{
		...
		// we also need to process degenerate cases when te ray lies in cap plane (edge-cap edge check doesn't
		// detect them)
		if (sqr(edge * pcyl->axis) < edge.len2() * 1E-4)
		{ // potential ray - cylinder cap contact
			j = sgnnz(pcyl->axis * pmode->dir);
			center = pcyl->center + pcyl->axis * pcyl->hh * j;
			dp = center - pray->origin;
			t.set(((dp * pcyl->axis) * edge.len2()) -
			          ((edge * pcyl->axis) *
			           (edge * dp)), // calculate when the closest point on edge will touch the cap
			...
			if (t > tmax && t < pmode->tmax)
			{
				...
				if (fabs_tpl((t0.x * 2) - t0.y) < t0.y &&
				    (edge * t0.x + (pcontact->pt - center) * t0.y).len2() < r2 * sqr(t0.y))
				{
					pcontact->pt += edge * (t0.val());
				}
				else if ((pcontact->pt + edge - center).len2() < r2)
...
int cylinder_ray_lin_unprojection(unprojection_mode* pmode, const primitives::cylinder* pcyl, int iFeature1,
                                  const primitives::ray* pray, int iFeature2, contact* pcontact,
                                  geom_contact_area* parea)
{
	pmode->dir.Flip();
...
int ray_sphere_lin_unprojection(unprojection_mode* pmode, const primitives::ray* pray, int iFeature1,
                                const primitives::sphere* psphere, int iFeature2, contact* pcontact,
                                geom_contact_area* parea)
{
	quotient t, tmax(pmode->tmin, 1), t0;
	...
	switch (idbest & 0xC0)
	{
		...
		case 0x80:
			t0.set(((psphere->center - pray->origin) * tmax.y - pmode->dir * tmax.x) * edge,
			       tmax.y * edge.len2());
			t0.y = 1.0 / t0.y;
			pcontact->t = tmax.x * t0.y * edge.len2();
			pcontact->pt = pray->origin + edge * (t0.x * t0.y) + pmode->dir * pcontact->t;
...
int sphere_ray_lin_unprojection(unprojection_mode* pmode, const primitives::sphere* psphere, int iFeature1,
                                const primitives::ray* pray, int iFeature2, contact* pcontact, geom_contact_area* parea)
{
	pmode->dir.Flip();
...
int ray_capsule_lin_unprojection(unprojection_mode* pmode, const primitives::ray* pray, int iFeature1,
                                 const primitives::capsule* pcaps, int iFeature2, contact* pcontact,
                                 geom_contact_area* parea)
{
	float tmin0 = pmode->tmin;
...
int capsule_ray_lin_unprojection(unprojection_mode* pmode, const primitives::capsule* pcaps, int iFeature1,
                                 const primitives::ray* pray, int iFeature2, contact* pcontact,
                                 geom_contact_area* parea)
{
	pmode->dir.Flip();
...
int tri_tri_rot_unprojection(unprojection_mode* pmode, const primitives::triangle* ptri1, int iFeature1,
                             const primitives::triangle* ptri2, int iFeature2, contact* pcontact,
                             geom_contact_area* parea)
{
	const primitives::triangle* ptri[2] = {ptri1, ptri2};
	Vec3r pt, ptz[2][3], ptx[2][3], pty[2][3], rotax, pt0, pt1, edge0, edge1, n0z, n0x, n0y, ptz0, ptx0, pty0, n,
	    pt0_rot, pt1_rot, edge0_rot, ptz1, ptx1, pty1, dir0, dir1, norm[2];
				...
				if (d >= 0)
				{
					...
					for (j = 0; j < 2; j++, tsin.x += d * 2)
					{
						if (isneg(fabs_tpl((tsin.x * 2) - tsin.y) - tsin.y) & // tsin in (0..1)
						    isneg((ksin * tsin.x + k * tsin.y) *
						          kcos)) // tcos in (0..1) (=eliminate foreign root)
						{
							...
							bContact = // point is inside itri
							    isneg((pt - ptri[itri]->pt[0] * tsin.y ^
							           ptri[itri]->pt[1] - ptri[itri]->pt[0]) *
							          ptri[itri]->n) &
							    isneg((pt - ptri[itri]->pt[1] * tsin.y ^
							           ptri[itri]->pt[2] - ptri[itri]->pt[1]) *
							          ptri[itri]->n) &
							    isneg((pt - ptri[itri]->pt[2] * tsin.y ^
							           ptri[itri]->pt[0] - ptri[itri]->pt[2]) *
							          ptri[itri]->n);
							// additionally check that triangles don't intersect during
							// contact
							pt = ptx[itri ^ 1][inc_mod3[i]] * tcos.x +
							     pty[itri ^ 1][inc_mod3[i]] * tsin.x +
							     (ptz[itri ^ 1][inc_mod3[i]] + pmode->center -
							      ptri[itri]->pt[0]) *
							         tsin.y;
							sg0 = pt * ptri[itri]->n + (pmode->minPtDist * tsin.y);
							pt = ptx[itri ^ 1][dec_mod3[i]] * tcos.x +
							     pty[itri ^ 1][dec_mod3[i]] * tsin.x +
							     (ptz[itri ^ 1][dec_mod3[i]] + pmode->center -
							      ptri[itri]->pt[0]) *
							         tsin.y;
							sg1 = pt * ptri[itri]->n + (pmode->minPtDist * tsin.y);
							bContact &= isnonneg(sg0 * sg1);
							bBest = bContact & isneg(tsin - tmin);
							update_idbest(tsin, tmin, itri << 2 | i, bBest, idbest);
						}
					}
				}
			}
			else
			{ // check if triangles are already separated in their initial state
				sg0 = (ptri[itri ^ 1]->pt[inc_mod3[i]] - ptri[itri]->pt[0]) * ptri[itri]->n +
				      pmode->minPtDist;
				sg1 = (ptri[itri ^ 1]->pt[dec_mod3[i]] - ptri[itri]->pt[0]) * ptri[itri]->n +
				      pmode->minPtDist;
	...
	for (i = 0; i < 3; i++)
	{
		...
		for (j = 0; j < 3; j++)
		{
			if ((0xA0 | i | (0xA0 | j) << 8) != imask)
			{
				...
				if (d >= 0)
				{
					...
					for (j1 = 0; j1 < 2; j1++, tsin.x += d * 2)
					{
						...
						{
							tcos.set(sqrt_tpl(sqr(tsin.y) - sqr(tsin.x)), tsin.y);
							pt0_rot = ptx[0][i] * tcos.x + pty[0][i] * tsin.x +
							          ptz[0][i] * tsin.y;
							pt1_rot = ptx[0][inc_mod3[i]] * tcos.x +
							          pty[0][inc_mod3[i]] * tsin.x +
							          ptz[0][inc_mod3[i]] * tsin.y;
							...
							bContact = isneg(fabs_tpl((t0.x * 2) - t0.y) - t0.y) &
							           isneg(fabs_tpl((t1.x * 2) - t1.y) - t1.y);
							// additionally check that triangles don't intersect during
							// contact
							...
							bBest = bContact & isneg(tsin - tmin);
							update_idbest(tsin, tmin, i << 2 | j | 0x80, bBest, idbest);
						}
					}
				}
			}
			...
			{ // check if triangles are already separated in their initial state
				...
				tsin.set(0, 1);
				// additionally check that edges intersection point will not move inside the 2nd
				// triangle during unprojection
				bBest &= isneg(
				    dir1 * (pmode->dir ^ (ptri[0]->pt[i] - pmode->center) * n.len2() +
				                             edge0 * ((ptri[1]->pt[j] - ptri[0]->pt[i] ^ edge1) * n)));
				update_idbest(tsin, tmin, i << 2 | j | 0x80, bBest, idbest);
			}
		}
	}
	...
	if (idbest & 0x80)
	{
		...
		pt1_rot = ptx[0][inc_mod3[i]] * pcontact->taux + pty[0][inc_mod3[i]] * pcontact->t +
		          ptz[0][inc_mod3[i]] + pmode->center;
		edge0_rot = pt1_rot - pt0_rot;
		edge1 = ptri[1]->pt[inc_mod3[j]] - ptri[1]->pt[j];
	...
	else
	{
		...
		pcontact->t = tmin.val();
		pcontact->taux = sqrt_tpl(1 - sqr(pcontact->t));
		pcontact->pt = ptz[itri ^ 1][i] + ptx[itri ^ 1][i] * max(pcontact->taux, (real)(itri ^ 1)) +
		               pty[itri ^ 1][i] * (pcontact->t * itri) + pmode->center;
...
int ray_tri_rot_unprojection(unprojection_mode* pmode, const primitives::ray* pray, int iFeature1,
                             const primitives::triangle* ptri, int iFeature2, contact* pcontact,
                             geom_contact_area* parea)
{
	Vec3r pt, ptz[2], ptx[2], pty[2], rotax = pmode->dir, edge1, ptz0, ptx0, pty0, pt1, pt0_rot, pt1_rot, edge0_rot,
					  ptz1, ptx1, pty1, n;
	quotient tsin, tcos, tmin(pmode->tmax, 1), t0, t1;
		...
		if (d >= 0)
		{
			...
			for (j = 0; j < 2; j++, tsin.x += d * 2)
			{
				    ...
				    isneg((ksin * tsin.x + k * tsin.y) * kcos))       // tcos in (0..1)
				{
					...
					bBest = bContact & isneg(tsin - tmin);
					update_idbest(tsin, tmin, i, bBest, idbest);
				}
			}
		}
				...
				for (j = 0; j < 2; j++, tsin.x += d * 2)
				{
					    ...
					    isneg((ksin * tsin.x + k * tsin.y) * kcos))       // tcos in (0..1)
					{
						...
						bContact = isneg(fabs_tpl((t0.x * 2) - t0.y) - t0.y) &
						           isneg(fabs_tpl((t1.x * 2) - t1.y) - t1.y);
						bBest = bContact & isneg(tsin - tmin);
						update_idbest(tsin, tmin, i | 0x80, bBest, idbest);
					}
				}
			}
		}
...
int tri_ray_rot_unprojection(unprojection_mode* pmode, const primitives::triangle* ptri, int iFeature1,
                             const primitives::ray* pray, int iFeature2, contact* pcontact, geom_contact_area* parea)
{
...
int ray_cyl_rot_unprojection(unprojection_mode* pmode, const primitives::ray* pray, int iFeature1,
                             const primitives::cylinder* pcyl, int iFeature2, contact* pcontact,
                             geom_contact_area* parea)
{
	Vec3r center, ccap, v, vx, vy, vz, l, lx, ly, lz, vsin, vcos, pt, ptz[2], ptx[2], pty[2], rotax = pmode->dir,
												  axis = pcyl->axis;
	...
	for (i = 1; i >= 0; i--)
	{
		...
		for (icap = -1; icap <= 1; icap += 2)
		{
			...
			if (d >= 0)
			{
				...
				for (j = 0; j < 2; j++, tsin.x += d * 2)
				{
					if (isneg(fabs_tpl((tsin.x * 2) - tsin.y) - tsin.y) & // tsin in (0..1)
					    isneg((ksin * tsin.x + k * tsin.y) *
					          kcos)) // tcos in (0..1) = remove phantom roots
					{
						tcos.set(sqrt_tpl(sqr(tsin.y) - sqr(tsin.x)), tsin.y);
						pt = ptx[i] * tcos.x + pty[i] * tsin.x + ptz[i] * tsin.y;
						bContact = isneg((pt - (center + axis * (hh * icap)) * tsin.y).len2() -
						                 (r2 * sqr(tsin.y)));
						bBest = bContact & isneg(tmax - tsin);
						update_idbest(tsin, tmax, (icap + 1) | i, bBest, idbest);
					}
				}
			}
		}
		...
		vcos = ptx[i] ^ axis;
		vsin = pty[i] ^ axis;
		pn = psqr(P2(sqr(vsin) - sqr(vcos)) + P1((v * vsin) * 2) + sqr(v) + sqr(vcos) - r2) -
		...
		if (pn.nroots(0, 1))
		{
			for (j = pn.findroots(0, 1, roots) - 1; j >= 0; j--)
			{
				...
				bContact = isneg(fabs_tpl(pt * axis) - hh) &
				           inrange((pt ^ axis).len2(), r2 * sqr((real)0.99), r2 * sqr((real)1.01));
				bBest = isneg(tmax - tsin) & bContact;
				update_idbest(tsin, tmax, 0x10 | i, bBest, idbest);
			}
		}
	...
	vsin = ly ^ axis;
	v = lz ^ axis;
	pn = psqr(P2(sqr(ksin) - sqr(kcos) - (r2 * (sqr(vsin) - sqr(vcos)))) + P1(((ksin * k) - r2 * (vsin * v)) * 2) +
	          sqr(kcos) + sqr(k) - r2 * (sqr(vcos) + sqr(v))) -
	...
	if (pn.nroots(0, 1))
	{
		for (j = pn.findroots(0, 1, roots) - 1; j >= 0; j--)
		{
			...
			v = l ^ axis;
			k = v.len2();
			bContact = inrange(sqr(pt * v), r2 * k * sqr((real)0.99),
			                   r2 * k * sqr((real)1.01)); // remove phantom roots
			t.set((-pt ^ axis) * v, k);
			(pt *= t.y) += l * t.x;
			bContact &= inrange(t.x, (real)0, t.y);
			bContact &= isneg(fabs_tpl(pt * axis) - (hh * t.y));
			bBest = isneg(tmax - tsin) & bContact;
			update_idbest(tsin, tmax, 0x20, bBest, idbest);
		}
	}
	...
	for (icap = -1; icap <= 1; icap += 2)
	{
		...
		ksin = ly * axis;
		k = lz * axis;
		pn = psqr(P2(sqr(vsin) - sqr(vcos) - (r2 * (sqr(ksin) - sqr(kcos)))) +
		          P1(((vsin * v) - r2 * (ksin * k)) * 2) + sqr(vcos) + sqr(v) - r2 * (sqr(kcos) + sqr(k))) -
		...
		if (pn.nroots(0, 1))
		{
			for (j = pn.findroots(0, 1, roots) - 1; j >= 0; j--)
			{
				...
				bContact = inrange(((pt - ccap) * t.y + l * t.x).len2(), r2 * sqr(t.y * (real)0.96),
...
int cyl_ray_rot_unprojection(unprojection_mode* pmode, const primitives::cylinder* pcyl, int iFeature1,
                             const primitives::ray* pray, int iFeature2, contact* pcontact, geom_contact_area* parea)
{
	(const_cast<unprojection_mode*>(pmode))->dir.Flip();
...
int ray_box_rot_unprojection(unprojection_mode* pmode, const primitives::ray* pray, int iFeature1,
                             const primitives::box* pbox, int iFeature2, contact* pcontact, geom_contact_area* parea)
{
	int i, j, ifeat, idir, ix, iy, bContact, bBest, idbest = -1;
	Vec3r pt, ptz[2], ptx[2], pty[2], ptz0, ptx0, pty0, pt1, pt0_rot, pt1_rot, edge0_rot, ptz1, ptx1, pty1, origin,
	    dir, center, rotax, n, size;
				...
				for (j = 0; j < 2; j++, tsin.x += d * 2)
				{
					          ...
					          kcos)) // tcos in (0..1) = remove phantom roots
					{
						...
						bContact = // point is inside the face
						    isneg(fabs_tpl(pt[ix]) - (size[ix] * tsin.y)) &
						    isneg(fabs_tpl(pt[iy]) - (size[iy] * tsin.y));
						bBest = bContact & isneg(tmax - tsin);
						update_idbest(tsin, tmax, i | ifeat << 1, bBest, idbest);
					}
				}
			}
		}
		...
		if (d >= 0)
		{
			...
			for (j = 0; j < 2; j++, tsin.x += d * 2)
			{
				    ...
				    isneg((ksin * tsin.x + k * tsin.y) * kcos))       // tcos in (0..1)
				{
					...
					t1.set((pt ^ edge0_rot) * n, t0.y * tsin.y);
					bContact = inrange(t0.x, (real)0, t0.y) &
					           isneg(fabs_tpl(t1.x) - (fabs_tpl(t1.y) * size[idir]));
					bBest = bContact & isneg(tmax - tsin);
					update_idbest(tsin, tmax, ifeat | 0x80, bBest, idbest);
				}
			}
		}
	}
	...
	if ((idbest & 0x80) == 0)
	{
		...
		pcontact->taux = sqrt_tpl(1 - sqr(pcontact->t));
		pcontact->pt =
		    (ptz[i] + ptx[i] * pcontact->taux + pty[i] * pcontact->t + center) * pbox->Basis + pbox->center;
...
int box_ray_rot_unprojection(unprojection_mode* pmode, const primitives::box* pbox, int iFeature1,
                             const primitives::ray* pray, int iFeature2, contact* pcontact, geom_contact_area* parea)
{
	(const_cast<unprojection_mode*>(pmode))->dir.Flip();
...
int ray_capsule_rot_unprojection(unprojection_mode* pmode, const primitives::ray* pray, int iFeature1,
                                 const primitives::capsule* pcaps, int iFeature2, contact* pcontact,
                                 geom_contact_area* parea)
{
	Vec3r center, ccap, v, vx, vy, vz, l, lx, ly, lz, vsin, vcos, pt, ptz[2], ptx[2], pty[2], rotax = pmode->dir,
	...
	for (icap = -1; icap <= 1; icap += 2)
	{
		...
		vsin = (ly ^ ccap) + vy;
		v = (lz ^ ccap) + vz;
		pn = psqr(P2(sqr(vsin) - sqr(vcos)) + P1((v * vsin) * 2) + sqr(v) + sqr(vcos) - r2 * sqr(pray->dir)) -
		     psqr(P1(vcos * vsin) + v * vcos) * ((real)1 - P2(1)) * 4;
		if (pn.nroots(0, 1))
		{
			for (j = pn.findroots(0, 1, roots) - 1; j >= 0; j--)
			{
				...
				bContact = inrange(((pt - ccap) * t.y + l * t.x).len2(), r2 * sqr(t.y * (real)0.99),
...
int capsule_ray_rot_unprojection(unprojection_mode* pmode, const primitives::capsule* pcaps, int iFeature1,
                                 const primitives::ray* pray, int iFeature2, contact* pcontact,
                                 geom_contact_area* parea)
{
	(const_cast<unprojection_mode*>(pmode))->dir.Flip();
...
int CUnprojectionChecker::Check(unprojection_mode* pmode, int type1, int type2, const primitives::primitive* prim1,
                                int iFeature1, const primitives::primitive* prim2, int iFeature2, contact* pcontact,
                                geom_contact_area* parea)
{
	return table[pmode->imode][type1][type2](pmode, prim1, iFeature1, prim2, iFeature2, pcontact, parea);
}

int CUnprojectionChecker::CheckExists(int imode, int type1, int type2)
{
