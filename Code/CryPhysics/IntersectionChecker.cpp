...
int box_ray_intersection(const primitives::box* pbox, const primitives::ray* pray, prim_inters* pinters)
{
	quotient t[2], t1[2];
...
int ray_tri_intersection(const primitives::ray* pray, const primitives::triangle* ptri, prim_inters* pinters)
{
	quotient t((ptri->pt[0] - pray->origin) * ptri->n, pray->dir * ptri->n);
	if (sqr(t.y) > pinters->minPtDist2 * sqr(1E-4f))
	{
		t.fixsign();
		Vec3r pt = pray->origin * t.y + pray->dir * t.x, edge;
		real nlen2 = ptri->n.len2() * t.y;
		int bOutside = isneg(t.y - fabsf((t.x * 2) - t.y));
		edge = ptri->pt[1] - ptri->pt[0];
		bOutside |= isneg(sqr_signed(ptri->n * (edge ^ pt - ptri->pt[0] * t.y)) +
		                  (pinters->minPtDist2 * edge.len2() * nlen2));
		edge = ptri->pt[2] - ptri->pt[1];
		bOutside |= isneg(sqr_signed(ptri->n * (edge ^ pt - ptri->pt[1] * t.y)) +
		                  (pinters->minPtDist2 * edge.len2() * nlen2));
		edge = ptri->pt[0] - ptri->pt[2];
		bOutside |= isneg(sqr_signed(ptri->n * (edge ^ pt - ptri->pt[2] * t.y)) +
		                  (pinters->minPtDist2 * edge.len2() * nlen2));
...
int default_intersection(const primitives::primitive*, const primitives::primitive*, prim_inters* pinters)
{
	return 0;
}

int tri_tri_intersection(const primitives::triangle* ptri1, const primitives::triangle* ptri2, prim_inters* pinters)
{
	...
	Vec3r ptloc, sgtest,
	    pt0 = Vec3r(ptri1->pt[0] * ptri1->n, -ptri2->pt[0] * ptri2->n, ptri1->pt[0] * axes[2]) *
	          GetMtxFromBasis(axes);
	...
	for (i = 0; i < 2; i++)
	{
		...
		const Vec3r testabs = sgtest.abs();
		imin = isneg(sgtest[idxmax3((real*)&testabs)]);
...
int tri_box_intersection(const primitives::triangle* ptri, const primitives::box* pbox, prim_inters* pinters)
{
	...
	{
		bOutside[i] = isneg(pbox->size.x - fabs_tpl(pt[i].x)) | isneg(pbox->size.y - fabs_tpl(pt[i].y)) |
		              isneg(pbox->size.z - fabs_tpl(pt[i].z));
	...
	{
		// if at least one triangle point is inside the box, use the closest box face normal as intersection
		// normal
		i = bOutside[0] + (bOutside[0] & bOutside[1]); // i - index of the 1st inside vertex
		pinters->ptbest = ptri->pt[i];
		dp(fabs_tpl(fabs_tpl(pt[i].x) - pbox->size.x), fabs_tpl(fabs_tpl(pt[i].y) - pbox->size.y),
		   fabs_tpl(fabs_tpl(pt[i].z) - pbox->size.z));
	...
	do
	{
		...
		t[i][1].set(1, 1);
		for (j = 0; j < 3; j++)
		{ // crop edge with box sides (plane sandwiches)
			idir = isneg(dp[j]);
			sg = 1 - (idir << 1);
		...
		if (t[i][1] > t[i][0]) // edge intersects the box
		{
			for (j = 0; j < 2; j++)
			{
				// bVertexContact = isneg(sqr(fabs_tpl(t[i][j].x*2-t[i][j].y)-t[i][j].y)*dplen2 -
				// pinters->minPtDist2*sqr(t[i][j].y)*4);
				bSwitch = t[i][j].isin01();
				bEnd = bSwitch & bState; // remember starting (ccw) point of every stripe (it'll serve
				                         // as end point in result)
				(iEndCur &= ~-bEnd) |= (i << 1 | j) & -bEnd;
				// if a point is a stripe start, check if it is the current closest to the suggested
				// stripe start
				dp1 = (ptri->pt[i] - pinters->pt[1]);
				dist.set((dp1 * t[i][j].y + (ptri->pt[dec_mod3[i]] - ptri->pt[i]) * t[i][j].x).len2(),
				         sqr(t[i][j].y));
	...
	if (iStartBest < 0)
	{ // triangle edges do not intersect box
	...
	if ((pt[1] - pt[0] ^ pt[2] - pt[0]).len2() > sqr(pbox->size[inc_mod3[iStart]] * pbox->size[dec_mod3[iStart]]))
	{
		...
		for (i = 0; i < 12; i++)
		{
			...
			if (inrange(t1[0].x, (real)0, pbox->size[idir] * t1[0].y * 2))
			{
				...
				{
					pinters->ptborder[pinters->nborderpt++] =
					    (ptbox / t1[0].y) * pbox->Basis + pbox->center;
				}
			}
		}
		...
		if (fabs_tpl(n[iStart]) > 0.98f && pinters->nborderpt > nborderpt0 + 1 && pinters->nBestPtVal < 6)
		{
			...
			if (isneg((ptbox - pt[0] ^ pt[1] - pt[0]) * n) & isneg((ptbox - pt[1] ^ pt[2] - pt[1]) * n) &
			    isneg((ptbox - pt[2] ^ pt[0] - pt[2]) * n))
			{
				pinters->nBestPtVal = 6;
				pinters->n = ptri->n;
...
int box_tri_intersection(const primitives::box* pbox, const primitives::triangle* ptri, prim_inters* pinters)
{
...
int tri_cylinder_intersection(const primitives::triangle* ptri, const primitives::cylinder* pcyl, prim_inters* pinters)
{
	...
	int i, j, iStart, iStartBest, iEndBest, bOutside, bEnd, iEndCur, bSwitch, bState, bBest, sg,
	    bInters; //,bVertexContact,bPrevVertexContact;
	r2 = sqr(pcyl->r);
	...
	mindist.set(1, 0);
	do
	{ // find edge-cylinder intersections
		...
		if (bInters)
		{
			...
			{
				// bVertexContact =
				// isneg(sqr(t[i][j].x-t[i][j].y)*dplen2-pinters->minPtDist2*sqr(t[i][j].y)); // t is
				// close to 1 bPrevVertexContact =
				// isneg(sqr(t[i][j].x)*dplen2-pinters->minPtDist2*sqr(t[i][j].y));	// t is close to
				// 0
	...
	{ // triangle edges do not intersect cylinder
		...
		// choose the points on cylinder caps that have the maximum and the minimum projections on triangle
		// normal
		...
		{
			bInters &=
			    isneg((ptmin - ptri->pt[i] * t[0][0].y ^ ptri->pt[inc_mod3[i]] - ptri->pt[i]) * ptri->n);
		}
		if (!bInters)
		{
			return 0;
		...
		if (inrange((pt - ptri->pt[0]) * ptri->n, (pcyl->hh + pcyl->r) * -0.2f, 0.0f) &&
		    (pt - ptri->pt[0] ^ ptri->pt[1] - ptri->pt[0]) * ptri->n < 0 &&
		    (pt - ptri->pt[1] ^ ptri->pt[2] - ptri->pt[1]) * ptri->n < 0 &&
		    (pt - ptri->pt[2] ^ ptri->pt[0] - ptri->pt[2]) * ptri->n < 0 &&
		...
		for (i = 0, pt2d.set(1, 0); i < 8; i++)
		{
			...
			pinters->nborderpt +=
			    isneg((pt1 - ptri->pt[0]) * ptri->n) & isneg(pinters->nborderpt - pinters->nbordersz);
			float t = pt2d.x;
	...
	// if cylinder cap's lowest (wrt trinagle normal) point is inside tringle's voronoi region, force triangle
	// normal as global intersection normal make this check only if the cylinder is not too below the triangle
	// surface
	...
	else if ((pt - ptri->pt[0]) * ptri->n > (pcyl->hh + pcyl->r) * -0.2f)
	{
		...
		{
			Vec3 pt1 = pinters->ptborder[pinters->nborderpt] = pt + axisx * pt2d.x + axisy * pt2d.y;
			pinters->nborderpt += isneg((pt1 - ptri->pt[0]) * ptri->n) &
			                      isneg((pt1 - ptri->pt[0] ^ ptri->pt[1] - ptri->pt[0]) * ptri->n) &
			                      isneg((pt1 - ptri->pt[1] ^ ptri->pt[2] - ptri->pt[1]) * ptri->n) &
			                      isneg((pt1 - ptri->pt[2] ^ ptri->pt[0] - ptri->pt[2]) * ptri->n) &
...
int cylinder_tri_intersection(const primitives::cylinder* pcyl, const primitives::triangle* ptri, prim_inters* pinters)
{
...
int tri_capsule_intersection(const primitives::triangle* ptri, const primitives::capsule* pcaps, prim_inters* pinters)
{
	Vec3r pc, dp, dp1, vec1, vec0;
	Vec3 pt, axisx, axisy;
	vector2df pt2d;
	...
	mindist.set(1, 0);
	do
	{ // find edge-capsule intersections
		dp = ptri->pt[dec_mod3[i]] - ptri->pt[i];
		pc = ptri->pt[i] - pcaps->center;
	...
	{ // triangle edges do not intersect capsule
		Vec3r ptmin, ptmax;
		// choose the points on capsule caps that have the maximum and the minimum projections on triangle
		// normal
	...
	// if capsule cap's lowest (wrt trinagle normal) point is inside tringle's voronoi region, force triangle normal
	// as global intersection normal
...
int capsule_tri_intersection(const primitives::capsule* pcaps, const primitives::triangle* ptri, prim_inters* pinters)
{
...
int tri_sphere_intersection(const primitives::triangle* ptri, const primitives::sphere* psphere, prim_inters* pinters)
{
	Vec3r pc, dp, dp1, vec1, vec0;
	real r2, a, b, c, d;
	quotient t[3][2], dist, mindist;
	...
	do
	{
		...
		if (d >= 0)
		{
			...
			if (t[i][0] < t[i][1])
			{
				...
				{
					bSwitch =
					    isneg(fabs_tpl((t[i][j].x * 2) - t[i][j].y) - t[i][j].y); // t is in (0..1)
					bEnd = bSwitch & bState; // remember starting (ccw) point of every stripe (it'll
					                         // serve as end point in result)
					(iEndCur &= ~-bEnd) |= (i << 1 | j) & -bEnd;
					// if a point is a stripe start, check if it is the current closest to the
					// suggested stripe start
					dp1 = (ptri->pt[i] - pinters->pt[1]);
					dist.set((dp1 * t[i][j].y + (ptri->pt[dec_mod3[i]] - ptri->pt[i]) * t[i][j].x)
					             .len2(),
					         sqr(t[i][j].y));
					bBest = isneg(dist - mindist) & bSwitch & (bState ^ 1);
					(mindist.x *= 1 - bBest) += dist.x * bBest;
	...
	if (iStartBest < 0)
	{ // triangle edges do not intersect sphere
		if (!bFaceContact)
		{
			return 0;
		}
...
int sphere_tri_intersection(const primitives::sphere* psphere, const primitives::triangle* ptri, prim_inters* pinters)
{
...
int tri_ray_intersection(const primitives::triangle* ptri, const primitives::ray* pray, prim_inters* pinters)
{
...
int tri_plane_intersection(const primitives::triangle* ptri, const primitives::plane* pplane, prim_inters* pinters)
{
	// axes: n2 x n  n1 x n  n = n1 x n2
	Vec3r axes[3];
	...
	real nlen2 = axes[2].len2(), ptw;
	Vec3r ptloc, pt0 = Vec3r(ptri->pt[0] * ptri->n, -pplane->origin * pplane->n, ptri->pt[0] * axes[2]) *
	                   GetMtxFromBasis(axes);
...
int plane_tri_intersection(const primitives::plane* pplane, const primitives::triangle* ptri, prim_inters* pinters)
{
...
int box_box_intersection(const primitives::box* pbox1, const primitives::box* pbox2, prim_inters* pinters)
{
	...
	for (ibox = 0; ibox < 2; ibox++)
	{
		for (iEdge = 0; iEdge < 12; iEdge++)
		{
			...
			pt0 = center[ibox] + axes[0] * (pbox[ibox ^ 1]->size.x * axsg.x) +
			      axes[1] * (pbox[ibox ^ 1]->size.y * axsg.y) + axes[2] * (pbox[ibox ^ 1]->size.z * axsg.z);
			dp = axes[idir];
			...
			if (t[0] < t[1])
			{
				...
				{
					pinters->ptborder[pinters->nborderpt++] =
					    (pt0 + dp * t[0].val()) * pbox[ibox]->Basis + pbox[ibox]->center;
				}
				if (bLocInters[1])
				{
					pinters->ptborder[pinters->nborderpt++] =
					    (pt0 + dp * t[1].val()) * pbox[ibox]->Basis + pbox[ibox]->center;
				}
			}
		}
		TransposeBasis(axes);
...
int box_cylinder_intersection(const primitives::box* pbox, const primitives::cylinder* pcyl, prim_inters* pinters)
{
	...
	for (iz = 0; iz < 3; iz++)
	{
		...
		for (i = 0; i < 4; i++)
		{
			...
			(pt *= t[0].y)[iz] = t[0].x;
			if (isneg(fabs_tpl(t[0].x) - (size[iz] * fabs_tpl(t[0].y))) &
			    isneg((pt - center * t[0].y ^ axis).len2() - sqr(r * t[0].y)))
			{
			...
			if (kd > 0)
			{
				...
				t[0].set(-kb - kd, ka);
				(pt *= t[0].y)[iz] = t[0].x;
				if (isneg(fabs_tpl(t[0].x) - (size[iz] * t[0].y)) &
				    isneg(fabs_tpl((pt - center * t[0].y) * axis) - (hh * t[0].y)))
				{
					pinters->ptborder[pinters->nborderpt++] =
					    (pt / t[0].y) * pbox->Basis + pbox->center;
				}
				t[0].x += kd * 2;
				pt[iz] = t[0].x;
				if (isneg(fabs_tpl(t[0].x) - (size[iz] * t[0].y)) &
	...
	for (iz = 0; iz < 3; iz++)
	{
		if (fabs_tpl(axis[iz]) < (real)0.999)
		{
			...
			for (i = 0; i < 4; i++)
			{
				icap = (i << 1 & 2) - 1;
				isgz = (i & 2) - 1;
				if (sqr(center[iz] + (axis[iz] * (hh * icap)) - (size[iz] * isgz)) * n.len2() <
				    sqr(n[iz] * r))
				{
					n.normalize();
					// inters_point(sg) = ( center.z + n*(size.z*isgz-center.z) +
					// sg*nx*(r^2*nz^2-(size.z*isgz-center.z)^2*n^2)^1/2 )/n.z
					c = center + axis * (icap * hh);
					ry = (size[iz] * isgz) - c[iz];
					rx2 = sqr(r * n[iz]) - (ry * ry);
					nx = n ^ axis;
					// for x,y coordinates check |inters_point(sg).x| < size.x, group into |
					// a+sg*b^1/2 | < c
					...
					bNega = isneg(ka * kb);
					imask = (bNegad & bNega | (bNegad ^ 1) & (bInRange ^ bNega)) << 1 |
					        ((bNegad ^ 1) & (bNega ^ 1) | bNegad & (bInRange ^ bNega ^ 1));
					...
					if (imask)
					{
						...
						{
							pinters->ptborder[pinters->nborderpt++] =
							    (c + (n * ry - nx * rx) * rnz) * pbox->Basis + pbox->center;
						}
						if (imask & 2)
						{
							pinters->ptborder[pinters->nborderpt++] =
	...
	if (t[0] < t[1])
	{
		...
		{
			pinters->ptborder[pinters->nborderpt++] =
			    (center + axis * t[0].val()) * pbox->Basis + pbox->center;
		}
		if (bLocInters[1])
		{
			pinters->ptborder[pinters->nborderpt++] =
			    (center + axis * t[1].val()) * pbox->Basis + pbox->center;
		}
	}
...
int cylinder_box_intersection(const primitives::cylinder* pcyl, const primitives::box* pbox, prim_inters* pinters)
{
...
int box_sphere_intersection(const primitives::box* pbox, const primitives::sphere* psphere, prim_inters* pinters)
{
	int idir;
	Vec3 center, dir;
...
int sphere_box_intersection(const primitives::sphere* psphere, const primitives::box* pbox, prim_inters* pinters)
{
...
int box_capsule_intersection(const primitives::box* pbox, const primitives::capsule* pcaps, prim_inters* pinters)
{
	...
	for (iz = 0; iz < 3; iz++)
	{
		...
		for (i = 0; i < 4; i++)
		{
			...
			{
				kd = sqrt_tpl(kd);
				if ((inrange((-kb - kd) * t[0].y, t[0].x, t[1].x) ^ 1) &
				    isneg(fabs_tpl(-kb - kd) - size[iz]))
				{
					pt[iz] = -kb - kd,
					pinters->ptborder[pinters->nborderpt++] = pt * pbox->Basis + pbox->center;
				}
				if ((inrange((-kb + kd) * t[0].y, t[0].x, t[1].x) ^ 1) &
				    isneg(fabs_tpl(-kb + kd) - size[iz]))
				{
...
int capsule_box_intersection(const primitives::capsule* pcaps, const primitives::box* pbox, prim_inters* pinters)
{
...
int ray_box_intersection(const primitives::ray* pray, const primitives::box* pbox, prim_inters* pinters)
{
...
int box_plane_intersection(const primitives::box* pbox, const primitives::plane* pplane, prim_inters* pinters)
{
	Vec3 dir = pbox->Basis.GetRow(0) * pbox->size.x * sgnnz(pbox->Basis.GetRow(0) * pplane->n) +
...
int plane_box_intersection(const primitives::plane* pplane, const primitives::box* pbox, prim_inters* pinters)
{
...
int root_inrange(float a, float ad, float b, float hca, float d, float h)
{
	int sg = sgnnz(((-b + d) * ad) + hca);
	return isneg((d * sqr_signed(ad) * sg) - sqr_signed((h * a) + ((b * ad - hca) * sg)));
}

int cylinder_cylinder_intersection(const primitives::cylinder* pcyl1, const primitives::cylinder* pcyl2,
                                   prim_inters* pinters)
{
	...
	for (icyl = 0; icyl < 2; icyl++)
	{
		...
		for (i = 0; i < 2;
		     i++, t.x += pcyl[icyl ^ 1]->hh * 2, pt += pcyl[icyl]->axis * (pcyl[icyl ^ 1]->hh * 2))
		{
		...
		if (a > 0.0001f)
		{
			...
			d = (b * b) - (a * c);
			if (d > 0 && (root_inrange(a, 1, b, 0, d, pcyl[icyl]->hh) &&
			                  root_inrange(a, cosa, b, -(center[0] * pcyl[icyl ^ 1]->axis) * a, d,
			                               pcyl[icyl ^ 1]->hh) ||
			              root_inrange(a, 1, b, 0, -d, pcyl[icyl]->hh) &&
			                  root_inrange(a, cosa, b, -(center[0] * pcyl[icyl ^ 1]->axis) * a, -d,
			                               pcyl[icyl ^ 1]->hh)))
			{
				bHasInters = 1;
		...
		for (i = -1; i <= 1; i += 2)
		{
			...
			{
				if (fabs_tpl(c2d.y) < r1 &&
				    inrange(sqr(r1) - sqr(c2d.y), sqr_signed(fabs_tpl(c2d.x) - r0),
				            sqr_signed(fabs_tpl(c2d.x) + r0)) &&
			...
			else
			{
				...
				b = sqr(r0 * r0 * cosa) + (sqr(r1 * r0) * (1 + sqr(cosa))) -
				    (sqr(r0) * (sqr(c2d.x) * sqr(cosa) + sqr(c2d.y)));
				c = sqr(r1 * r0 * r0 * cosa);
				p = b - (a * a * (1.0f / 3));
				q = (a * (b * (1.0f / 6) - a * a * (1.0f / 27))) - (c * 0.5f);
				if ((cube(p) * (1.0f / 27)) + sqr(q) > 0)
				{
					// now check if the closest point on the edge to the icyl^1 axis is inside h
					// range
					...
					if (fabs_tpl(dc * pcyl[icyl ^ 1]->axis +
					             (sina * sgnnz(cosa) * g_sintab[iangle] * sg2d.y * r0 *
					              (icyl * 2 - 1))) < pcyl[icyl ^ 1]->hh)
					{
						bHasInters = 1;
						int icenter, idx1, imiddle, iquad, ipass;
						icenter = ((idx[0] * sg2d.x * sg2d.y) + (SINCOSTABSZ * (1 - sg2d.x))) &
						          ((SINCOSTABSZ * 4) - 1);
							...
							do
							{
								...
								iangle = (SINCOSTABSZ - 1 & -(iquad & 1)) +
								         ((imiddle & SINCOSTABSZ - 1) *
								          (1 - (iquad & 1) * 2));
								pt2d.set(g_costab[iangle] *
								             (1 - ((iquad ^ iquad << 1) & 2)) * kpt2d.x,
								         g_sintab[iangle] * (1 - (iquad & 2)) *
								             kpt2d.y);
								idx[isneg(sqr(r1) - len2(pt2d - c2d))] = imiddle;
							}
							while (fabs_tpl(idx[0] - idx[1]) > 2);
							pt2d.y =
							    sgnnz(cosa) * g_sintab[iangle] * (1 - (iquad & 2)) * r0;
							if (idx[1] != idx1 &&
							    fabs_tpl(dc * pcyl[icyl ^ 1]->axis +
							             (sina * pt2d.y * (icyl * 2 - 1))) <
							        pcyl[icyl ^ 1]->hh)
							{
								pinters->ptborder[pinters->nborderpt++] =
								    dc + axisx * pt2d.x +
								    (pcyl[icyl]->axis ^ axisx) * pt2d.y +
								    pcyl[icyl ^ 1]->center;
							}
						}
					}
	...
	if (sina > 0.003f)
	{
		...
		for (i = -1; i <= 1; i += 2)
		{
			for (j = -1; j <= 1; j += 2)
			{
				...
				if (min(span2, d) > 0)
				{
					if (inrange(b * b, d, span2) |
					    isneg((sqr_signed(k) * sg) - (span2 * sqr(b) * 4)))
					{
						d = sqrt_tpl(d);
						for (sg = -1; sg <= 1; sg += 2)
						{
							...
							{
								pinters->ptborder[pinters->nborderpt++] =
								    center[0] + l * (-b + (d * sg)) + center[1] +
								    pcyl[1]->center;
							}
						}
					}
					if (sqr(2 * center[0] * l) * span2 >
					    max(0.0f, center[0].len2() + span2 - sqr(pcyl[1]->r)))
					{
...
int cylinder_sphere_intersection(const primitives::cylinder* pcyl, const primitives::sphere* psphere,
                                 prim_inters* pinters)
{
	Vec3 dir, dc;
	float dh, h;
...
int sphere_cylinder_intersection(const primitives::sphere* psphere, const primitives::cylinder* pcyl,
                                 prim_inters* pinters)
{
...
int cylinder_capsule_intersection(const primitives::cylinder* pcyl, const primitives::capsule* pcaps,
                                  prim_inters* pinters)
{
	...
	for (i = -1; i <= 1; i += 2)
	{
		...
		else
		{
			...
			if ((cube(p) * (1.0f / 27)) + sqr(q) > 0)
			{
				...
				if (fabs_tpl(dc * pcaps->axis - (sina * sgnnz(cosa) * g_sintab[iangle] * sg2d.y * r0)) <
				    pcaps->hh)
				{
					bHasInters = 1;
					int icenter, idx1, imiddle, iquad, ipass;
					...
					for (ipass = -1; ipass <= 1; ipass += 2)
					{
						...
						do
						{
							...
							pt2d.set(g_costab[iangle] * (1 - ((iquad ^ iquad << 1) & 2)) *
							             kpt2d.x,
							         g_sintab[iangle] * (1 - (iquad & 2)) * kpt2d.y);
							idx[isneg(sqr(r1) - len2(pt2d - c2d))] = imiddle;
						}
						while (fabs_tpl(idx[0] - idx[1]) > 2);
						pt2d.y = sgnnz(cosa) * g_sintab[iangle] * (1 - (iquad & 2)) * r0;
						if (idx[1] != idx1 &&
						    fabs_tpl(dc * pcaps->axis - (sina * pt2d.y)) < pcaps->hh)
						{
							pinters->ptborder[pinters->nborderpt++] =
							    dc + axisx * pt2d.x + (pcyl->axis ^ axisx) * pt2d.y +
							    pcaps->center;
						}
					}
				}
...
int capsule_cylinder_intersection(const primitives::capsule* pcaps, const primitives::cylinder* pcyl,
                                  prim_inters* pinters)
{
...
int cylinder_ray_intersection(const primitives::cylinder* pcyl, const primitives::ray* pray, prim_inters* pinters)
{
	...
	t.set(pcyl->axis * ((pcyl->center - pray->origin) * sg - pcyl->axis * pcyl->hh),
	      fabs_tpl(pray->dir * pcyl->axis));
	bHit = bHit0 =
	    isneg(fabs_tpl((t.x * 2) - t.y) - t.y) &
	    isneg(((pray->origin - pcyl->center) * t.y + pray->dir * t.x ^ pcyl->axis).len2() - sqr(pcyl->r * t.y));
	t.set(pcyl->axis * ((pcyl->center - pray->origin) * sg - pcyl->axis * (pcyl->hh * (bHit * 2 - 1))),
	      fabs_tpl(pray->dir * pcyl->axis));
	...
	if (!bHit0)
	{
		...
		if (d >= 0)
		{
			...
			bHitSide = bHit =
			    isneg(fabs_tpl((t.x * 2) - t.y) - t.y) &
			    isneg(fabs_tpl(((pray->origin - pcyl->center) * t.y + pray->dir * t.x) * pcyl->axis) -
			          (pcyl->hh * t.y));
			t.x += d * (bHit ^ 1) * 2;
			bHitSide = bHit =
			    isneg(fabs_tpl((t.x * 2) - t.y) - t.y) &
...
int ray_cylinder_intersection(const primitives::ray* pray, const primitives::cylinder* pcyl, prim_inters* pinters)
{
...
int cylinder_plane_intersection(const primitives::cylinder* pcyl, const primitives::plane* pplane, prim_inters* pinters)
{
	...
	pinters->pt[0] = pinters->pt[1] =
	    pcyl->center - pcyl->axis * pcyl->hh * sgnnz(pcyl->axis * pplane->n) + dirdown;
	pinters->n = -pplane->n;
	pinters->iFeature[0][0] = pinters->iFeature[1][0] = 0x40;
	pinters->iFeature[0][1] = pinters->iFeature[1][1] = 0x40;
	return isneg((pinters->pt[0] - pplane->origin) * pplane->n);
...
int plane_cylinder_intersection(const primitives::plane* pplane, const primitives::cylinder* pcyl, prim_inters* pinters)
{
...
int sphere_sphere_intersection(const primitives::sphere* psphere1, const primitives::sphere* psphere2,
                               prim_inters* pinters)
{
	...
	dc.normalize();
	pinters->pt[0] = pinters->pt[1] =
	    (dc * (psphere1->r - psphere2->r) + psphere1->center + psphere2->center) * (real)0.5;
...
int sphere_ray_intersection(const primitives::sphere* psphere, const primitives::ray* pray, prim_inters* pinters)
{
	int bHit;
	real a, b, c, d;
	quotient t;
...
int ray_sphere_intersection(const primitives::ray* pray, const primitives::sphere* psphere, prim_inters* pinters)
{
...
int sphere_plane_intersection(const primitives::sphere* psphere, const primitives::plane* pplane, prim_inters* pinters)
{
...
int plane_sphere_intersection(const primitives::plane* pplane, const primitives::sphere* psphere, prim_inters* pinters)
{
...
int ray_plane_intersection(const primitives::ray* pray, const primitives::plane* pplane, prim_inters* pinters)
{
...
int plane_ray_intersection(const primitives::plane* pplane, const primitives::ray* pray, prim_inters* pinters)
{
...
int capsule_capsule_intersection(const primitives::capsule* pcaps1, const primitives::capsule* pcaps2,
                                 prim_inters* pinters)
{
	...
	for (icaps = 0; icaps < 2; icaps++)
	{
		for (i = -1; i <= 1; i += 2)
		{
			dir = pcaps[icaps ^ 1]->center - pcaps[icaps]->center -
			      pcaps[icaps]->axis * (pcaps[icaps]->hh * i);
			dir -= pcaps[icaps ^ 1]->axis *
			       max(-pcaps[icaps ^ 1]->hh, min(pcaps[icaps ^ 1]->hh, dir * pcaps[icaps ^ 1]->axis));
			if (dir.len2() < sqr(pcaps[0]->r + pcaps[1]->r))
			{
				pinters->ptborder[pinters->nborderpt++] = pcaps[icaps]->center +
				                                          pcaps[icaps]->axis * (pcaps[icaps]->hh * i) +
				                                          dir.normalize() * pcaps[icaps]->r;
...
int capsule_sphere_intersection(const primitives::cylinder* pcaps, const primitives::sphere* psphere,
                                prim_inters* pinters)
{
	Vec3 dir;
...
int sphere_capsule_intersection(const primitives::sphere* psphere, const primitives::capsule* pcaps,
                                prim_inters* pinters)
{
...
int capsule_ray_intersection(const primitives::capsule* pcaps, const primitives::ray* pray, prim_inters* pinters)
{
	...
	for (icap = 0; icap < 2; icap++)
	{
		...
		if ((d = (b * b) - (a * c)) >= 0)
		{
			...
			bHit = inrange(t.x * tmin.y, (real)0, t.y * tmin.x) &
			       isneg((pcaps->hh * t.y) - fabs_tpl((axcdiff * t.y) + (axdir * t.x)));
	...
	if (d >= 0)
	{
		...
		t.set(-b - d, a);
		bHit = inrange(t.x * tmin.y, (real)0, t.y * tmin.x) &
		       isneg(fabs_tpl(((pray->origin - pcaps->center) * t.y + pray->dir * t.x) * pcaps->axis) -
		             (pcaps->hh * t.y));
		tmin.x += (t.x - tmin.x) * bHit;
		tmin.y += (t.y - tmin.y) * bHit;
...
int ray_capsule_intersection(const primitives::ray* pray, const primitives::capsule* pcaps, prim_inters* pinters)
{
...
int CIntersectionChecker::Check(int type1, int type2, const primitives::primitive* prim1,
                                const primitives::primitive* prim2, prim_inters* pinters)
{
	return table[type1][type2](prim1, prim2, pinters);
}

int CIntersectionChecker::CheckExists(int type1, int type2)
{
