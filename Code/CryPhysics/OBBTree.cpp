...
float COBBTree::BuildNode(int iNode, int iTriStart, int nTris, int nDepth)
{
	...
	for (iAxis = 0; iAxis < 3; iAxis++)
	{
		...
		for (i = iTriStart; i < iTriStart + nTris; i++)
		{
			center = m_pMesh->m_pVertices[m_pMesh->m_pIndices[i * 3]] +
			...
			numtris[1] += iPart;
			iPart = isnonneg(
			    (((center * axis) * (1.0f / 3)) - cx)); // mode2: sort triangles basing on centroids only
		...
		for (i = 0; i < 3; i++)
		{
			diff[i] = bounds[i][1] - bounds[i][0] -
			          (sz * (isneg(numtris[i] - m_nMinTrisPerNode) |
			                 isneg(nTris - numtris[i] - m_nMinTrisPerNode)));
		}
		iMode[iAxis] = idxmax3(diff);
		nTrisAx[iAxis] = numtris[iMode[iAxis]];
		axdiff[iAxis] =
		    diff[iMode[iAxis]] * m_pNodes[iNode].size[dec_mod3[iAxis]] * m_pNodes[iNode].size[inc_mod3[iAxis]];
	}
...
void COBBTree::GetBBox(primitives::box* pbox)
{
...
void COBBTree::GetNodeBV(const Matrix33& Rw, const Vec3& offsw, float scalew, BV*& pBV, const Vec3& sweepdir,
...
void COBBTree::GetNodeChildrenBVs(const BV* pBV_parent, const Vec3& sweepdir, float sweepstep, BV*& pBV_child1,
...
int COBBTree::GetNodeContents(int iNode, BV* pBVCollider, int bColliderUsed, int bColliderLocal,
...
int COBBTree::GetNodeContentsIdx(int iNode, int& iStartPrim)
{
...
void COBBTree::MarkUsedTriangle(int itri, geometry_under_test* pGTest)
{
	    ...
	    max(max(m_pNodes[iNode].size.x, m_pNodes[iNode].size.y), m_pNodes[iNode].size.z) < m_maxSkipDim)
	{
		do
		{
			...
			pGTest->pUsedNodesIdx[pGTest->nUsedNodes =
			                          min(pGTest->nUsedNodes + 1, pGTest->nMaxUsedNodes - 1)] = iNode;
...
float COBBTree::GetMaxSkipDim()
{
	return m_maxSkipDim / max(max(m_pNodes[0].size.x, m_pNodes[0].size.y), m_pNodes[0].size.z);
}

int COBBTree::PrepareForIntersectionTest(geometry_under_test* pGTest, CGeometry* pCollider,
