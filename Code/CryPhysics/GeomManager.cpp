...
constexpr int GEOM_CHUNK_SZ = 64;
constexpr int PHYS_GEOM_VER = 1;
...
IGeometry* CGeomManager::CreateMesh(strided_pointer<const Vec3> pVertices, strided_pointer<unsigned short> pIndices,
                                    char* pIds, int* pForeignIdx, int nTris, int flags, float approx_tolerance,
                                    SMeshBVParams* pParams)
{
	Vec3r axes[3], center;
	primitives::primitive* pprim = nullptr;
	int i, itype = primitives::triangle::type;
	...
	if ((i == nTris || approx_tolerance > 0.5f) && approx_tolerance > 0)
	{ // only try approximation if the mesh has one material
		ComputeMeshEigenBasis(pVertices, pIndices, nTris, axes, center);
		itype =
		    ChoosePrimitiveForMesh(pVertices, pIndices, nTris, axes, center, flags, approx_tolerance, pprim);
	}

	if (itype == primitives::triangle::type)
	{
		if (flags & mesh_VoxelGrid)
		{
			CVoxelGeom* pVox = new CVoxelGeom;
			SVoxGridParams* params = (SVoxGridParams*)pParams;
			primitives::grid3d grd;
		...
		else
		{
			...
			return (new CTriMesh)
			    ->CreateTriMesh(pVertices, pIndices, pIds, pForeignIdx, nTris, flags,
...
IGeometry* CGeomManager::CreatePrimitive(int type, const primitives::primitive* pprim)
{
	...
	switch (type)
	{
		case primitives::cylinder::type:
			pRes = (new CCylinderGeom)->CreateCylinder((primitives::cylinder*)pprim);
			break;
		case primitives::capsule::type:
			pRes = (new CCapsuleGeom)->CreateCapsule((primitives::capsule*)pprim);
			break;
		case primitives::sphere::type:
			pRes = (new CSphereGeom)->CreateSphere((primitives::sphere*)pprim);
			break;
		case primitives::box::type:
			pRes = (new CBoxGeom)->CreateBox((primitives::box*)pprim);
			break;
		case primitives::heightfield::type:
			pRes = (new CHeightfield)->CreateHeightfield((primitives::heightfield*)pprim);
			break;
		case primitives::ray::type:
			pRes = new CRayGeom((primitives::ray*)pprim);
			break;
	}
                                      ...
                                      strided_pointer<unsigned short> pIndices, char* pIds)
{
	...
	switch (itype)
	{
		...
		case GEOM_BOX:
			...
			break;
		default:
			CryLogErrorAlways("CGeomManager::LoadGeometry: Unrecognized geometry type %d", itype);
			return nullptr;
	}
...
IGeometry* CGeomManager::GetCrackGeom(const Vec3* pt, int idmat, geom_world_data* pgwd)
{
	...
	for (i = 0, i0 = -1, mindiff = 1E10; i < m_nCracks; i++)
	{
		if (m_pCracks[i].idmat == idmat)
		{
			...
			diff = (m_kCrackSkew * len2(m_pCracks[i].pt3 * scale - pt3) * sqr(rmaxedge)) +
			       (m_kCrackScale * sqr(scale));
			if (diff < mindiff)
			{
				mindiff = diff;
				i0 = i;
...
IBreakableGrid2d* CGeomManager::GenerateBreakableGrid(vector2df* ptsrc, int npt, const vector2di& nCells,
                                                      int bStaticBorder, int seed)
{
