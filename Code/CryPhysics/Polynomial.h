...
public:
	...
	polynomial_tpl& operator=(const polynomial_tpl<ftype, degree1>& src)
	{
		int i;
		denom = src.denom;
...
constexpr ftype polye();

template<>
constexpr float polye()
{
	return 1E-6f;
}

template<>
constexpr double polye()
{
	return 1E-10;
}

template<class ftype, int degree>
polynomial_tpl<ftype, degree> operator+(const polynomial_tpl<ftype, degree>& pn, ftype op)
{
...
template<class ftype, int degree>
polynomial_tpl<ftype, degree> operator+(ftype op, const polynomial_tpl<ftype, degree>& pn)
{
	polynomial_tpl<ftype, degree> res = pn;
	res.data[0] += op * res.denom;
	return res;
...
template<class ftype, int degree>
polynomial_tpl<ftype, degree * 2> psqr(const polynomial_tpl<ftype, degree>& op)
{
	return op * op;
}

constexpr int degmax(int degree1, int degree2)
{
	return degree1 - (degree1 - degree2 & (degree1 - degree2) >> 31);
}

template<class ftype, int degree1, int degree2>
polynomial_tpl<ftype, degmax(degree1, degree2)> operator+(const polynomial_tpl<ftype, degree1>& op1,
                                                          const polynomial_tpl<ftype, degree2>& op2)
{
...
template<class ftype, int degree1, int degree2>
polynomial_tpl<ftype, degree1>& operator+=(polynomial_tpl<ftype, degree1>& op1,
                                           const polynomial_tpl<ftype, degree2>& op2)
{
...
template<class ftype, int degree1, int degree2>
polynomial_tpl<ftype, degree1 + degree2> operator*(const polynomial_tpl<ftype, degree1>& op1,
                                                   const polynomial_tpl<ftype, degree2>& op2)
{
	...
	switch (degree1)
	{
		case 8: for (j = 0; j <= degree2; j++) { res.data[8 + j] += op1.data[8] * op2.data[j]; } [[fallthrough]];
		case 7: for (j = 0; j <= degree2; j++) { res.data[7 + j] += op1.data[7] * op2.data[j]; } [[fallthrough]];
		case 6: for (j = 0; j <= degree2; j++) { res.data[6 + j] += op1.data[6] * op2.data[j]; } [[fallthrough]];
		case 5: for (j = 0; j <= degree2; j++) { res.data[5 + j] += op1.data[5] * op2.data[j]; } [[fallthrough]];
		case 4: for (j = 0; j <= degree2; j++) { res.data[4 + j] += op1.data[4] * op2.data[j]; } [[fallthrough]];
		case 3: for (j = 0; j <= degree2; j++) { res.data[3 + j] += op1.data[3] * op2.data[j]; } [[fallthrough]];
		case 2: for (j = 0; j <= degree2; j++) { res.data[2 + j] += op1.data[2] * op2.data[j]; } [[fallthrough]];
		case 1: for (j = 0; j <= degree2; j++) { res.data[1 + j] += op1.data[1] * op2.data[j]; } [[fallthrough]];
...
template<class ftype>
void polynomial_divide(const polynomial_tpl<ftype, 8>& num, const polynomial_tpl<ftype, 8>& den,
                       polynomial_tpl<ftype, 8>& quot, polynomial_tpl<ftype, 8>& rem, int degree1, int degree2)
{
	int i, j, k, l;
	ftype maxel;
	for (i = 0; i <= degree1; i++)
...
template<class ftype, int degree1, int degree2>
polynomial_tpl<ftype, degree1 - degree2> operator/(const polynomial_tpl<ftype, degree1>& num,
                                                   const polynomial_tpl<ftype, degree2>& den)
{
	polynomial_tpl<ftype, degree1 - degree2> quot;
	polynomial_tpl<ftype, degree1> rem;
	polynomial_divide((polynomial_tpl<ftype, 8>&)num, (polynomial_tpl<ftype, 8>&)den,
	                  (polynomial_tpl<ftype, 8>&)quot, (polynomial_tpl<ftype, 8>&)rem, degree1, degree2);
	return quot;
}

template<class ftype, int degree1, int degree2>
polynomial_tpl<ftype, degree2 - 1> operator%(const polynomial_tpl<ftype, degree1>& num,
                                             const polynomial_tpl<ftype, degree2>& den)
{
	polynomial_tpl<ftype, degree1 - degree2> quot;
	polynomial_tpl<ftype, degree1> rem;
...
int polynomial_tpl<ftype, degree>::nroots(ftype start, ftype end) const
{
	...
	polynomial_divide((polynomial_tpl<ftype, 8>&)*this, (polynomial_tpl<ftype, 8>&)f[0],
	                  (polynomial_tpl<ftype, 8>&)f[degree], (polynomial_tpl<ftype, 8>&)f[1], degree, degree - 1);
	f[1].denom = -f[1].denom;
	for (i = 2; i < degree; i++)
	{
		polynomial_divide((polynomial_tpl<ftype, 8>&)f[i - 2], (polynomial_tpl<ftype, 8>&)f[i - 1],
		                  (polynomial_tpl<ftype, 8>&)f[degree], (polynomial_tpl<ftype, 8>&)f[i], degree + 1 - i,
		                  degree - i);
		f[i].denom = -f[i].denom;
...
int polynomial_tpl<ftype, maxdegree>::findroots(ftype start, ftype end, ftype* proots, int nIters, int degree) const
{
	...
	{
		double t, a, b, c, a3, p, q, Q, Qr, Ar, Ai, phi;
	...
	else if (degree > 4)
	{
		...
		for (nExtremes = deriv.findroots(start, end, roots + 1, nIters, degree - 1) + 1;
		     nExtremes > 1 && roots[nExtremes - 1] > end; nExtremes--)
			;
		...
		for (prevroot = start, prevval[0] = eval(start, degree), nRoots = 0; i < nExtremes;
		     prevval[0] = val, prevroot = roots[i++])
		{
...
using P3 = polynomial_tpl<double, 3>;
using P2 = polynomial_tpl<double, 2>;
using P1 = polynomial_tpl<double, 1>;
using P3f = polynomial_tpl<float, 3>;
using P2f = polynomial_tpl<float, 2>;
using P1f = polynomial_tpl<float, 1>;
