...
void CRayGeom::GetBBox(primitives::box* pbox)
{
...
int CRayGeom::PrepareForIntersectionTest(geometry_under_test* pGTest, CGeometry* pCollider,
                                         geometry_under_test* pGTestColl, bool bKeepPrevContacts)
{
	static char g_id = (char)-1;
	m_Tree.PrepareForIntersectionTest(pGTest, pCollider, pGTestColl);
	pGTest->pGeometry = this;
	pGTest->pBVtree = &m_Tree;
	pGTest->primbuf = pGTest->primbuf1 = g_idata[pGTest->iCaller].RayBuf;
	pGTest->typeprim = primitives::ray::type;
	pGTest->szprim = sizeof(primitives::ray);
	pGTest->idbuf = &g_id;
	pGTest->surfaces = 0;
	pGTest->edges = 0;
	pGTest->minAreaEdge = 0;
	PrepareRay(g_idata[pGTest->iCaller].RayBuf, pGTest);
	return 1;
}

int CRayGeom::PreparePrimitive(geom_world_data* pgwd, primitives::primitive*& pprim, int iCaller)
{
	g_idata[iCaller].RayBuf[0].origin = m_ray.origin + pgwd->offset;
	g_idata[iCaller].RayBuf[0].dir = pgwd->R * m_ray.dir * pgwd->scale;
	pprim = g_idata[iCaller].RayBuf;
	return primitives::ray::type;
}

void CRayGeom::PrepareRay(primitives::ray* pray, geometry_under_test* pGTest)
{
	pray->origin = pGTest->R * m_ray.origin * pGTest->scale + pGTest->offset;
	pray->dir = pGTest->R * m_ray.dir * pGTest->scale;
}

int CRayGeom::GetUnprojectionCandidates(int iop, const contact* pcontact, primitives::primitive*& pprim,
                                        int*& piFeature, geometry_under_test* pGTest)
{
	if (pGTest->bTransformUpdated)
	{
		PrepareRay((primitives::ray*)pGTest->primbuf1, pGTest);
	}
...
int CRayGeom::GetPrimitiveList(int iStart, int nPrims, int typeCollider, primitives::primitive* pCollider,
                               int bColliderLocal, geometry_under_test* pGTest, geometry_under_test* pGTestOp,
                               primitives::primitive* pRes, char* pResId)
{
	PrepareRay((primitives::ray*)pRes, pGTest);
	pGTest->bTransformUpdated = 0;
	*pResId = (char)-1;
	return 1;
}

int CRayGeom::RegisterIntersection(primitives::primitive* pprim1, primitives::primitive* pprim2,
                                   geometry_under_test* pGTest1, geometry_under_test* pGTest2, prim_inters* pinters)
{
	...
	pres->iPrim[1] = pGTest2 && pGTest2->typeprim == primitives::indexed_triangle::type
	                     ? ((primitives::indexed_triangle*)pprim2)->idx
	                     : 0;
