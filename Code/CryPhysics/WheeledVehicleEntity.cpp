...
int CWheeledVehicleEntity::SetParams(pe_params* _params, int bThreadSafe)
{
	...
	if (res)
	{
		...
		else if (_params->type == pe_params_part::type_id)
		{
			...
			if (i >= m_nHullParts && (!is_unused(params->pPhysGeom) || !is_unused(params->pPhysGeomProxy)))
			{
				...
				m_susp[i - m_nHullParts].ptc0 =
				    m_parts[i].q * m_parts[i].pPhysGeomProxy->origin + m_parts[i].pos;
			}
		}
		return res;
	}
	...
	if (_params->type == pe_params_wheel::type_id)
	{
		...
		{
			m_susp[iWheel].kDamping =
			    params->kDamping > 0
				? params->kDamping
				: -params->kDamping * sqrt_tpl(4 * m_susp[iWheel].kStiffness * m_susp[iWheel].Mpt);
		}
...
int CWheeledVehicleEntity::GetParams(pe_params* _params)
{
	if (_params->type == pe_params_car::type_id)
	{
		...
		params->engineMaxRPM = physics_float2int(m_engineMaxw * (30.0 / g_PI));
...
int CWheeledVehicleEntity::Action(pe_action* _action, int bThreadSafe)
{
	...
	if (_action->type == pe_action_drive::type_id)
	{
		...
		if (!is_unused(action->steer) || !is_unused(action->dsteer))
		{
			...
			{
				if (m_steer != 0)
				{ // apply Ackerman steering to front wheels
					WriteLock lock(m_lockVehicle);
					Vec3 pt[2], rarax, cm = (m_body.pos - m_pos) * m_qrot;
					float ha, la, hb, lb, tgsteer = tan_tpl(m_steer);
					pt[0] = pt[1] = cm;
					for (i = 0; i < m_nParts - m_nHullParts; i++)
					{
						if (m_susp[i].iAxle >= 0 &&
						    m_susp[i].pt.y < pt[j = isneg(cm.x - m_susp[i].pt.x)].y)
						{
							pt[j] = m_susp[i].pt;
						}
					}
					rarax = (pt[1] - pt[0])
					            .normalized(); // rear wheels axis (pt[0,1] - rear wheels positions
					for (i = 0; i < m_nParts - m_nHullParts; i++)
					{
						if (m_susp[i].iAxle >= 0 && m_susp[i].pt.y > cm.y &&
						    (m_susp[i].pt.x - cm.x) * m_steer > 0)
						{
							...
							{
								if (fabsf(m_susp[j].pt.y - m_susp[i].pt.y) <
								        ha * 0.05f &&
								    (m_susp[j].pt.x - cm.x) * m_steer < 0)
								{
									hb = (m_susp[j].pt - pt[0] ^ rarax).len();
									lb = (m_susp[j].pt - pt[0]) * rarax;
									m_susp[j].steer =
									    atan_tpl(hb * tgsteer /
									             (ha + fabs((lb - la) * tgsteer)));
								}
							}
						}
					}
...

				{
					...
					for (i = 0; i < m_nParts - m_nHullParts; i++)
					{
						m_susp[i].q = m_parts[i + m_nHullParts].q =
						    Quat::CreateRotationAA(m_susp[i].steer, Vec3(0, 0, -1)) *
						    m_susp[i].q0;
						// GetRotationAA(m_susp[i].steer,Vec3(0,0,-1))*GetRotationAA(m_susp[i].rot,Vec3(-1,0,0))*m_susp[i].q0;
						Vec3 ptc1 = m_parts[i + m_nHullParts].q *
						                m_parts[i + m_nHullParts].pPhysGeomProxy->origin +
						            m_susp[i].pos0;
						(m_parts[i + m_nHullParts].pos = m_susp[i].pos0 + m_susp[i].ptc0 - ptc1)
						    .z -= m_susp[i].curlen - m_susp[i].len0;
...
int CWheeledVehicleEntity::GetStatus(pe_status* _status)
{
	...
	if (_status->type == pe_status_pos::type_id)
	{
		...
		if ((unsigned int)iwheel < (unsigned int)(m_nParts - m_nHullParts))
		{
			...
			prevq = m_parts[i].q;
			m_parts[i].q = Quat::CreateRotationAA(m_susp[iwheel].steer, Vec3(0, 0, -1)) *
			               Quat::CreateRotationAA(m_susp[iwheel].rot, Vec3(-1, 0, 0)) * m_susp[iwheel].q0;
			ptc1 = m_parts[i].q * m_parts[i].pPhysGeomProxy->origin + m_susp[iwheel].pos0;
			(m_parts[i].pos = m_susp[iwheel].pos0 + m_susp[iwheel].ptc0 - ptc1).z -=
			    m_susp[iwheel].curlen - m_susp[iwheel].len0;
	...
	if (_status->type == pe_status_wheel::type_id)
	{
		...
		{
			for (status->iWheel = 0; status->iWheel < m_nParts - m_nHullParts &&
			                         m_parts[status->iWheel + m_nHullParts].id != status->partid;
			     status->iWheel++)
				;
		...
		if (status->bContact)
		{
			...
			Vec3 axis(cos_tpl(m_susp[status->iWheel].steer), -sin_tpl(m_susp[status->iWheel].steer), 0),
			    pulldir, ncontact;
	...
	if (_status->type == pe_status_vehicle_abilities::type_id)
	{
		...
		if (nd > 0)
		{
			...
			do
			{
				...
				wlim[isneg(((sin_tpl(w * k) * m_enginePower - m_axleFriction * w) * nd) -
				           (sqr(w * m_susp[0].r) * m_dampingVehicle * m_body.M))] = w;
			}
...
void CWheeledVehicleEntity::RecalcSuspStiffness()
{
	...
	for (i = 0; i < m_nParts - m_nHullParts; i++)
	{
		for (j = i + 1; j < m_nParts - m_nHullParts; j++)
		{
			...
			{
				if (fabs_tpl(m_susp[i].pt.y - m_susp[j].pt.y) < e &&
				    fabs_tpl(m_susp[i].pt.x + m_susp[j].pt.x) < e)
				{
	...
	for (i = 0; i < m_nParts - m_nHullParts; i++)
	{
		if (m_susp[i].fullen > 0)
		{
			...
			{
				m_susp[i].kDamping =
				    -m_susp[i].kDamping0 * sqrt_tpl(4 * m_susp[i].kStiffness * m_susp[i].Mpt);
			}
		}
		else
		{
...
int CWheeledVehicleEntity::AddGeometry(phys_geometry* pgeom, pe_geomparams* _params, int id, int bThreadSafe)
{
	...
	if (!is_unused(params->bDriving))
	{
		if (density <= 0)
		{
			CryLogError("CWheeledVehicleEntity::AddGeometry: trying to add a weightless wheel");
			return -1;
		}
	...
	if (params->bDriving < 0 || m_nParts - m_nHullParts > NMAXWHEELS)
	{
		if (m_nParts > nPartsOld)
		{
			if (m_nHullParts < m_nParts - 1)
			{
				...
				memmove(m_parts + m_nHullParts + 1, m_parts + m_nHullParts,
				        sizeof(geom) * (m_nParts - m_nHullParts - 1));
	...
	else
	{
		...
		m_susp[idx].steer = 0;
		m_susp[idx].T = m_susp[idx].w = m_susp[idx].wa = m_susp[idx].rot = m_susp[idx].prevTdt =
		    m_susp[idx].prevw = 0;
		...
		for (i = 0; i < 3; i++)
		{
			if ((diff = min(fabs_tpl(bbox.size[i] - bbox.size[inc_mod3[i]]),
			                fabs_tpl(bbox.size[i] - bbox.size[dec_mod3[i]]))) > maxdiff)
			{
				maxdiff = diff;
...
void CWheeledVehicleEntity::RemoveGeometry(int id, int bThreadSafe)
{
	...
	if (i < m_nParts)
	{
		...
		{
			memmove(m_susp + i - m_nHullParts, m_susp + i - m_nHullParts + 1,
			        sizeof(m_susp[0]) * (m_nParts - i - 1));
			for (; i < m_nParts - 1; i++)
			{
...
void CWheeledVehicleEntity::CheckAdditionalGeometry(float time_interval)
{
	...
	Vec3 r, ptc1, geompos, axis, axisz, ptcw, dirmax, unproj, n_accum; // axisx=m_qrot*Vec3(1,0,0);
	...
	nents =
	    m_pWorld->GetEntitiesAround(m_BBoxNew[0], m_BBoxNew[1], pentlist,
	                                ent_terrain | ent_static | ent_sleeping_rigid | ent_rigid | ent_triggers, this);
	for (i = j = 0; i < nents; i++)
	{
		if (pentlist[i] != this)
		{
			...
			else if (m_bAwake)
			{
				...
				for (j1 = 0; j1 < pentlist[i]->m_nParts; j1++)
				{
					...
					ptcw = (pentlist[i]->m_pos +
					        pentlist[i]->m_qrot * (pentlist[i]->m_parts[j1].pos +
					                               pentlist[i]->m_parts[j1].q * abox.center *
					                                   pentlist[i]->m_parts[j1].scale) -
					        m_posNew) *
					       m_qNew;
				...
				{
					(ptcw = m_susp[iwheel].ptc0).z +=
					    (m_susp[iwheel].fullen * 0.5f) - m_susp[iwheel].len0;
					if (m_susp[iwheel].steer)
					{
						axis.x = fabs_tpl(cos_tpl(m_susp[iwheel].steer)),
						axis.y = fabs_tpl(sin_tpl(m_susp[iwheel].steer));
					}
					else
					{
						axis.x = 1, axis.y = 0;
	...
	// ip.bNoBorder = true;
	ip.vrel_min = m_susp[0].r / ip.time_interval * 0.2f;
	...
	if (bFakeInnerWheels)
	{
		for (iwheel = 0; iwheel < m_nParts - m_nHullParts; iwheel++)
		{
			if (m_susp[iwheel].fullen > 0)
			{
				...
				iwMiny[j] +=
				    (iwheel - iwMiny[j]) & -isneg(m_susp[iwheel].pt.y - m_susp[iwMiny[j]].pt.y);
				iwMaxy[j] +=
				    (iwheel - iwMaxy[j]) & -isneg(m_susp[iwMaxy[j]].pt.y - m_susp[iwheel].pt.y);
			}
		}
	}
	...
	for (i = m_nHullParts; i < m_nParts; i++)
	{
		...
		axis = (m_qNew * qsteer) *
		       Vec3(sgnnz(m_susp[iwheel].pt.x), 0, 0); // outward wheel rotation axis in world cs
		...
		(m_susp[iwheel].pos = m_susp[iwheel].pos0 + m_susp[iwheel].ptc0 - ptc1).z -=
		    max(0.0f, m_susp[iwheel].fullen - m_susp[iwheel].len0);
		geompos = m_posNew + m_qNew * m_susp[iwheel].pos;

		Matrix33 Rabs = Matrix33(m_qNew * qsteer);
		sz = Rabs.Fabs() *
		     Vec3(m_susp[iwheel].width, m_susp[iwheel].r, m_susp[iwheel].r + m_susp[iwheel].fullen);
		...
		j = isneg(m_susp[iwheel].pt.x);
		if (bFakeInnerWheels &&
		    (m_susp[iwheel].fullen * (iszero(iwheel - iwMiny[j]) | iszero(iwheel - iwMaxy[j]))) == 0.0f)
		{
			continue;
		}
		...
		if (ip.bSweepTest)
		{
			...
			{
				aray.m_ray.origin = ptcw - axisz * (m_susp[iwheel].len0 + m_susp[iwheel].fullen) +
				                    axis * m_susp[iwheel].width;
		...
		for (ient = 0; ient < nents; ient++)
		{
			for (j = 0; j < pentlist[ient]->m_nParts; j++)
			{
				if (pentlist[ient]->m_parts[j].flags & m_susp[iwheel].flagsCollider0 &&
				    ((pentlist[ient]->m_parts[j].BBox[1] - pentlist[ient]->m_parts[j].BBox[0]).len2() ==
				         0 ||
				     AABB_overlap(pentlist[ient]->m_parts[j].BBox, BBoxWheel)))
				{
					gwd[1].offset = pentlist[ient]->m_pos +
					...
					if (!bRayCast)
					{
						ncontacts = m_parts[i].pPhysGeomProxy->pGeom->Intersect(
						    pentlist[ient]->m_parts[j].pPhysGeomProxy->pGeom, gwd + 0, gwd + 1,
						    &ip, pcontacts);
					}
					else
					{
						ip.bSweepTest = false;
						ncontacts = pentlist[ient]->m_parts[j].pPhysGeomProxy->pGeom->Intersect(
						    &aray, gwd + 1, 0, &ip, pcontacts);
					...
					lockColl.SetActive(isneg(-ncontacts));
					...
					for (icont = 0; icont < ncontacts; icont++)
					{
						...
						if (!ip.bSweepTest)
						{
							...
							{
								if (!bHasContacts &&
								    sqr(pcontacts[icont].t) > unproj.len2())
								{
									unproj =
									    pcontacts[icont].dir * pcontacts[icont].t;
								}
							}
						...
						tcur = ip.bSweepTest
						           ? ip.time_interval - pcontacts[icont].t
						           : pcontacts[icont].t * -(pcontacts[icont].dir * axisz);
						n_accum += pcontacts[icont].n * -tcur;
						if (pcontacts[icont].n * axisz > 0.5f)
						{
							if (tcur > tmax)
							{
								tmax = tcur;
								...
								m_susp[iwheel].ptcontact =
								    pcontacts[icont].pt +
								    axis * (m_susp[iwheel].width -
								            axis * (pcontacts[icont].pt - ptcw));
						...
						else if (pcontacts[icont].n * axisz >
						             -0.01f && // if contact normal is too steep (but not
						                       // negative),
						         pentlist[ient]->GetRigidBody(j)->Minv <
						             m_body.Minv * 10.f && // ..and colliding body is not loo
						                                   // light (too avoid degeneracy)
						         (m_kSteerToTrack == 0 ||
						          m_susp[iwheel].fullen ==
						              0)) // never do it for tank's road wheels
						{
		...
		if (tmaxTilted > tmax * 1.01f)
		{
			...
			m_susp[iwheel].bContact = 0; // treat wheel as part of vehicle's rigid geometry
		}
		m_posNew += unproj;
		m_body.pos += unproj;
		...
		if (m_susp[iwheel].bContact)
		{
			...
			{
				m_susp[iwheel].vrel -=
				    m_susp[iwheel].pbody->v +
				    (m_susp[iwheel].pbody->w ^ m_susp[iwheel].ptcontact - m_susp[iwheel].pbody->pos);
			}
			...
			if (ip.bSweepTest)
			{
				...
				{
					m_susp[iwheel].curlen =
					    min(newlen, m_susp[iwheel].curlen +
					                    ((newlen - m_susp[iwheel].curlen) * (time_interval * 7)));
				}

				if (tmax > m_susp[iwheel].fullen)
				{ // spring is fully compressed, but we still have penetration
					tmax -= m_susp[iwheel].fullen;
					m_susp[iwheel].contact.vreq =
					    axisz * -min(tmax * m_pWorld->m_vars.unprojVelScale * 2,
					                 m_pWorld->m_vars.maxUnprojVel * 0.7f);
				}
				else
				{
			...
			else
			{
				...
				m_susp[iwheel].contact.vreq = (dirmax - axis * (axis * dirmax)) *
				                              min(m_pWorld->m_vars.maxUnprojVel * 0.7f,
				                                  max(0.0f, (tmax - m_pWorld->m_vars.maxContactGap)) *
				                                      m_pWorld->m_vars.unprojVelScale * 2);
			}
		}
	}
	...
	if (bFakeInnerWheels)
	{
		float rspan[2] = {1.0f / max(1e-6f, m_susp[iwMaxy[0]].pt.y - m_susp[iwMiny[0]].pt.y),
		                  1.0f / max(1e-6f, m_susp[iwMaxy[1]].pt.y - m_susp[iwMiny[1]].pt.y)};
		for (iwheel = 0; iwheel < m_nParts - m_nHullParts; iwheel++)
		{
			if (j = isneg(m_susp[iwheel].pt.x),
			    m_susp[iwheel].fullen >
			        (iszero(iwheel - iwMiny[j]) | iszero(iwheel - iwMaxy[j])) * 2 * m_susp[iwheel].fullen)
			{
				...
				m_susp[iwheel].pent->DetachPartContacts(i, 1, this, 1);
				m_susp[iwheel].curlen =
				    (m_susp[iwMiny[j]].curlen * kmin) + (m_susp[iwMaxy[j]].curlen * kmax);
				...
				if (m_susp[iwheel].pbody)
				{
					m_susp[iwheel].vrel -= m_susp[iwheel].pbody->v +
					                       (m_susp[iwheel].pbody->w ^
					                        m_susp[iwheel].ptcontact - m_susp[iwheel].pbody->pos);
				}
				m_susp[iwheel].contact.vreq.zero();
			}
		}
...
float CWheeledVehicleEntity::ComputeDrivingTorque(float time_interval)
{
	...
	if (m_iCurGear != 1)
	{
		...
		if (fabs_tpl(m_wengine) > m_engineMinw)
		{ // m_enginePedal!=0) {
	...
	iNewGear = max(m_minGear, min(m_maxGear, iNewGear));
	...
	if (m_enginePedal * sgn(m_iCurGear - 1) > 0)
	{
		if (m_wengine > 0.1f)
		{
			power = sin_tpl(min(m_wengine, m_engineMaxw * 1.5f) / m_engineMaxw * g_PI) * enginePower *
			        fabs_tpl(m_enginePedal);
			T = power / m_wengine;
		}
		else
		{
...
void CWheeledVehicleEntity::AddAdditionalImpulses(float time_interval)
{
	...
	for (i = 0; i < m_nParts - m_nHullParts; i++)
	{
		if (m_susp[i].bContact)
		{
			...
			if (!(m_susp[i].pent->m_bProcessed & 1))
			{
				AtomicAdd(&m_susp[i].pent->m_bProcessed, 1);
			}
		}
	}
	for (i = m_nColliders - 1; i >= 0; i--)
	{
		if (!(m_pColliders[i]->m_bProcessed & 1) && !m_pColliderContacts[i] && !m_pColliderConstraints[i] &&
	...
	for (i = 0; i < m_nParts - m_nHullParts; i++)
	{
		if (m_susp[i].bContact && m_susp[i].pent->m_bProcessed & 1)
		{
			AtomicAdd(&m_susp[i].pent->m_bProcessed, -1);
		}
	}
	...
	for (i = 0; i < m_nParts - m_nHullParts; i++)
	{
		...
		{
			quotientf T0(-m_susp[i].w * m_susp[i].prevTdt * 0.7f,
			             (m_susp[i].w - m_susp[i].prevw) * time_interval);
		...
		if (m_susp[i].bDriving)
		{
			...
			{
				if (m_susp[i].w * m_enginePedal >=
				    0) // for 'braking' use max(brakingTorque,drivingTorque)
				{
	...
	for (i = 0; i < m_nParts - m_nHullParts; i++)
	{
		...
		if (m_susp[i].bContact && m_susp[i].iAxle >= 0)
		{
			...
			{
				fN -= (m_susp[i].curlen - m_susp[m_susp[i].iBuddy].curlen) * m_susp[i].kStiffness *
				      m_kStabilizer;
			}
	...
	{
		if ((m_kSteerToTrack != 0 && bContact[iside = isneg(m_susp[i].pt.x)] && m_susp[i].bDriving ||
		     m_susp[i].bContact) &&
		    m_susp[i].iAxle >= 0)
		{
			axis = R * Vec3(cos_tpl(m_susp[i].steer), -sin_tpl(m_susp[i].steer), 0);
			iDriver[iside] |= i & -m_susp[i].bDriving;
			...
			if (m_susp[i].bContact)
			{
				...
				friction =
				    min(m_susp[i].maxFriction,
				        max(m_susp[i].minFriction,
				...
				Npull = N =
				    (m_susp[i].PN + iszero(m_susp[i].fullen) * m_susp[i].contact.Pspare) * friction;
				...
				if (m_kSteerToTrack != 0)
				{
					...
					else
					{
						...
						pulldir = R * (m_enginePedal >= 0
						                   ? Vec3(0, cos_tpl(m_pullTilt), -sin_tpl(m_pullTilt))
						                   : Vec3(0, 1, 0));
					}
				}
			...
			else
			{
				...
				pulldir = R * (m_enginePedal >= 0 ? Vec3(0, cos_tpl(m_pullTilt), -sin_tpl(m_pullTilt))
				                                  : Vec3(0, 1, 0));
			}
			...
			if ((m_bHandBrake & m_susp[i].bCanBrake) | m_susp[i].bBlocked)
			{
				if (m_susp[i].curlen > 0)
				{
					...
					m_susp[i].contact.flags = contact_wheel | contact_use_C;
				}
				else
				{
					m_susp[i].contact.flags = 0;
				}
				frictionLat = friction;
			}
			else
			{
				ptc = R * (m_susp[i].ptc0 - Vec3(0, 0, m_susp[i].curlen - m_susp[i].len0)) + m_posNew -
				      m_body.pos;
				...
				if (isneg(fabs_tpl(Npull) - fabs_tpl(fpull)) & m_susp[i].bDriving)
				{
					...
					if (m_gears[m_iCurGear] * m_enginePedal > 0)
					{
						float wengineLoc =
						    wengine *
						    (m_steer * m_susp[i].pt.x > 0
						         ? fabs_tpl(
							       max(-1.0f, 1.0f - fabs_tpl(m_steer * m_kSteerToTrack)))
						         : 1.0f);
						m_susp[i].w = (m_susp[i].w * (1 - time_interval * 4)) +
						              (wengineLoc * time_interval * 4);
				...
				else
				{
					...
					m_susp[i].contact.flags = contact_use_C;
				}
				frictionLat = friction * m_susp[i].kLatFriction;
			}
			...
			if (m_susp[i].bContact)
			{
				...
				m_susp[i].pent->GetContactMatrix(m_susp[i].ptcontact, m_susp[i].ipart,
				                                 m_susp[i].contact.K);
				m_susp[i].contact.friction = frictionLat;
				m_susp[i].contact.Pspare =
				    m_susp[i].kLatFriction * max(0.0f, (axisz * m_susp[i].ncontact) * N);
...
int CWheeledVehicleEntity::RegisterContacts(float time_interval, int nMaxPlaneContacts)
{
	...
	for (int i = 0; i < m_nParts - m_nHullParts; i++)
	{
		...
		if (m_susp[i].bContact)
		{
			...
			if (m_susp[i].pent->m_parts[m_susp[i].ipart].flags & geom_manually_breakable ||
			    m_susp[i].curlen <= 0)
			{
				...
				if (m_susp[i].curlen <= 0)
				{
					epc.vloc[0] +=
					    epc.n *
					    (epc.n * (m_body.v + (m_body.w ^ m_susp[i].ptcontact - m_body.pos)));
...
{
	return m_bHasContacts ? CRigidEntity::GetDamping(time_interval)
	                      : max(0.0f, 1.0f - (m_dampingVehicle * time_interval));
}
...
int CWheeledVehicleEntity::Update(float time_interval, float damping)
{
	...
	return (m_bAwake ^ 1) | isneg(m_timeStepFull - m_timeStepPerformed - 0.001f) |
	       iszero(m_enginePedal + (~m_flags & pef_invisible));
}

int CWheeledVehicleEntity::GetStateSnapshot(TSerialize ser, float time_back, int flags)
{
	SWheeledVehicleEntityNetSerialize helper = {m_susp,    m_enginePedal,     m_steer,   m_clutch,
	                                            m_wengine, m_bHandBrake != 0, m_iCurGear};
...
void CWheeledVehicleEntity::DrawHelperInformation(IPhysRenderer* pRenderer, int flags)
{
	...
	if (flags & pe_helper_collisions)
	{
		for (int i = 0; i < m_nParts - m_nHullParts; i++)
		{
			if (m_susp[i].bContact && m_susp[i].iAxle >= 0)
			{
				pRenderer->DrawLine(m_susp[i].ptcontact,
				                    m_susp[i].ptcontact +
				                        m_susp[i].ncontact * m_pWorld->m_vars.maxContactGap * 30,
				                    m_iSimClass);
			}
		}
	}
