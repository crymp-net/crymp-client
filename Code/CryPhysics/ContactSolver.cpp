...
RigidBody* g_pBodies[MAX_CONTACTS];
contact_helper g_ContactsRB[MAX_CONTACTS];
body_helper g_Bodies[MAX_CONTACTS];
body_info g_infos[MAX_CONTACTS];
contact_sandwich g_sandwichbuf[MAX_CONTACTS];
buddy_info g_buddybuf[MAX_CONTACTS];
follower_thunk g_followersbuf[MAX_CONTACTS];
int g_nFollowers;
int g_idLastUpdate;
int g_nUnprojLoops;
char g_SolverBuf[262144];
int g_iSolverBufPos;

bool should_swap(entity_contact** pContacts, int i1, int i2, int& diff)
{
...
bool should_swap(RigidBody** pBodies, int i1, int i2, int& diff)
{
	diff |= g_infos[pBodies[i1]->bProcessed].iLevel - g_infos[pBodies[i2]->bProcessed].iLevel;
...
bool should_swap(entity_contact_unproj** pContacts, int i1, int i2, int& diff)
{ // sorts contacts by newly sorted bodies
	int iop1 = isneg(g_infos[pContacts[i1]->pbody[0]->bProcessed].iLevel -
	                 g_infos[pContacts[i1]->pbody[1]->bProcessed].iLevel);
	int iop2 = isneg(g_infos[pContacts[i2]->pbody[0]->bProcessed].iLevel -
	                 g_infos[pContacts[i2]->pbody[1]->bProcessed].iLevel);
	diff |=
	    g_infos[pContacts[i1]->pbody[iop1]->bProcessed].idx - g_infos[pContacts[i2]->pbody[iop2]->bProcessed].idx;
	return g_infos[pContacts[i1]->pbody[iop1]->bProcessed].idx <
	       g_infos[pContacts[i2]->pbody[iop2]->bProcessed].idx;
}

template<class dtype>
void qsort(dtype* pArray, int left, int right)
{
	if (left >= right)
	{
		return;
...
int bidx0(int i)
{
	return g_pContacts[i]->pbody[0]->bProcessed;
}

int bidx1(int i)
{
	return g_pContacts[i]->pbody[1]->bProcessed;
}
...
void add_route_follower(int iBody, int iFollower)
{
	...
	for (pfollower = g_infos[iBody].pfollower; pfollower && pfollower->iBody != iFollower;
	     pfollower = pfollower->next)
		;
...
void trace_unproj_route(int iMiddle, int iBread)
{
	...
	{
		if (iszero(psandwich->iMiddle - iMiddle) &
		    ((iop = iszero(psandwich->iBread[0] - iBread)) | iszero(psandwich->iBread[1] - iBread)))
		{
...
real ComputeRc(RigidBody* body0, entity_contact** pContacts, int nAngContacts,
               int nContacts) // for MINRES unprojection solver
{
	int i;
	real rT_x_rc = 0;
	Vec3 dP, r0;
...
void InvokeContactSolver(float time_interval, SolverSettings* pss, float Ebefore)
{
	...
	for (i = nBodies = nConstraints = 0; i < g_nContacts; i++)
	{
		for (iop = 0; iop < 2; iop++)
		{
			...
			g_ContactsRB[i].iBody[iop] = g_pContacts[i]->pbody[iop]->bProcessed - 1;
		}
		...
		{
			for (iop = 0, prope = (rope_solver_vtx*)g_pContacts[i]->pBounceCount;
			     iop < g_pContacts[i]->iCount; iop++)
			{
				if (!(prope[iop].pbody->bProcessed))
				{
					g_pBodies[nBodies++] = prope[iop].pbody;
					prope[iop].pbody->bProcessed = nBodies;
				}
				prope[iop].iBody = prope[iop].pbody->bProcessed - 1;
			}
		}
	...
	if (pss->nMaxLCPCGiters > 0)
	{
		...
		if (!bNeedLCPCG)
		{
			...
			for (istart = nBuddies = 0; istart < g_nContacts; istart = iend)
			{
				...
				for (iend = istart + 1;
				     iend < g_nContacts && g_pContacts[iend]->pbody[0] == g_pContacts[istart]->pbody[0];
				     iend++)
					;
				// qsort(g_pContacts, istart,iend-1);
				//  for each body: find all its contacting bodies and for each such body get integral
				//  vreq (but ignore separating contacts)
				for (i = istart; i < iend; i = j)
				{
					for (j = i, n.zero();
					     j < iend && g_pContacts[j]->pbody[1] == g_pContacts[i]->pbody[1]; j++)
					{
						n += g_pContacts[j]->n;
					}
					if (g_pContacts[i]->pbody[0]->Minv >
					    g_pContacts[i]->pbody[1]->Minv * pss->maxMCMassRatio)
					{
					...
					if (g_pContacts[i]->pbody[1]->Minv >
					    g_pContacts[i]->pbody[0]->Minv * pss->maxMCMassRatio)
					{
			...
			// for every 2 contacting bodies of each body check if vreqs are conflicting, register sandwich
			// triplet if so (1 inside, 2 outside)
	...
	if (g_bUsePreCG && g_nContacts < 16)
	{
		...
		{
			Eafter += (g_pBodies[i]->P + g_pBodies[i]->Fcollision) *
			              (g_pBodies[i]->v + g_pBodies[i]->Fcollision * g_pBodies[i]->Minv) +
			          (g_pBodies[i]->L + g_pBodies[i]->Tcollision) *
			              (g_pBodies[i]->w + g_pBodies[i]->Iinv * g_pBodies[i]->Tcollision);
		}
		for (i = 0; i < g_nContacts; i++)
		{
			n = g_pContacts[i]->n;
			...
			{
				dp = g_pContacts[i]->P *
				     (g_pContacts[i]->pbody[0]->Minv + g_pContacts[i]->pbody[1]->Minv);
				vdiff = -0.004f;
			}
			else
			{
				dp = g_pContacts[i]->pbody[0]->Iinv * g_pContacts[i]->P +
				     g_pContacts[i]->pbody[1]->Iinv * g_pContacts[i]->P;
				vdiff = -0.015f;
			}
		...
		{ // conjugate gradient yielded acceptable results, apply these impulses and quit
			for (i = 0; i < nBodies; i++)
			{
			...
			for (i = 0; i < g_nContacts; i++)
			{
				...
				g_pContacts[i]->Pspare = g_pContacts[i]->dPn = g_pContacts[i]->P * g_pContacts[i]->n;
			}
			return;
		}
	}
	...
	for (i = 0; i < g_nContacts; i++)
	{
		...
		g_ContactsRB[i].iCountDst =
		    ((entity_contact*)((char*)g_pContacts[i]->pBounceCount -
		                       ((char*)&g_pContacts[i]->iCount - (char*)g_pContacts[i])))
			->bProcessed;
		g_ContactsRB[i].Pn = 0;
	}
	for (i = 0; i < nBodies; i++)
	...
	// bBounced=1; if (false)
	{
			...
			for (i = istart; i != iend; i += istep)
			{
				if (g_ContactsRB[i].iCount >= (g_ContactsRB[i].flags & contact_count_mask))
				{
					...
					if (g_ContactsRB[i].flags & contact_rope)
					{
						...
						{
							g_ContactsRB[i].Pn += dPn = -vrel * g_ContactsRB[i].K(0, 1);
					...
					{
						if ((g_pContacts[i]->C * dp).len2() >
						    max(sqr(e), g_ContactsRB[i].vreq.len2() * sqr(0.05f)))
						{
							dP = g_pContacts[i]->Kinv * -dp;
							g_ContactsRB[i].Pn += dP * g_ContactsRB[i].n;
							bContactBounced = 1;
						}
					}
					...
					{
						if ((vrel = dp * n) < 0 &&
						    (isneg(e - fabs_tpl(vrel)) |
						     isneg(0.0001f - g_ContactsRB[i].Pspare) &
						         isneg(sqr(pss->minSeparationSpeed) - (dp - n * vrel).len2())))
						// if ((vrel=dp*n)<0 && (vrel<-0.003 || g_pContacts[i]->Pspare>0.0001f
						// && (dp-n*vrel).len2()>sqr(pss->minSeparationSpeed)))
						{
							if (g_ContactsRB[i].friction > 0.01f)
							{
								dP = dp * (-dp.len2() / (dp * g_ContactsRB[i].K * dp));
								g_ContactsRB[i].Pn += dPn = dP * n;
								g_ContactsRB[i].Pspare +=
								    (dPn *= g_ContactsRB[i].friction);
								dPtang = sqrt_tpl(max(0.0f, dP.len2() - sqr(dP * n)));
								g_ContactsRB[i].Pspare -= dPtang;
								if (g_ContactsRB[i].Pspare < 0)
								{ // friction cannot stop sliding
									dp += (dp - n * vrel) *
									      ((g_ContactsRB[i].Pspare) /
									       dPtang); // remove part of dp that
									                // friction cannot stop
									Kdp = g_ContactsRB[i].K * dp;
									if (sqr(Kdp * n) <
									    Kdp.len2() *
									        0.04f) // switch to frictionless contact
									               // in dangerous cases
									{
										dP = n * -vrel /
										     (n * g_ContactsRB[i].K * n);
									}
									else
									{
										dP = dp * -vrel /
										     (n * Kdp); // apply impulse along
										                // dp so that it stops
										                // normal component
									}
					...
					else if (dp.len2() > sqr(pss->minSeparationSpeed) &&
					         g_ContactsRB[i].Pspare > g_pContacts[i]->pbody[0]->M * 0.001f)
					{
						dP = dp * (-dp.len2() / (dp * g_ContactsRB[i].K * dp));
						g_ContactsRB[i].Pn += dPn = dP * n;
						dPn *= g_ContactsRB[i].friction;
		...
		for (i = 0; i < nBodies; i++)
		{
			...
			g_pBodies[i]->L = g_pBodies[i]->q * (g_pBodies[i]->Ibody *
			                                     (!g_pBodies[i]->q * (g_pBodies[i]->w = g_Bodies[i].w)));
		}
		for (i = 0; i < g_nContacts; i++)
		{
	...
	if (bBounced && pss->nMaxLCPCGiters > 0 && bNeedLCPCG && g_nContacts < pss->maxLCPCGContacts)
	{
		...
		// prepare for CG solver: calculate target residuals (they will be used as starting residuals during
		// each iteration) and vrel's
		for (i = bBounced = 0; i < g_nContacts; i++)
		{
			...
			if (g_pContacts[i]->flags & contact_rope)
			{
				g_pContacts[i]->C.SetRow(0, g_pContacts[i]->n);
				g_pContacts[i]->C.SetRow(1, g_pContacts[i]->vreq);
				g_pContacts[i]->n(1, 0, 0);
				for (iop = 0, g_pContacts[i]->vrel.zero(); iop < 2; iop++)
				{
					body0 = g_pContacts[i]->pbody[iop];
					r0 = g_pContacts[i]->pt[iop] - body0->pos;
					g_pContacts[i]->vrel.x +=
					    g_pContacts[i]->C.GetRow(iop) *
					    (body0->Fcollision * body0->Minv + (body0->Iinv * body0->Tcollision ^ r0));
				}
				for (iop = 0, prope = (rope_solver_vtx*)g_pContacts[i]->pBounceCount;
				     iop < g_pContacts[i]->iCount; iop++)
				{
					...
					g_pContacts[i]->vrel.x +=
					    prope[iop].v * (body0->Fcollision * body0->Minv +
					                    (body0->Iinv * body0->Tcollision ^ prope[iop].r));
				}
				g_pContacts[i]->r0 = -g_pContacts[i]->vrel;
				vreq(g_pContacts[i]->friction, 0, 0);
				g_pContacts[i]->Kinv.SetZero();
				g_pContacts[i]->Kinv(0, 0) = g_pContacts[i]->K(0, 1);
			}
			else
			{
				if (!(g_pContacts[i]->flags & contact_angular))
				...
				if (g_pContacts[i]->flags & contact_wheel)
				{
					...
					{
						g_pContacts[i]->flags =
						    (g_pContacts[i]->flags & contact_use_C) | contact_constraint_3dof;
					}
					else
					{
						g_pContacts[i]->flags =
						    contact_count_mask; // don't solve for this contact
					}
				}
				...
				if (g_pContacts[i]->flags & contact_constraint_1dof)
				{
					g_pContacts[i]->r0 =
					    (g_pContacts[i]->vrel -=
					     g_pContacts[i]->n * (g_pContacts[i]->vrel * g_pContacts[i]->n));
				}
				else if (g_pContacts[i]->flags & contact_constraint_2dof ||
				         !(g_pContacts[i]->flags & contact_constraint_3dof ||
				           g_pContacts[i]->Pspare > 0))
				{
					...
					g_pContacts[i]->Kinv.SetZero();
					g_pContacts[i]->Kinv(0, 0) =
					    1.0f / (g_pContacts[i]->n * g_pContacts[i]->K * g_pContacts[i]->n);
				}
				else
				{
					g_pContacts[i]->r0 = g_pContacts[i]->vrel;
					if (!(g_pContacts[i]->flags & contact_constraint))
					{
						(g_pContacts[i]->Kinv = g_pContacts[i]->K)
						    .Invert(); // for constraints it's computed earlier
					}
				}
			...
			if (g_pContacts[i]->flags & contact_constraint || g_pContacts[i]->vrel * g_pContacts[i]->n < 0)
			{
				...
				bBounced += isneg((sqr(body0->softness[iop] + body1->softness[iop]) * 0.25f) -
				                  ((g_pContacts[i]->r0 + vreq).len2() * sqr(time_interval)));
			}
		...
		{
			{
				...
				vrel =
				    min(pss->nMaxLCPCGmicroitersFinal, pss->nMaxLCPCGsubitersFinal) <= 0 ? 1E10f : 0.0f;

				do
				{
					// sort contacts for the current iteration in the following way:
					// 0 | 1dof angular (always remove v-n*(v*n)) |	angular 1 | 2dof angular
					// constraints (always remove n*v)		|	angular 2 | angular
					// limits (remove n*v if <0) |	angular 3 | ropes
					// | special 4 | frictionless contacts (remove n*v if <0)
					// |	positional 5 | infinite friction contacts (remove v if n*v<0)	|
					// positional 6 | 3dof constraints (always remove v)
					// |	positional
					for (i = 0; i < 7; i++)
					{
						iSortedContacts[i] = 0;
					...
					for (i = bStateChanged = 0; i < g_nContacts; i++)
					{
						...
						if (!(g_pContacts[i]->flags & contact_count_mask))
						{
							...
							{
								if (g_pContacts[i]->vrel * g_pContacts[i]->n < vrel)
								{
									if (g_pContacts[i]->flags & contact_angular)
									{
										iSortedContacts[2]++;
								...
								bStateChanged +=
								    iszero((flags ^ g_pContacts[i]->flags) &
								           contact_solve_for) ^
								    1;
							}
						}
					...
					iter = (iSortedContacts[0] * 2) + iSortedContacts[1] + iSortedContacts[2] +
					       iSortedContacts[3] + iSortedContacts[4] +
					...
					for (i = 0, r2 = 0; i < g_nContacts; i++)
					{
						...
						g_pContacts[i]->vrel =
						    g_pContacts[i]->Kinv * (g_pContacts[i]->r = g_pContacts[i]->r0);
						if (g_pContacts[i]->flags & contact_use_C)
						{
							g_pContacts[i]->vrel = g_pContacts[i]->C * g_pContacts[i]->vrel;
						}
						...
						g_pContacts[i]->dP =
						    iClass - 1u < 4u ? g_pContacts[i]->n * (g_pContacts[i]->dPn =
						                                                g_pContacts[i]->vrel.x)
								     : g_pContacts[i]->vrel;
						g_pContacts[i]->P.zero();
					}
					...
					if (r2 > 0 && iter > 0)
					{
						do
						{
							...
							for (; i < nRopes; i++)
							{ // ropes
								for (iop = 0, dPn = pContacts[i]->dPn; iop < 2;
								     iop++, dPn *= pContacts[i]->K(0, 0))
								{
									body0 = pContacts[i]->pbody[iop];
									dP = pContacts[i]->C.GetRow(iop) * dPn;
									body0->Fcollision += dP;
									body0->Tcollision +=
									    pContacts[i]->pt[iop] - body0->pos ^ dP;
								}
								for (iop = 0,
								    prope =
								         (rope_solver_vtx*)pContacts[i]->pBounceCount;
								     iop < pContacts[i]->iCount; iop++)
								{
									prope[iop].pbody->Fcollision +=
									    prope[iop].P * pContacts[i]->dPn;
									prope[iop].pbody->Tcollision +=
									    prope[iop].r ^
									    prope[iop].P * pContacts[i]->dPn;
								}
							}
							...
							{ // angular contacts
								...
								body1 = pContacts[i]->pbody[1];
								pContacts[i]->vrel = body0->Iinv * body0->Tcollision -
								                     body1->Iinv * body1->Tcollision;
							}
							for (; i < nRopes; i++)
							{
								for (iop = 0, pContacts[i]->vrel.zero(); iop < 2; iop++)
								{
									body0 = pContacts[i]->pbody[iop];
									r0 = pContacts[i]->pt[iop] - body0->pos;
									pContacts[i]->vrel.x +=
									    pContacts[i]->C.GetRow(iop) *
									    (body0->Fcollision * body0->Minv +
									     (body0->Iinv * body0->Tcollision ^ r0));
								}
								for (iop = 0,
								    prope =
								         (rope_solver_vtx*)pContacts[i]->pBounceCount;
							...
							{ // positional contacts
								...
								pContacts[i]->vrel =
								    ...
								    (body0->Iinv * body0->Tcollision ^ r0);
								pContacts[i]->vrel -=
								    body1->Fcollision * body1->Minv +
								    (body1->Iinv * body1->Tcollision ^ r1);
							}

							pAp = 0;
							for (i = 0; i < n1dofContacts; i++)
							{ // 1-dof contacts (remove 1 axis from vrel)
								pContacts[i]->vrel -=
								    pContacts[i]->n *
							...
							{ // general 2-dof contacts (remove 2 axes from vrel - leave 1)
								pContacts[i]->vrel.x =
								    pContacts[i]->n * pContacts[i]->vrel;
								pAp += pContacts[i]->vrel.x * pContacts[i]->dPn;
							}
							for (; i < nContacts;
							     i++) // general 3-dof contacts (leave vrel untouched - all
							          // directions need to be 0ed)
							{
								pAp += pContacts[i]->vrel * pContacts[i]->dP;
							}
							...
							if (nFric0Contacts - n1dofContacts < nContacts)
							{
								...
								{ // contacts with 3(and 2)-components vel
									pContacts[i]->vrel =
									    pContacts[i]->Kinv *
									    (pContacts[i]->r -= pContacts[i]->vrel * a);
									if (pContacts[i]->flags & contact_use_C)
									{
										pContacts[i]->vrel = pContacts[i]->C *
										                     pContacts[i]->vrel;
									}
							...
							for (; i < nFric0Contacts; i++)
							{ // contacts with 1-component vel
								pContacts[i]->vrel.x =
								    pContacts[i]->Kinv(0, 0) *
							...
							bNoImprovement = max(
							    0, bNoImprovement -
								   sgnnz(r2 - r2new - (r2 * pss->minLCPCGimprovement)));
							b = r2new / r2;
							r2 = r2new;
							vmax = 0;
							i = nFric0Contacts & nFric0Contacts - nContacts >> 31;
						...
						while (--iter && vmax > sqr(e) &&
						       !(bNoImprovement > pss->nMaxLCPCGFruitlessIters &&
						         vmax < sqr(pss->accuracyLCPCGnoimprovement)));
					}
					...
					for (; i < nRopes; i++)
					{ // ropes
						for (iop = 0, dPn = pContacts[i]->P.x; iop < 2;
						     iop++, dPn *= pContacts[i]->K(0, 0))
						{
							body0 = pContacts[i]->pbody[iop];
							dP = pContacts[i]->C.GetRow(iop) * dPn;
							body0->Fcollision += dP;
							body0->Tcollision += pContacts[i]->pt[iop] - body0->pos ^ dP;
						}
						for (iop = 0, prope = (rope_solver_vtx*)pContacts[i]->pBounceCount;
						     iop < pContacts[i]->iCount; iop++)
						{
							prope[iop].pbody->Fcollision +=
							    prope[iop].P * pContacts[i]->P.x;
							prope[iop].pbody->Tcollision +=
							    prope[iop].r ^ prope[iop].P * pContacts[i]->P.x;
						}
					}
					for (; i < nContacts; i++)
					{ // positional contacts
						...
						if (!(g_pContacts[i]->flags & (contact_solve_for | contact_count_mask)))
						{
							if (!(g_pContacts[i]->flags & contact_rope))
							{
								...
								if (!(g_pContacts[i]->flags & contact_angular))
								{
									...
									r1 = g_pContacts[i]->pt[1] - body1->pos;
									g_pContacts[i]->vrel =
									    body0->v + body0->Fcollision * body0->Minv +
									    (body0->w +
									         body0->Iinv * body0->Tcollision ^
									     r0);
									g_pContacts[i]->vrel -=
									    body1->v + body1->Fcollision * body1->Minv +
									    (body1->w +
									         body1->Iinv * body1->Tcollision ^
									     r1);
								}
								...
								{
									g_pContacts[i]->vrel =
									    body0->w + body0->Iinv * body0->Tcollision -
									    body1->w - body1->Iinv * body1->Tcollision;
								}
								if (g_pContacts[i]->flags & contact_use_C)
								{
									g_pContacts[i]->vrel =
									    g_pContacts[i]->C * g_pContacts[i]->vrel;
								}
							}
							else
							{
								for (iop = 0, g_pContacts[i]->vrel.zero(); iop < 2;
								     iop++)
								{
									body0 = g_pContacts[i]->pbody[iop];
									r0 = g_pContacts[i]->pt[iop] - body0->pos;
								...
								for (iop = 0,
								    prope =
								         (rope_solver_vtx*)g_pContacts[i]->pBounceCount;
								     iop < g_pContacts[i]->iCount; iop++)
								{
									body0 = prope[iop].pbody;
									g_pContacts[i]->vrel.x +=
					...
					if (cgiter > 2)
					{
						...
						{
							pContacts[i]->vrel =
							    -pContacts[i]->P * max(pContacts[i]->pbody[0]->Minv,
							                           pContacts[i]->pbody[1]->Minv) -
							    pContacts[i]->n * (e * 3);
						}
					}
					else
					{
						for (i = n2dofContacts; i < nFricInfContacts;
						     i++) // if we reached the last iteration, keep the current 'solve
						          // for' contacts unconditionally
						{
							pContacts[i]->vrel = -pContacts[i]->n;
						}
					}
				...
				if (cgiter < pss->nMaxLCPCGiters)
				{
					...
					{
						vmax =
						    max(vmax, (pContacts[i]->pbody[0]->Ibody * pContacts[i]->P).len2());
						vmax =
						    max(vmax, (pContacts[i]->pbody[1]->Ibody * pContacts[i]->P).len2());
					}
					...
					{
						vmax = max(vmax,
						           sqr(pContacts[i]->pbody[0]->Minv) * pContacts[i]->P.len2());
						vmax = max(vmax,
						           sqr(pContacts[i]->pbody[1]->Minv) * pContacts[i]->P.len2());
					}
					...
					if (bBounced)
					{
						...
						{ // ropes
							     ...
							     iop++, dPn *= pContacts[i]->K(0, 0))
							{
								...
								dP = pContacts[i]->C.GetRow(iop) * dPn;
								body0->P += dP;
								body0->L += pContacts[i]->pt[iop] - body0->pos ^ dP;
							}
							for (iop = 0,
							    prope = (rope_solver_vtx*)pContacts[i]->pBounceCount;
							     iop < pContacts[i]->iCount; iop++)
							{
								prope[iop].pbody->P += prope[iop].P * pContacts[i]->P.x;
								prope[iop].pbody->L +=
								    prope[iop].r ^ prope[iop].P * pContacts[i]->P.x;
							}
						}
				...
				for (i = 0; i < g_nContacts; i++)
				{
					if (g_pContacts[i]->flags & contact_rope)
					{
						g_pContacts[i]->n = g_pContacts[i]->C.GetRow(0);
						g_pContacts[i]->vreq = g_pContacts[i]->C.GetRow(1);
					}
				}
			...
			if (bBounced)
			{
				...
				{
					g_pContacts[i]->flags &=
					    ~contact_solve_for | -(g_pContacts[i]->flags & contact_rope) >> 31;
				}
				// require that the all contacts are grouped by pbody[0]
				for (istart = nBuddies = 0; istart < g_nContacts; istart = iend)
				{
					// sub-group contacts relating to each pbody[0] by pbody[1] (using quick sort)
					for (iend = istart + 1; iend < g_nContacts && g_pContacts[iend]->pbody[0] ==
					                                                  g_pContacts[istart]->pbody[0];
					     iend++)
						;
					qsort(g_pContacts, istart, iend - 1);
					// for each body: find all its contacting bodies and for each such body get
					// integral vreq (but ignore separating contacts)
					for (i = istart; i < iend; i = j)
					{
						for (j = i, vreq.zero(), flags = 0;
						     j < iend && g_pContacts[j]->pbody[1] == g_pContacts[i]->pbody[1];
						     j++)
						{
							if (g_pContacts[j]->flags & contact_solve_for &&
							    g_pContacts[j]->vreq.len2() > 0)
							{
				...
				// for every 2 contacting bodies of each body check if vreqs are conflicting, register
				// sandwich triplet if so (1 inside, 2 outside)
				for (i = nSandwiches = 0; i < nBodies; i++)
				{
					for (pbuddy0 = g_infos[i].pbuddy; pbuddy0; pbuddy0 = pbuddy0->next)
					{
						for (pbuddy1 = pbuddy0->next; pbuddy1; pbuddy1 = pbuddy1->next)
						{
							if (pbuddy0->vreq * pbuddy1->vreq < 0 &&
							    !((pbuddy0->flags ^ pbuddy1->flags) & contact_angular))
							{
								...
								if (nSandwiches ==
								    sizeof(g_sandwichbuf) / sizeof(g_sandwichbuf[0]))
								{
									goto endsandw;
								}
							}
			...
			endsandw:
				// call tracepath for each sandwich with static	as 'bread' (if no static - assign a
				// 'pseudo-static')
				for (i = 0; i < nBodies; i++)
				{
				...
				if (minMinv > 0)
				{ // if no static participates in sandwich, assign bodies contacting with statics Minv 0
				  // and repeat the procedure
					for (i = 0; i < g_nContacts; i++)
					{
						if (g_pContacts[i]->pbody[0]->Minv * g_pContacts[i]->pbody[1]->Minv ==
						    0)
						{
							g_infos[g_pContacts[i]
							            ->pbody[g_pContacts[i]->pbody[0]->Minv == 0]
							            ->bProcessed]
							    .Minv = 0;
						}
					}
				...
				{
					if (j == g_sandwichbuf[i].iBread[0] ||
					    g_infos[g_sandwichbuf[i].iBread[0]].Minv == 0)
					{
						iop = 0;
					}
					else if (j == g_sandwichbuf[i].iBread[1] ||
					         g_infos[g_sandwichbuf[i].iBread[1]].Minv == 0)
					{
						iop = 1;
					}
					else
					...
					g_infos[g_sandwichbuf[i].iMiddle].iLevel =
					    max(1, g_infos[g_sandwichbuf[i].iMiddle].iLevel);
					trace_unproj_route(g_sandwichbuf[i].iMiddle, g_sandwichbuf[i].iBread[iop]);
				}

				// option: since after this moment we are starting to get 'heuristic', maybe we should
				// randomize the order of sandwiches?
				iter = 0;
				do
				{
					// iteratively select an unprocessed sandwich with the min level body (bread or
					// middle), call trace_unproj_route for it; when the route reaches initialized
					// branches, they will automatically update all previously traced followers
					do
					{
						for (i = 0, j = -1, iMinLevel = 0x10000; i < nSandwiches; i++)
						{
							...
							{
								iLevel = min(
								    g_infos[g_sandwichbuf[i].iMiddle].iLevel & 0xFFFF,
								    min(g_infos[g_sandwichbuf[i].iBread[0]].iLevel &
								            0xFFFF,
						...
						i = (g_infos[g_sandwichbuf[j].iBread[0]].iLevel >> 31 & 1 |
						     g_infos[g_sandwichbuf[j].iBread[1]].iLevel >> 31 & 2 |
						     g_infos[g_sandwichbuf[j].iMiddle].iLevel >> 31 & 4) ^
						    7;
						if (i == 7)
						{ // all participating bodies already have levels
							iop = isneg(g_infos[g_sandwichbuf[j].iBread[1]].iLevel -
							            g_infos[g_sandwichbuf[j].iBread[0]]
							                .iLevel); // min level bread
							if (g_infos[g_sandwichbuf[j].iMiddle].iLevel >=
							    g_infos[g_sandwichbuf[j].iBread[iop]].iLevel)
							{
								add_route_follower(
								    g_sandwichbuf[j].iBread[iop],
								    g_sandwichbuf[j]
									.iMiddle); // breadmin -> middle ->
								                   // breadmax(will be updated)
							}
						}
						else if (i == 3)
						{ // both breads have levels
							g_infos[g_sandwichbuf[j].iMiddle].iLevel = 1;
							if (iMinLevel > 1)
							{ // bread0 <- middle=1 -> bread1
								add_route_follower(g_sandwichbuf[j].iMiddle,
								                   g_sandwichbuf[j].iBread[0]);
								add_route_follower(g_sandwichbuf[j].iMiddle,
								                   g_sandwichbuf[j].iBread[1]);
							}
							else
							{ // breadmin -> middle=1 -> breadmax
								iop = isneg(g_infos[g_sandwichbuf[j].iBread[1]].iLevel -
								            g_infos[g_sandwichbuf[j].iBread[0]]
								                .iLevel); // min level bread
								add_route_follower(g_sandwichbuf[j].iBread[iop],
								                   g_sandwichbuf[j].iMiddle);
							}
						}
						else if (i == 4)
						{ // only the middle has level
							g_infos[g_sandwichbuf[j].iBread[0]].iLevel =
							    g_infos[g_sandwichbuf[j].iBread[1]].iLevel =
								g_infos[g_sandwichbuf[j].iMiddle].iLevel + 1;
							add_route_follower(g_sandwichbuf[j].iMiddle,
							                   g_sandwichbuf[j].iBread[0]);
							add_route_follower(g_sandwichbuf[j].iMiddle,
						...
						{ // either one bread or one bread and the middle have levels
							iop = i >> 1 & 1; // the bread that has level
							g_infos[g_sandwichbuf[j].iMiddle].iLevel =
							    min(g_infos[g_sandwichbuf[j].iMiddle].iLevel & 0xFFFF,
							        iMinLevel + 1);
							if (i & 2) // the middle had level
							{
								add_route_follower(g_sandwichbuf[j].iMiddle,
								                   g_sandwichbuf[j].iBread[iop ^ 1]);
							}
							else
							{
								add_route_follower(g_sandwichbuf[j].iBread[iop],
					...
					// now we have only fully clear sandwiches in isolated branches; as we progress,
					// however, some unprocessed sandwiches might become partially initialized;
					// ignore them - the previous loop will pick them up during the next iteration
					for (i = 0; i < nSandwiches; i++)
					{
						if ((g_infos[g_sandwichbuf[i].iMiddle].iLevel &
						     g_infos[g_sandwichbuf[i].iBread[0]].iLevel &
						     g_infos[g_sandwichbuf[i].iBread[1]].iLevel) == -1)
						{
							g_sandwichbuf[i].bProcessed = 1;
							g_infos[g_sandwichbuf[i].iMiddle].iLevel = 0;
							g_infos[g_sandwichbuf[i].iBread[0]].iLevel =
							    g_infos[g_sandwichbuf[i].iBread[1]].iLevel = 1;
							add_route_follower(g_sandwichbuf[i].iMiddle,
							                   g_sandwichbuf[i].iBread[0]);
							add_route_follower(g_sandwichbuf[i].iMiddle,
							                   g_sandwichbuf[i].iBread[1]);
						}
					}
					++iter;
				}
				...
				// (they either didn't form sandwiches, or belonged to sandwiches that had only middles
				// initialized, and didn't belong to routes
				...
				{
					g_infos[i].iLevel =
					    (iMaxLevel & g_infos[i].iLevel >> 31) | max(g_infos[i].iLevel, 0);
					if (g_pBodies[i]->Minv == 0)
					{
						g_infos[i].iLevel =
						    0; // force level 0 to statics (some can accidentally get non-0
						       // level if they don't participate in sandwiches)
					}
				}
				...
				// sort contact list so that each contact gets assigned to the body with the higher
				// level (among the two participating in the contact)
				qsort((entity_contact_unproj**)g_pContacts, 0, g_nContacts - 1);

				for (istart = 0; istart < g_nContacts; istart = j)
				{
					// among contacts assigned to a body, select only non-separating with non-zero
					// vreq and use cg to enforce vreq (along its direction only) don't enforce
					// contacts with 0 vreq, since if we violate them, they'll be reported to have
					// vreq during the next step
					body0 = g_pContacts[istart]->pbody[isneg(g_infos[bidx0(istart)].iLevel -
					                                         g_infos[bidx1(istart)].iLevel)];
					iSortedContacts[0] = iSortedContacts[1] = 0;
					for (j = istart;
					     j < g_nContacts &&
					     g_pContacts[j]->pbody[iop = isneg(g_infos[bidx0(j)].iLevel -
					                                       g_infos[bidx1(j)].iLevel)] == body0;
					     j++)
					{
						if (g_pContacts[j]->flags & contact_solve_for &&
						    g_pContacts[j]->vreq.len2() > sqr(0.01f))
						{
							g_pContacts[j]->flags =
							    (g_pContacts[j]->flags & ~contact_bidx) |
							    iop << contact_bidx_log2;
							g_pContacts[j]->vrel = g_pContacts[j]->vreq * ((iop * 2) - 1);
							// from now on use dP to store normalized unprojection direction
							if (fabsf(sqr(g_pContacts[j]->vreq * g_pContacts[j]->n) -
							          g_pContacts[j]->vreq.len2()) <
							    g_pContacts[j]->vreq.len2() * 0.01f)
							{
							...
							if (!(g_pContacts[j]->flags & contact_angular))
							{
								...
								g_pContacts[j]->Kinv(0, 0) =
								    body0->Minv +
								    g_pContacts[j]->dP *
									(body0->Iinv * (r0 ^ g_pContacts[j]->dP) ^ r0);
								if (g_pContacts[j]->Kinv(0, 0) < body0->Minv * 0.1f)
								{ // contact has near-degenerate Kinv, skip it
									g_pContacts[j]->flags &= ~contact_solve_for;
									continue;
								}
								g_pContacts[j]->vrel +=
								    body0->v + g_infos[body0->bProcessed].v_unproj +
								    (body0->w + g_infos[body0->bProcessed].w_unproj ^
								     r0);
								g_pContacts[j]->vrel -=
								    body1->v + g_infos[body1->bProcessed].v_unproj +
								    (body1->w + g_infos[body1->bProcessed].w_unproj ^
								     r1);
								iSortedContacts[1]++;
							}
							else
					     ...
					     j++)
					{
						if (g_pContacts[j]->flags & contact_solve_for)
						{
							...
							g_pContacts[j]->r0.x = g_pContacts[j]->r.x =
							    -(g_pContacts[j]->vrel * g_pContacts[j]->dP);
							g_pContacts[j]->dPn =
							    g_pContacts[j]->r.x * g_pContacts[j]->Kinv(0, 0);
							g_pContacts[j]->P.x = 0;
							pContacts[iSortedContacts[(g_pContacts[j]->flags >>
							                               contact_angular_log2 &
							                           1) ^
							                          1]++] = g_pContacts[j];
						}
					}
					...
					if (iter = nContacts)
					{
						do
						{
							...
							{
								body0->Tcollision +=
								    pContacts[i]->dP * pContacts[i]->dPn;
							}
							for (; i < nContacts; i++)
							{ // positional contacts
								r0 = pContacts[i]
								         ->pt[pContacts[i]->flags >> contact_bidx_log2 &
								              1] -
								     body0->pos;
								body0->Fcollision +=
								    pContacts[i]->dP * pContacts[i]->dPn;
								body0->Tcollision +=
								    r0 ^ pContacts[i]->dP * pContacts[i]->dPn;
							}
							for (i = 0; i < nAngContacts; i++) // angular contacts
							{
								pContacts[i]->vrel = body0->Iinv * body0->Tcollision;
							...
							{
								pContacts[i]->vrel.x =
								    pContacts[i]->dP * pContacts[i]->vrel;
								pAp += sqr(pContacts[i]->vrel.x) *
								       pContacts[i]->Kinv(0, 0);
							}
							...
							{
								(pContacts[i]->dPn *= b) +=
								    pContacts[i]->r.x * pContacts[i]->Kinv(0, 0);
							}
						}
						...
						{ // positional contacts
							r0 = pContacts[i]
							         ->pt[pContacts[i]->flags >> contact_bidx_log2 & 1] -
							     body0->pos;
						...
						else
						{
							...
							{
								r2max =
								    max(r2max, (pContacts[i]->pt[pContacts[i]->flags >>
								                                     contact_bidx_log2 &
								                                 1] -
								                body0->pos)
								                   .len2());
						...
						v2unproj = max(g_infos[body0->bProcessed].v_unproj.len2(),
						               g_infos[body0->bProcessed].w_unproj.len2() * r2max);
				...
				for (i = 0; i < nBodies; i++)
				{
					j = g_pBodies[i]->bProcessed;
					g_pBodies[i]->bProcessed = 0;
					Vec3 L = g_pBodies[i]->q *
					         (g_pBodies[i]->Ibody * (g_infos[j].w_unproj * g_pBodies[i]->q));
					g_pBodies[i]->Eunproj = (g_infos[j].v_unproj.len2() +
					                         (g_infos[j].w_unproj * L) * g_pBodies[i]->Minv) *
					                        0.5f;
					if (g_pBodies[i]->Eunproj > 0)
					{
						if (!pss->bCGUnprojVel)
						{
							...
							if (g_infos[j].w_unproj.len2() * sqr(time_interval) <
							    sqr(0.003f))
							{
								g_pBodies[i]->q +=
								    quaternionf(0, g_infos[j].w_unproj * 0.5f) *
								    g_pBodies[i]->q * time_interval;
							}
							else
							{
								...
								g_pBodies[i]->q =
								    Quat::CreateRotationAA(wlen * time_interval,
								                           g_infos[j].w_unproj / wlen) *
								    g_pBodies[i]->q;
							}
							g_pBodies[i]->q.Normalize();
							// don't unpdate Iinv and w, no1 will appreciate it after this
							// point
						}
						else
						{
							...
							g_pBodies[i]->L =
							    g_pBodies[i]->q * (g_pBodies[i]->Ibody *
							                       (!g_pBodies[i]->q * g_pBodies[i]->w));
						}
					}
					g_pBodies[i]->Fcollision = g_infos[j].Fcollision;
					g_pBodies[i]->Tcollision = g_infos[j].Tcollision;
