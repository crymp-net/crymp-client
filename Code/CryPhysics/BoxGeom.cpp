...
CBoxGeom* CBoxGeom::CreateBox(primitives::box* pbox)
{
...
int CBoxGeom::CalcPhysicalProperties(phys_geometry* pgeom)
{
	...
	pgeom->V = m_box.size.GetVolume() * 8;
...
{
	Vec3 ptloc = pt - m_box.center;
	if (m_box.bOriented)
	{
		ptloc = m_box.Basis * ptloc;
	}
	return isneg(
	    max(max(fabsf(ptloc.x) - m_box.size.x, fabsf(ptloc.y) - m_box.size.y), fabsf(ptloc.z) - m_box.size.z));
}

int CBoxGeom::PrepareForIntersectionTest(geometry_under_test* pGTest, CGeometry* pCollider,
                                         geometry_under_test* pGTestColl, bool bKeepPrevContacts)
{
	...
	pGTest->primbuf = pGTest->primbuf1 = g_idata[pGTest->iCaller].BoxBuf + g_idata[pGTest->iCaller].BoxBufPos++;
	pGTest->szprimbuf = 1;
	pGTest->typeprim = primitives::box::type;
	pGTest->szprim = sizeof(primitives::box);
	pGTest->idbuf = g_idata[pGTest->iCaller].BoxIdBuf;
	pGTest->surfaces = g_idata[pGTest->iCaller].BoxSurfaceBuf;
	pGTest->edges = g_idata[pGTest->iCaller].BoxEdgeBuf;

	pGTest->minAreaEdge = 0;
	return 1;
}

void CBoxGeom::PrepareBox(primitives::box* pbox, geometry_under_test* pGTest)
{
...
int CBoxGeom::PreparePrimitive(geom_world_data* pgwd, primitives::primitive*& pprim, int iCaller)
{
	primitives::box* pbox = g_idata[iCaller].BoxBuf + g_idata[iCaller].BoxBufPos;
	g_idata[iCaller].BoxBufPos = (g_idata[iCaller].BoxBufPos + 1) &
	                             ((sizeof(g_idata[iCaller].BoxBuf) / sizeof(g_idata[iCaller].BoxBuf[0])) - 1);
...
int CBoxGeom::GetPrimitiveList(int iStart, int nPrims, int typeCollider, primitives::primitive* pCollider,
                               int bColliderLocal, geometry_under_test* pGTest, geometry_under_test* pGTestOp,
                               primitives::primitive* pRes, char* pResId)
{
	PrepareBox((primitives::box*)pRes, pGTest);
	primitives::box* pbox = (primitives::box*)pRes;
	float szmax = max(max(pbox->size.x, pbox->size.y), pbox->size.z),
	      szmin = min(min(pbox->size.x, pbox->size.y), pbox->size.z);
...
int CBoxGeom::GetUnprojectionCandidates(int iop, const contact* pcontact, primitives::primitive*& pprim,
                                        int*& piFeature, geometry_under_test* pGTest)
{
	if (pGTest->bTransformUpdated)
	{
		pprim = pGTest->primbuf1;
		PrepareBox((primitives::box*)pprim, pGTest);
	}
	primitives::box* pbox = (primitives::box*)pprim;
	...
	{ // check if the point on the face is close to some edge
		iz = iFeature >> 1 & 3;
		ix = inc_mod3[iz];
		iy = dec_mod3[iz];
	...
	if ((iFeature & 0xE0) == 0x20)
	{ // check if the point on the edge is close to one of its ends
		iz = iFeature >> 2 & 3;
		ix = inc_mod3[iz];
		iy = dec_mod3[iz];
	...
	else
	{
		for (iz = 0; iz < 3; iz++)
		{
			...
			pGTest->edges[iz].dir = pGTest->surfaces[iz].n =
			    pbox->Basis.GetRow(iz) * (((iFeature >> iz) << 1 & 2) - 1);
...
int CBoxGeom::PreparePolygon(primitives::coord_plane* psurface, int iPrim, int iFeature, geometry_under_test* pGTest,
                             vector2df*& ptbuf, int*& pVtxIdBuf, int*& pEdgeIdBuf)
{
	...
	for (i = 0; i < 4; i++)
	{
		...
		pt[ic.z] = 0;
		pt = center + R * pt;
		g_idata[pGTest->iCaller].BoxCont[i ^ sg].set(pt * psurface->axes[0], pt * psurface->axes[1]);
		g_idata[pGTest->iCaller].BoxEdgeId[((i ^ sg) - (sg & 1)) & 3] =
		    0x20 | ic[i & 1] << 2 | (iFeature & 1) << (i & 1) | (((i ^ i >> 1) & 1) ^ 1) << ((i & 1) ^ 1);
		g_idata[pGTest->iCaller].BoxVtxId[i ^ sg] =
		    (iFeature & 1) << ic.z | ((i ^ i >> 1) & 1) << ic.x | (i >> 1 & 1) << ic.y;
	}
	g_idata[pGTest->iCaller].BoxCont[4] = g_idata[pGTest->iCaller].BoxCont[0];

	ptbuf = g_idata[pGTest->iCaller].BoxCont;
	pVtxIdBuf = g_idata[pGTest->iCaller].BoxVtxId;
	pEdgeIdBuf = g_idata[pGTest->iCaller].BoxEdgeId;
	return 4;
}

int CBoxGeom::PreparePolyline(primitives::coord_plane* psurface, int iPrim, int iFeature, geometry_under_test* pGTest,
                              vector2df*& ptbuf, int*& pVtxIdBuf, int*& pEdgeIdBuf)
{
	int ix, iy, iz;
	Vec3 pt, offs;
	...
	g_idata[pGTest->iCaller].BoxCont[0].set(pt * psurface->axes[0], pt * psurface->axes[1]);
	offs[ix] = offs[iy] = 0;
	offs[iz] = m_box.size[iz] * 2;
	pt += R * offs * pGTest->scale;
	g_idata[pGTest->iCaller].BoxCont[1].set(pt * psurface->axes[0], pt * psurface->axes[1]);
	g_idata[pGTest->iCaller].BoxEdgeId[0] = iFeature;
	g_idata[pGTest->iCaller].BoxEdgeId[1] = iFeature;
	g_idata[pGTest->iCaller].BoxVtxId[0] = (iFeature & 1) << ix | (iFeature >> 1 & 1) << iy;
	g_idata[pGTest->iCaller].BoxVtxId[1] = g_idata[pGTest->iCaller].BoxVtxId[0] | 1 << iz;
	g_idata[pGTest->iCaller].BoxCont[2] = g_idata[pGTest->iCaller].BoxCont[0];
                               ...
                               Vec3* ptres, int nMaxIters)
{
	...
	if (bLine)
	{
		...
		for (iz = 0; iz < 3; iz++)
		{
			...
			for (iedge = 0, mindistPt = 1E20f, isg = 0; iedge < 4; iedge++)
			{
				...
				dist =
				    ((ptbox[ix] - ptloc[0][ix]) * diri[iy]) - ((ptbox[iy] - ptloc[0][iy]) * diri[ix]);
				isg += sgnnz(dist);
			...
			{ // for(iedge=0;iedge<4;iedge++) {
				...
				if (inrange(t0, (float)0, n2) &
				    isneg(max(fabs_tpl(t1) - (n2 * size[iz]),
				              (fabs_tpl((ptbox - ptloc[0]) * n) * mindist.y) - (mindist.x * n2))))
				{
					iFeature = 0x20 | iz << 2 | iedge;
					rn2 = 1.0f / n2;
					ptbox[iz] = t1 * rn2;
					ptres[0] =
					    pgwd->R * (ptbox * m_box.Basis + m_box.center * pgwd->scale) + pgwd->offset;
					ptres[1] = pgwd->R * ((ptloc[0] + diri * (t0 * rn2)) * m_box.Basis +
					                      m_box.center * pgwd->scale) +
					           pgwd->offset;
		...
		for (ivtx = 0; ivtx < 8; ivtx++)
		{
			...
			if (inrange(t0 = (ptbox - ptloc[0]) * diri, (float)0, dir2))
			{
				...
				{
					ptres[1] = pgwd->R * (ptres[1] * m_box.Basis + m_box.center * pgwd->scale) +
					           pgwd->offset;
					return 1;
				}
			}
	...
	for (i = 0; i <= bLine; i++)
	{
		...
		if (nFaces[i] == 0)
		{
			const Vec3 diffSize = size - ptloc[i].abs();
			iz = idxmin3((const float*)&diffSize);
...
{
	static int idx[36] = {0, 4, 6, 6, 2, 0, 1, 3, 7, 7, 5, 1, 0, 1, 5, 5, 4, 0,
	                      2, 6, 7, 7, 3, 2, 0, 2, 3, 3, 1, 0, 4, 5, 7, 7, 6, 4};
	static Vec3 normals[12], vtx[8];
	...
	for (i = 0; i < 8; i++)
	{
		vtx[i].Set(m_box.size.x * ((i << 1 & 2) - 1), m_box.size.y * ((i & 2) - 1),
		           m_box.size.z * ((i >> 1 & 2) - 1));
		if (m_box.bOriented)
		{
...
int CBoxGeom::UnprojectSphere(Vec3 center, float r, float rsep, contact* pcontact)
{
	...
	sph.center = center;
	sph.r = rsep;
	if (!box_sphere_overlap_check(&m_box, &sph, nullptr))
	{
		return 0;
	}
...
float CBoxGeom::CalculateBuoyancy(const primitives::plane* pplane, const geom_world_data* pgwd, Vec3& massCenter)
{
	CTriMesh mesh;
	BuildTriMesh(mesh);
...
void CBoxGeom::CalculateMediumResistance(const primitives::plane* pplane, const geom_world_data* pgwd, Vec3& dPres,
                                         Vec3& dLres)
{
	int i, j, ix, iy, iz;
	Vec3 pt[4], n, size = m_box.size * pgwd->scale, offset = pgwd->R * m_box.center * pgwd->scale + pgwd->offset;
	Matrix33 R;
	if (!m_box.bOriented)
	{
		R = pgwd->R;
...
int CBoxGeom::DrawToOcclusionCubemap(const geom_world_data* pgwd, int iStartPrim, int nPrims, int iPass, int* pGrid[6],
