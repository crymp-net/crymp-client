...
struct featherstone_data
{
	...
	float qinv_down[9];
	float s[18];
	alignas(16) float Ia[6][6];
	float Ia_s[18];
	alignas(16) float Ia_s_qinv_sT[6][6];
	alignas(16) float s_qinv_sT[6][6];
	alignas(16) float s_qinv_sT_Ia[6][6];
	float qinv_sT[3][6];
	float qinv_sT_Ia[3][6];
	alignas(16) float Iinv[6][6];
};
...
CArticulatedEntity::CArticulatedEntity(CPhysicalWorld* pWorld) : CRigidEntity(pWorld)
{
	...
	m_nCollLyingMode = 5;
	m_gravity.Set(0, 0, -7.5f);
	m_Emin = sqr(0.025f);
	m_damping = 0.1f;
	m_gravityLyingMode.Set(0, 0, -5.0f);
	m_dampingLyingMode = 0.5f;
	m_EminLyingMode = sqr(0.05f);
...
{
	if (pgeom && fabs_tpl(_params->mass) + fabs_tpl(_params->density) != 0 &&
	    (pgeom->V <= 0 || pgeom->Ibody.x < 0 || pgeom->Ibody.y < 0 || pgeom->Ibody.z < 0))
	{
		CryLogError("CArticulatedEntity::AddGeometry: (%s at %.1f,%.1f,%.1f) Trying to add bad geometry",
	...
	if ((res = CPhysicalEntity::AddGeometry(pgeom, params, id, 1)) < 0 ||
	    m_nParts == nParts && params->flags & geom_proxy)
	{
		return res;
	}
	...
	Vec3 bodypos = m_pos + m_qrot * (params->pos + params->q * pgeom->origin);
	...
	if (M > 0)
	{
		...
		{
			m_joints[i].body.Create(bodypos, pgeom->Ibody * sqr(params->scale) * cube(params->scale), bodyq,
			                        V, M, m_qrot * params->q, bodypos);
			m_joints[i].body.softness[0] = m_softness[0];
			m_joints[i].body.softness[1] = m_softness[1];
		}
		else
		{
			m_joints[i].body.Add(bodypos, pgeom->Ibody * sqr(params->scale) * cube(params->scale), bodyq, V,
			                     M);
		}
	}
...
void CArticulatedEntity::RemoveGeometry(int id, int bThreadSafe)
{
	...
	if (m_parts[i].mass > 0)
	{
		Vec3 bodypos = m_pos + m_qrot * (m_parts[i].pos + pgeom->origin);
...
void CArticulatedEntity::RecomputeMassDistribution(int ipart, int bMassChanged)
{
	...
	for (i = iStart; i <= iEnd; i++)
	{
		...
		if (m_parts[i].mass > 0)
		{
			if (ipart < 0)
			{
				if (m_body.M == 0)
				{
					m_body.Create(bodypos,
					              m_parts[i].pPhysGeom->Ibody * sqr(m_parts[i].scale) *
					                  cube(m_parts[i].scale),
					              bodyq, V, m_parts[i].mass, m_qrot, m_pos);
				}
				else
				{
					m_body.Add(bodypos,
			...
			{
				m_joints[m_infos[i].iJoint].body.Create(
				    bodypos,
				    m_parts[i].pPhysGeom->Ibody * sqr(m_parts[i].scale) * cube(m_parts[i].scale), bodyq,
				    V, m_parts[i].mass, m_qrot * m_parts[i].q, bodypos);
				m_joints[m_infos[i].iJoint].body.softness[0] = m_softness[0];
				m_joints[m_infos[i].iJoint].body.softness[1] = m_softness[1];
			}
			else
			{
				m_joints[m_infos[i].iJoint].body.Add(bodypos,
				                                     m_parts[i].pPhysGeom->Ibody *
				                                         sqr(m_parts[i].scale) * cube(m_parts[i].scale),
				                                     bodyq, V, m_parts[i].mass);
			}
		}
	}
	...
	if (ipart < 0)
	{
		...
		for (i = 0; i < m_nJoints; i++)
		{
			m_joints[i].body.P = (m_joints[i].body.v = m_joints[i].prev_v) * m_body.M;
			m_joints[i].body.L =
			    m_joints[i].body.q * (m_joints[i].body.Ibody *
			                          (!m_joints[i].body.q * (m_joints[i].body.w = m_joints[i].prev_w)));
			m_joints[i].Pext *= M0;
			m_joints[i].Lext *= M0;
...
int CArticulatedEntity::SetParams(pe_params* _params, int bThreadSafe)
{
	...
	if (CRigidEntity::SetParams(_params, 1))
	{
		if (_params->type == pe_params_pos::type_id)
		{
			...
			int i = -1;
			{
				...
				if ((prevq.v - m_qrot.v).len2() > 0 && (m_joints[0].quat0.v - m_qrot.v).len2() > 0)
				{
					if (m_nJoints > 0)
					{
						if (prevq.w == 1.0f)
						...
						for (i = 0; i < m_nJoints; i++)
						{
							if (m_joints[i].iParent < 0)
							{
								...
								{
									m_joints[i].pivot[0] =
									    (m_qrot * !prevq) * m_joints[i].pivot[0];
								}
							}
						}
						i = 0;
				...
				if (diff > sqr(0.003f) && m_body.M > 0 && !(m_flags & aef_recorded_physics) &&
				    !m_bGrounded && m_bCollisionResp && m_iLastLog > 0)
				{
					for (i = 0; i < m_nJoints; i++)
					{
		...
		{
			if (!is_unused(((pe_params_part*)_params)->pos) &&
			    !(m_nJoints == 0 || m_joints[0].flags & joint_rotate_pivot))
			{
				m_bPartPosForced = 1;
			}
			if (!m_pHost && ((pe_params_part*)_params)->bRecalcBBox && m_body.Minv == 0 && m_nParts)
			{
				CPhysicalEntity** pentlist;
				int nEnts = m_pWorld->GetEntitiesAround(
	...
	if (_params->type == pe_params_joint::type_id)
	{
		...
		{
			m_joints[op[1]].flags = params->flags | (m_joints[op[1]].flags & params->flags &
			                                         ((angle0_limit_reached | angle0_gimbal_locked) * 7));
			for (i = 0; i < 3; i++)
			{
		...
		if (!is_unused(params->pivot))
		{
			...
			{
				pivot = m_parts[m_joints[op[0]].iStartPart].q * pivot +
				        m_parts[m_joints[op[0]].iStartPart].pos;
			}
		...
		float rdt = !is_unused(params->ranimationTimeStep)
		                ? params->ranimationTimeStep
		...
		for (i = 0; i < 3; i++)
		{
			...
			{
				m_joints[op[1]].kd[i] = ((m_joints[op[1]].flags & angle0_auto_kd << i)
				                             ? 2.0f * sqrt_tpl(max(1E-10f, m_joints[op[1]].ks[i]))
				                             : 1.0f) *
				                        params->kd[i];
			}
			...
			if (!is_unused(params->qext[i]))
			{
				...
				{ // qext violates limits; adjust the limits
					...
					m_joints[op[1]].limits[isneg(fabs_tpl(diff[1]) - fabs_tpl(diff[0]))][i] =
					    m_joints[op[1]].qext[i];
				}
			}
		}
		if (params->pMtx0)
		...
		if (m_joints[op[1]].iParent != op[0])
		{
			...
			{
				m_joints[op[1]].q =
				    Ang3::GetAnglesXYZ(Matrix33(!(qparent * m_joints[op[1]].quat0) * qchild));
			...
			{
				int nChildren;
				for (nChildren = m_joints[op[1]].nChildren + 1, i = 1; i < nChildren;
				     nChildren += m_joints[op[1] + i++].nChildren)
					;
				...
				if (op[1] < op[0])
				{
					...
					memmove(m_joints + op[1], m_joints + op[1] + nChildren,
					        (op[0] - op[1] - nChildren + 1) * sizeof(ae_joint));
					op[0] -= nChildren;
				}
				else
				{
					...
					memmove(m_joints + op[0] + 1 + nChildren, m_joints + op[0] + 1,
					        (op[1] - op[0] - 1) * sizeof(ae_joint));
				}
...
int CArticulatedEntity::GetParams(pe_params* _params)
{
	...
	if (_params->type == pe_params_joint::type_id)
	{
		...
		params->pivot = (m_joints[i].body.pos +
		                 (m_joints[i].body.q * m_joints[i].body.qfb) * m_joints[i].pivot[1] - m_pos) *
		                m_qrot;
	...
	if (_status->type == pe_status_dynamics::type_id)
	{
		...
		if ((unsigned int)i >= (unsigned int)m_nParts)
		{
			...
			if (m_body.M > 0)
			{
				...
				status->centerOfMass /= status->mass;
			}
			else
			{
			...
			{
				status->energy += (m_joints[i].body.M * m_joints[i].body.v.len2()) +
				                  m_joints[i].body.L * m_joints[i].body.w;
			}
			status->energy *= 0.5f;
			return i;
		}
		...
		status->wa =
		    pbody->Iinv *
		    (m_joints[i].Tcollision -
		     (status->w ^ m_joints[i].prev_qrot * (pbody->Ibody * (m_joints[i].prev_qrot * status->w))));
...
int CArticulatedEntity::Action(pe_action* _action, int bThreadSafe)
{
	...
	if (_action->type == pe_action_impulse::type_id)
	{
		...
		if ((unsigned int)i >= (unsigned int)m_nParts)
		{
			...
			else
			{
				for (i = 0; i < m_nParts; i++)
				{
					...
					posloc =
					    (m_qrot * m_parts[i].q) * bbox.center + m_qrot * m_parts[i].pos + m_pos;
		...
		if ((unsigned int)i >= m_nParts || (unsigned int)(j = m_infos[i].iJoint) >= m_nJoints ||
		    m_joints[j].flags & joint_ignore_impulses)
		{
			return 0;
		}
		if (action->iSource == 0)
		...
		else if (!is_unused(action->point))
		{
			...
			posloc = bbox.Basis * ((action->point - posloc) * (m_qrot * m_parts[i].q));
			if (fabs_tpl(posloc.x) < bbox.size.x * 3 && fabs_tpl(posloc.y) < bbox.size.y * 3 &&
			    fabs_tpl(posloc.z) < bbox.size.z * 3)
			{
				// recompute body's com, since sometimes only parts are in sync with animation, not
				// joint bodies
				Vec3 bodypos =
				    m_parts[i].pos + m_pos - (m_parts[i].q * !m_infos[i].q0) * m_infos[i].pos0;
		...
		else
		{
			...
			m_joints[j].prev_w = m_joints[j].body.w =
			    (R * m_joints[j].body.Ibody_inv * R.T()) * (m_joints[j].body.L += L);
		}
	...
	if (_action->type == pe_action_set_velocity::type_id)
	{
		...
		if (!is_unused(action->ipart) || !is_unused(action->partid))
		{
			...
			if (!is_unused(action->w))
			{
				m_joints[i].body.L =
				    m_joints[i].body.q *
				    (m_joints[i].body.Ibody * (!m_joints[i].body.q * (m_joints[i].body.w = action->w)));
			}
			...
			if (m_joints[i].body.w.len2() > sqr(m_maxw))
			{
				...
				m_joints[i].body.L = m_joints[i].body.q * (m_joints[i].body.Ibody *
				                                           (!m_joints[i].body.q * m_joints[i].body.w));
			}
			if ((m_joints[i].bAwake = m_joints[i].body.v.len2() + m_joints[i].body.w.len2() > 0) &&
			    !m_bAwake)
			{
				if (m_body.Minv == 0)
				{
...
int CArticulatedEntity::IsChildOf(int idx, int iParent)
{
	...
	if (m_pHost)
	{
		...
		else
		{
			for (i = 0; i < m_nJoints; i++)
			{
				...
				m_joints[i].quat = m_parts[ipart].q * !m_infos[ipart].q0;
				m_joints[i].body.q = pp.q * m_joints[i].quat * !m_joints[i].body.qfb;
				m_joints[i].body.pos =
				    pp.q * (m_parts[ipart].pos - m_joints[i].quat * m_infos[ipart].pos0) + m_pos;
			}
		}
		ComputeBBox(m_BBoxNew);
		UpdatePosition(m_pWorld->RepositionEntity(this, 1, m_BBoxNew));
...
void CArticulatedEntity::UpdateJointRotationAxes(int idx)
{
	...
	cosa = cos_tpl(angle);
	sina = sin_tpl(angle);
	m_joints[idx].rotaxes[0] =
	    m_joints[idx].rotaxes[0].GetRotated(m_joints[idx].rotaxes[1], cosa, sina); // rot x around y - pitch
	angle = m_joints[idx].q[2] + m_joints[idx].qext[2];
	cosa = cos_tpl(angle);
	sina = sin_tpl(angle);
	m_joints[idx].rotaxes[0] =
	    m_joints[idx].rotaxes[0].GetRotated(m_joints[idx].rotaxes[2], cosa, sina); // rot x around z - yaw
	m_joints[idx].rotaxes[1] =
	    m_joints[idx].rotaxes[1].GetRotated(m_joints[idx].rotaxes[2], cosa, sina); // rot y around z
}
...
void CArticulatedEntity::CheckForGimbalLock(int idx)
{
	...
	if (!(m_joints[idx].flags & (angle0_locked * 5)) &&
	    sqr(m_joints[idx].rotaxes[0] * m_joints[idx].rotaxes[2]) > 0.999f * 0.999f)
	{
		if (!(m_joints[idx].flags & (angle0_locked * 7)) &&
		    cry_fabsf(m_joints[idx].limits[1][0] - m_joints[idx].limits[0][0]) > 10 &&
		    fabsf(m_joints[idx].limits[1][1] - m_joints[idx].limits[0][1]) > 10 &&
		    fabsf(m_joints[idx].limits[1][2] - m_joints[idx].limits[0][2]) > 10)
		{ // if joint is 3dof w/o limits, just rotate its quat0 to avoid gimbal lock
			...
			m_joints[idx].quat0 *= Quat::CreateRotationAA(
			    (float)g_PI / 6,
			    Vec3(0, 1, 0)); //(m_joints[idx].rotaxes[0]+m_joints[idx].rotaxes[1])*(1.0f/sqrt2));
			m_joints[idx].bQuat0Changed = 1;
			SyncJointWithBody(idx, 3);
		}
		else
		{
			i = iszero((int)m_joints[idx].flags & angle0_limit_reached)
			    << 1; // gimbal lock angle that already reached its limit, if this is the case
			m_joints[idx].flags |= angle0_gimbal_locked << i;
			m_joints[idx].dq[i ^ 2] += m_joints[idx].dq[i];
			m_joints[idx].dq[i] = 0;
			m_joints[idx].rotaxes[i] =
			    m_joints[idx].rotaxes[i ^ 2] * 0.99f + m_joints[idx].rotaxes[1] * 0.01f;
		}
	}
}
...
void CArticulatedEntity::SyncBodyWithJoint(int idx, int flags)
{
	...
	{ // sync geometry
		...
		//	m_joints[idx].quat =
		// q_parent*m_joints[idx].quat0*quaternionf(m_joints[idx].q+m_joints[idx].qext);
		m_joints[idx].quat =
		    q_parent * m_joints[idx].quat0 * Quat::CreateRotationXYZ(m_joints[idx].q + m_joints[idx].qext);
		m_joints[idx].body.q = m_joints[idx].quat * !m_joints[idx].body.qfb;
		m_joints[idx].body.pos =
		    pos_parent + q_parent * m_joints[idx].pivot[0] - m_joints[idx].quat * m_joints[idx].pivot[1];
	...
	{ // sync velocities
		if (m_joints[idx].iParent >= 0)
		{
			...
			m_joints[idx].body.v = m_joints[m_joints[idx].iParent].body.v +
			                       (m_joints[m_joints[idx].iParent].body.w ^
			                        m_joints[idx].body.pos - m_joints[m_joints[idx].iParent].body.pos);
		}
		else
		{
		...
		{
			wpivot += m_joints[idx].rotaxes[i] *
			          (m_joints[idx].dq[i] + (m_joints[idx].dqext[i] * (m_bCollisionResp ^ 1)));
		}
	...
	{ // sync velocities
		...
		else
		{
			...
			else if (j == 2)
			{
				...
				{
					// only x and z axes can be nonorthogonal, so just project wrel on active axes
					// in other cases
					m_joints[idx].dq[iAxes[0]] = m_joints[idx].rotaxes[iAxes[0]] * wrel;
				...
				{
					Vec3 axisz, axisy;
					axisz = (m_joints[idx].rotaxes[iAxes[0]] ^ m_joints[idx].rotaxes[iAxes[1]])
					            .normalized();
					axisy = axisz ^ m_joints[idx].rotaxes[iAxes[0]];
					m_joints[idx].dq[iAxes[1]] =
					    (axisy * wrel) / (axisy * m_joints[idx].rotaxes[iAxes[1]]);
					m_joints[idx].dq[iAxes[0]] =
					    wrel * m_joints[idx].rotaxes[iAxes[0]] -
					    ((m_joints[idx].rotaxes[iAxes[0]] * m_joints[idx].rotaxes[iAxes[1]]) *
					     m_joints[idx].dq[iAxes[1]]);
				}
			}
		}
...
float CArticulatedEntity::CalcEnergy(float time_interval)
{
	...
	if (time_interval > 0)
	{
		for (pContact = m_pContactStart; pContact != CONTACT_END(m_pContactStart); pContact = pContact->next)
		{
			...
			m_joints[m_infos[pContact->ipart[0]].iJoint].ddq +=
			    pContact->n * max(0.0f, max(0.0f, pContact->vreq * pContact->n -
			...
			if (pContact->pbody[1]->Minv == 0 && pContact->pent[1]->m_iSimClass > 1)
			{
				...
				Emax = m_body.M * sqr(10.0f) *
				       2; // since we'll allow some extra energy, make sure we restrict the upper limit
			}
		}
	}
	...
	for (i = 0; i < m_nJoints; i++)
	{
		E += (m_joints[i].body.M * (m_joints[i].body.v + m_joints[i].ddq).len2()) +
		     m_joints[i].body.L * m_joints[i].body.w;
	}
		...
		for (i = 0; i < NMASKBITS && getmask(i) <= constraint_mask; i++)
		{
			                                     ...
			                                     m_pConstraintInfos[i].flags & constraint_rope))
			{
				v += m_pConstraints[i].n *
				     max(0.0f, max(0.0f, m_pConstraints[i].vreq * m_pConstraints[i].n -
				                             max(0.0f, m_pConstraints[i].vrel * m_pConstraints[i].n)) -
				...
				if (m_pConstraints[i].pbody[1]->Minv == 0 && m_pConstraints[i].pent[1]->m_iSimClass > 1)
				{
					...
					vmax =
					    max(vmax,
					        (pbody->v + (pbody->w ^ pbody->pos - m_pConstraints[i].pt[1])).len2());
				}
			}
		}
...
void CArticulatedEntity::GetSleepSpeedChange(int ipart, Vec3& v, Vec3& w)
{
	int i = m_infos[ipart].iJoint;
	v = m_joints[i].vSleep;
	w = m_joints[i].wSleep;
}
...
{
	axis.Set(0, 0, 0);
	if (!(m_joints[idx].flags &
	      ((angle0_limit_reached | angle0_locked | angle0_gimbal_locked) * 6))) // y and z axes are not locked
	{
		return 1;
	}
	else
	{
		int i;
		for (i = 2;
		     i >= 0 && m_joints[idx].flags & (angle0_limit_reached | angle0_locked | angle0_gimbal_locked) << i;
		     i--)
			;
		if (i >= 0)
		{
...
int CArticulatedEntity::StepJoint(int idx, float time_interval, int& bBounced, int bFlying)
{
	int i, j, idxpivot, ncont, itmax, curidx, sgq, bSelfColl, bFreeFall = isneg(-m_nBodyContacts) ^ 1;
	float qlim[2], dq, curq, e, diff[2], tdiff,
	    minEnergy = m_nBodyContacts >= m_nCollLyingMode ? m_EminLyingMode : m_Emin;
	Ang3 q;
	...
	{
		e = ((m_joints[idx].body.v.len2() +
		      (m_joints[idx].body.L * m_joints[idx].body.w) * m_joints[idx].body.Minv) *
		     0.5f) +
		    m_joints[idx].body.Eunproj;
			...
			if (!(m_joints[idx].flags & (angle0_locked | angle0_gimbal_locked) << i))
			{
				...
				else
				{
					m_joints[idx].flags |=
					    ...
					    -isneg(min(fabs_tpl(qlim[0] - q[i]), fabs_tpl(qlim[1] - q[i])) -
					           m_joints[idx].qdashpot[i]);
				}
			}
		}
	...
	else
	{
		for (i = 0; i < 3; i++)
		{
			if (!(m_joints[idx].flags & (angle0_locked | angle0_gimbal_locked) << i))
			{
				...
				{
					m_joints[idx].flags &= ~(angle0_limit_reached << i);
					// if angle passes through 0 during this step, make it snap to 0 and also clamp
					// dq if it's small enough
					if (m_joints[idx].ks[i] != 0 &&
					    (m_joints[idx].q[i] + dq) * m_joints[idx].q[i] < 0)
					{
						dq = -m_joints[idx].q[i];
	...
	pivot = m_joints[idx].iParent < 0 ? m_posPivot
	                                  : m_joints[m_joints[idx].iParent].body.pos +
	                                        m_joints[m_joints[idx].iParent].quat * m_joints[idx].pivot[0];
	...
	if (m_bCheckCollisions)
	{
		...
		else if (m_iSimTypeCur == 0 && m_joints[idx].nChildren == 0 &&
		         (m_joints[idx].body.w.len2() > sqr(6) || m_joints[idx].body.v.len2() > sqr(4)) &&
		...
		if (itmax >= 0)
		{
			if (pcontacts[itmax].iUnprojMode)
			{
				...
				if (!m_bCollisionResp)
				{
					for (i = 0; i < 3; i++)
					{
						if (!(m_joints[idxpivot].flags &
						      (angle0_locked | angle0_gimbal_locked | angle0_limit_reached)
						          << i) &&
						    m_joints[idxpivot].rotaxes[i] * pcontacts[itmax].dir *
						            sgnnz(m_joints[idxpivot].dq[i]) <
						        -0.5f)
						{
		...
		if (m_bCollisionResp)
		{
			for (j = m_joints[idx].iStartPart + m_joints[idx].nParts - 1; j >= m_joints[idx].iStartPart;
			     j--)
			{
				...
				m_infos[j].BBox[0] = m_infos[j].BBox[1] =
				    m_posNew +
				    m_qNew * (m_infos[j].pos + m_infos[j].q * bbox.center * m_parts[j].scale);
				m_infos[j].BBox[0] -= sz;
				m_infos[j].BBox[1] += sz;
			}
			j++;
			if (m_nParts > 3)
			{
				m_parts[j].minContactDist =
				    max(max(bbox.size.x, bbox.size.y), bbox.size.z) * m_parts[j].scale * 0.4f;
			}
			...
			for (i = 0; i < ncont; i++)
			{
				if (pcontacts[i].vel == 0)
				{
					...
					if (idx > 0)
					{
						for (; j < pcontacts[i].nborderpt &&
						       (pcontacts[i].ptborder[j] - pcontacts[i].ptborder[0]).len2() < e;
						     j++)
							;
					}
					if (j == pcontacts[i].nborderpt)
					{
						RegisterContactPoint(i, pcontacts[i].center, pcontacts,
						                     pcontacts[i].iPrim[0], pcontacts[i].iFeature[0],
						                     pcontacts[i].iPrim[1], pcontacts[i].iFeature[1],
						                     contact_new | contact_last, pcontacts[i].t);
					}
					else
					{
						sgq = contact_last;
				...
				{
					e = sqr(max(max(bbox.size.x, bbox.size.y), bbox.size.z) * 0.8f *
					        (iszero(idx) ^ 1));
					prevpt.zero();
					sgq = contact_last;
					for (j = 0; j < pcontacts[i].parea->npt; j++)
					{
						...
						{
							RegisterContactPoint(i, pcontacts[i].parea->pt[j], pcontacts,
				...
				{
					RegisterContactPoint(i, pcontacts[i].pt, pcontacts, pcontacts[i].iPrim[0],
	...
	{
		curidx = StepJoint(curidx, time_interval, bBounced, bFlying);
	}
	return curidx;
}
...
int CArticulatedEntity::Step(float time_interval)
{
	int i, j, bBounced = 0;
	Vec3 Y_vec[2];
	vectornf Y(6, static_cast<float*>(Y_vec[0]));
	float Iabuf[39];
	matrixf Ia(6, 6, 0, align16(Iabuf));
	Vec3 Za_vec[2];
	vectornf Za(6, static_cast<float*>(Za_vec[0]));
	Vec3 dv, dw;
	Vec3 gravity;
	...
	if (m_body.M == 0 || m_flags & aef_recorded_physics)
	{
		if (!m_pHost)
		{
			for (dv.x = 0, i = 0; i < m_nJoints; i++)
			{
			...
			if (m_pStructure && m_pStructure->autoDetachmentDist > 0)
			{
				...
				{
					if ((m_joints[m_infos[m_pStructure->pJoints[i].ipart[0]].iJoint].body.pos -
					     m_pos)
					        .len2() > sqr(m_pStructure->autoDetachmentDist))
					{
		...
		m_nCollEnts = m_pWorld->GetEntitiesAround(
		    m_BBox[0], m_BBox[1], m_pCollEntList,
		    ent_sleeping_rigid | ent_rigid | ent_living | ent_independent | ent_triggers, this);
	...
	if (!m_bAwake && !m_bCheckCollisions || m_nRoots > 1 || (!m_bCheckCollisions && !m_bGrounded) ||
	    m_flags & aef_recorded_physics)
	{
		return 1;
	}
	...
	if (m_bCheckCollisions)
	{
		...
		m_nCollEnts =
		    m_pWorld->GetEntitiesAround(m_BBox[0] - sz, m_BBox[1] + sz, m_pCollEntList,
		                                ent_terrain | ent_static | ent_sleeping_rigid | ent_rigid | ent_living |
		                                    ent_independent | ent_sort_by_mass | ent_triggers,
		                                this);
		...
		for (i = 0; i < m_nCollEnts; i++)
		{
			if (m_pCollEntList[i]->m_iSimClass > 2)
			{
				if (m_pCollEntList[i]->GetType() != PE_ARTICULATED)
				{
					...
					{
						m_pWorld->ScheduleForStep(m_pCollEntList[i]), m_nNonRigidNeighbours++;
					}
				}
			...
			for (j = 0; j < 3; j++)
			{
				if (!(m_joints[i].flags & angle0_locked << j) &&
				    isneg(m_joints[i].limits[0][j] - m_joints[i].qext[j]) +
				            ...
				            isneg(m_joints[i].limits[1][j] - m_joints[i].limits[1][j]) <
				        2)
				{ // qext violates limits; adjust the limits
					float diff[2];
					...
					diff[0] -= (int)(diff[0] / (2 * g_PI)) * 2 * g_PI;
					diff[1] -= (int)(diff[1] / (2 * g_PI)) * 2 * g_PI;
					m_joints[i].limits[isneg(fabs_tpl(diff[1]) - fabs_tpl(diff[0]))][j] =
					    m_joints[i].qext[j];
				}
			}
		}
	...
	m_maxPenetrationCur = 0;
	StepJoint(0, time_interval, bBounced, iszero(m_nBodyContacts));
	...
	if (!m_joints[0].fs)
	{
		...
		{
			m_joints[i].fs =
			    (featherstone_data*)align16(m_joints[i].fsbuf = new char[sizeof(featherstone_data) + 16]);
			m_joints[i].fs->Q.zero();
			m_joints[i].fs->Ya_vec[0].zero();
			m_joints[i].fs->Ya_vec[1].zero();
			IdentityBasis((Vec3*)m_joints[i].fs->qinv);
		}
	}

	if (bBounced && m_bIaReady)
	{ // apply bounce impulses at joints that reached their limits
		bBounced = 0;
	...
	for (i = 0; i < m_nJoints; i++)
	{
		SyncBodyWithJoint(
		    i,
		    3); // synchronize body geometry as well as dynamics to accomodate potential changes in joint limits
	}
...
int CArticulatedEntity::GetStateSnapshot(TSerialize ser, float time_back, int flags)
{
	...
	// ser.Value("vel", m_body.v, NSerPolicy::AC_Velocity(20, 10));
	...
	if (ser.GetSerializationTarget() != eST_Network)
	{
		...
		for (int i = 0; i < m_nJoints; i++)
		{
			...
			ser.Value("q", (Vec3&)m_joints[i].q);
			if (m_bAwake)
			{
				ser.Value("dq", m_joints[i].dq);
			}
			ser.Value("qext", (Vec3&)m_joints[i].qext);
...
int CArticulatedEntity::CalcBodyZa(int idx, float time_interval, vectornf& Za_change)
{
	...
	vectornf Q(3, static_cast<float*>(m_joints[idx].fs->Q));
	vectornf Qtmp(3, qbuf);
	...
	for (j = 0; j < m_joints[idx].nPotentialAngles; j++)
	{
		...
		// if the joint is approaching limit (but haven't breached it yet) and is in dashpot area, damp the
		// velocity
		if (fabsf(m_joints[idx].dq[i]) > 0.5f)
		{
			...
			dq_dashpot = min(fabsf(m_joints[idx].dq[i]) * Qscale[j],
			                 (qdashpot - tlim) * m_joints[idx].kdashpot[i] *
			                     isneg(fabsf(tlim - (2 * qdashpot)) - qdashpot) * k);
			Q[j] += dq_dashpot * (1 - idir * 2);
		}
	}
	if (!(m_joints[idx].flags & ((angle0_locked | angle0_gimbal_locked) * 5)))
	{ // take into accout that x and z axes influence one another
		Q[m_joints[idx].fs->qidx2axidx[0]] +=
		    Q[m_joints[idx].fs->qidx2axidx[2]] * (m_joints[idx].rotaxes[0] * m_joints[idx].rotaxes[2]);
		Q[m_joints[idx].fs->qidx2axidx[2]] +=
		    Q[m_joints[idx].fs->qidx2axidx[0]] * (m_joints[idx].rotaxes[0] * m_joints[idx].rotaxes[2]);
	}
	if (m_joints[idx].flags & joint_isolated_accelerations)
	{
	...
	Vec3 Za_child_vec[2];
	vectornf Za_child(6, static_cast<float*>(Za_child_vec[0]));
	...
	Za_change = Za;
	vectornf ddq(m_joints[idx].nActiveAngles, static_cast<float*>(m_joints[idx].ddq));
	if (m_joints[idx].nActiveAngles > 0)
	{
		matrixf sT(Q.len = m_joints[idx].nActiveAngles, 6, 0,
		           static_cast<float*>(m_joints[idx].fs->s_vec[0][0]));
		matrixf qinv_down(m_joints[idx].nActiveAngles, m_joints[idx].nActiveAngles, 0,
		                  m_joints[idx].fs->qinv_down);
	...
	if (m_joints[idx].nActiveAngles < m_joints[idx].nPotentialAngles &&
	    !(m_joints[idx].flags & joint_isolated_accelerations))
	{
		matrixf sT(Q.len = m_joints[idx].nPotentialAngles, 6, 0,
		           static_cast<float*>(m_joints[idx].fs->s_vec[0][0]));
...
int CArticulatedEntity::CalcBodyIa(int idx, matrixf& Ia_change)
{
	...
	float buf[39];
	matrixf Ia_child(6, 6, 0, align16(buf));
	...
	Ia_change = Ia;
	matrixf sT(m_joints[idx].nActiveAngles, 6, 0, static_cast<float*>(m_joints[idx].fs->s_vec[0][0]));
...
int CArticulatedEntity::CollectPendingImpulses(int idx, int& bNotZero)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		{
			// note that articulation matrices from the previous frame are used, while rotaxes are already
			// new; this introduces slight inconsistency
			...
			matrixf Mresp(6, 6, 0, align16(buf)), MrespLoc(6, 6, 0, align16(buf1));
			...
			LeftOffsetSpatialMatrix(Mresp,
			                        m_joints[m_joints[idx].iParent].body.pos - m_joints[idx].body.pos);
			...
			float resp = -(Vec3::FromPtr(m_joints[idx].fs->qinv_sT[j] + 3) * axis);
			resp -= Vec3::FromPtr(m_joints[idx].fs->qinv_sT_Ia[j]) * dw +
			        Vec3::FromPtr(m_joints[idx].fs->qinv_sT_Ia[j] + 3) * dv;
			...
			if (resp > 0.1f * m_body.Minv)
			{
				...
				if (m_joints[idx].iParent >= 0)
				{
					m_joints[m_joints[idx].iParent].fs->Ya_vec[1] +=
					    m_joints[idx].rotaxes[i] * resp;
				}
				bNotZero++;
			}
		}
	...
	for (i = 0, curidx = idx + 1; i < m_joints[idx].nChildren; i++)
	{
		...
		if (bChildNotZero)
		{
			...
			vectornf Ya(6, static_cast<float*>(m_joints[idx].fs->Ya_vec[0]));
			vectornf Ya_child(6, static_cast<float*>(m_joints[curidx].fs->Ya_vec[0]));
	...
	if (idx == 0)
	{
		vectorn_tpl<float> tmp(6, static_cast<float*>(m_joints[0].fs->Ya_vec[0]));
		vectornf(6, (float*)(m_Ya_vec[0])) = operator*(
		    (const matrix_tpl<float>&)matrixf(6, 6, 0, m_joints[0].fs->Ia_s_qinv_sT[0]), tmp);
	}
	return curidx;
}
...
void CArticulatedEntity::PropagateImpulses(const Vec3& dv, int bLockLimits)
{
	...
	{
		vectornf dv_spat(6, static_cast<float*>(m_joints[idx].fs->dv_vec[0]));
		if (m_joints[idx].iParent >= 0)
		{
		...
		if (m_joints[idx].nPotentialAngles > 0)
		{
			vectornf ddq(m_joints[idx].nPotentialAngles, static_cast<float*>(m_joints[idx].ddq));
			matrixf qinv_sT_Ia(m_joints[idx].nPotentialAngles, 6, 0, m_joints[idx].fs->qinv_sT_Ia[0]);
			matrixf qinv_sT(m_joints[idx].nPotentialAngles, 6, 0, m_joints[idx].fs->qinv_sT[0]);
			vectornf Ya(6, static_cast<float*>(m_joints[idx].fs->Ya_vec[0]));
			...
			for (i = 0; i < m_joints[idx].nPotentialAngles; i++)
			{
				...
				else if (m_joints[idx].flags & angle0_limit_reached << j)
				{
					...
					{
						m_joints[idx].ddq[i] =
						    0; // do not accelerate angle that reached its limit
					}
					if (!bHitsLimit && bLockLimits)
					{
...
void CArticulatedEntity::CalcVelocityChanges(float time_interval, const Vec3& dv, const Vec3& dw)
{
	...
	for (idx = 0; idx < m_nJoints; idx++)
	{
		...
		if (m_joints[idx].nPotentialAngles > 0)
		{
			...
			{
				j = m_joints[idx].fs->axidx2qidx[i];
				if (m_joints[idx].flags & angle0_limit_reached << j &&
				    m_joints[idx].ddq[i] * m_joints[idx].dq_limit[j] > 0)
				{
...
void CArticulatedEntity::CalcBodiesIinv(int bLockLimits)
{
	...
	Vec3 r, pos_parent;
	matrixf Iinv_parent(6, 6, 0, align16(Iinv_buf)), Iaux(6, 6, 0, align16(aux_buf));
	...
	for (idx = 0; idx < m_nJoints; idx++)
	{
		...
		{
			Iinv_parent.data = align16(Iinv_buf);
			pos_parent = m_posPivot;
		}
		...
		if (nAngles > 0)
		{
			matrixf Mright(6, 6, 0, align16(Mright_buf)), s(6, nAngles, 0, m_joints[idx].fs->s);
			matrixf Ia(6, 6, 0, m_joints[idx].fs->Ia[0]);
			...
			{
				matrixf sT(6, nAngles, 0, static_cast<float*>(m_joints[idx].fs->s_vec[0][0]));
				s.data = s_buf;
				SpatialTranspose(sT, s);
...
int CArticulatedEntity::RegisterContacts(float time_interval, int nMaxPlaneContacts)
{
	...
	for (pContact = m_pContactStart; pContact != CONTACT_END(m_pContactStart); pContact = pContact->next)
	{
		if (!(pContact->flags & contact_2b_verified))
		{
			...
			pContact->prevAux = m_joints[i].pContact->prevAux;
			pContact->nextAux = m_joints[i].pContact;
			m_joints[i].pContact->prevAux = pContact;
			m_joints[i].pContact = pContact;
		}
	}

	for (i = 0, constraints_mask = 0; i < m_nColliders; i++)
	{
	...
	for (idx = 0; idx < m_nJoints; idx++)
	{
		...
		for (pContact = m_joints[idx].pContact; pContact != CONTACT_END(m_joints[idx].pContact);
		     pContact = pContact->nextAux)
		{
		...
		for (i = 0; i < NMASKBITS && getmask(i) <= constraints_mask; i++)
		{
			if (constraints_mask & getmask(i) && m_pConstraintInfos[i].bActive &&
			    (unsigned int)(m_pConstraints[i].ipart[0] - m_joints[idx].iStartPart) <
			        (unsigned int)m_joints[idx].nParts)
			{
		...
		if (m_bGrounded || m_joints[idx].iParent >= 0)
		{
			...
			if (m_joints[idx].iParent >= 0)
			{
				...
				pivot[0] = m_joints[m_joints[idx].iParent].body.pos +
				           m_joints[m_joints[idx].iParent].quat * m_joints[idx].pivot[0];
			}
			else
			{
			...
			for (i = j = 0, ks = kd = 0; i < 3; i++)
			{
				...
				{
					iAxes[j++] = i;
					axisTens += m_joints[idx].rotaxes[i] *
					            (min(1.0f, max(-1.0f, m_joints[idx].q0[i] - m_joints[idx].q[i] -
					                                      m_joints[idx].qext[i])));
					kd = max(kd, m_joints[idx].kd[i]);
					kd = max(kd, m_joints[idx].kdashpot[i] * (1 ^ iszero((int)m_joints[idx].flags &
					                                                     joint_dashpot_reached)));
					ks = max(ks, m_joints[idx].ks[i]);
				}
				else
				{
					axisDrift += m_joints[idx].rotaxes[i] *
					             min(1.0f, m_joints[idx].q0[i] - m_joints[idx].q[i]);
				}
			}
			if (!(pContact = CreateConstraintContact(idx)))
			{
			...
			dL = pContact->K.GetInverted() *
			     (dw * min(0.0f, (time_interval * kd) - 1.0f) + axisTens * (time_interval * ks));
			...
			for (i = 0; i < 3; i++)
			{
				if (m_joints[idx].flags & angle0_limit_reached << i)
				{
					...
					{
						pContact->vreq =
						    pContact->n * min(5.0f, fabsf(m_joints[idx].dq_limit[i]) * 4.0f);
					}
					// pContact->vsep = -0.01f;
					RegisterContact(pContact);
				}
...
int CArticulatedEntity::Update(float time_interval, float damping)
{
	...
	if (m_body.M > 0)
	{
		for (i = 0; i < m_nJoints; i++)
		{
			...
			e = ((m_joints[i].body.v.len2() +
			      (m_joints[i].body.L * m_joints[i].body.w) * m_joints[i].body.Minv) *
			     0.5f) +
			    m_joints[i].body.Eunproj;
			...
			if (m_iSimTypeCur && m_joints[i].body.Eunproj > 0)
			{
				...
				for (j = m_joints[i].iStartPart; j < m_joints[i].iStartPart + m_joints[i].nParts; j++)
				{
					...
					m_infos[j].pos =
					    m_joints[i].quat * m_infos[j].pos0 + m_joints[i].body.pos - m_pos;
				}
				bPosChanged = 1;
			}
		}
		...
		m_timeIdle *=
		    isneg(-m_nColliders - m_nPrevColliders - m_submergedFraction) & m_pWorld->m_bCurGroupInvisible;
