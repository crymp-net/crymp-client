...
int WinAPI::InitializeCodeWallInternal() {
	HMODULE hKernel32 = GetModuleHandleA("kernel32.dll");
	if (!hKernel32) return CodeWallStatus;
	...
	if (pSetProcessMitigationPolicy && (CodeWallStatus & eCW_CIG) == 0) {
		...
		if (pSetProcessMitigationPolicy((PROCESS_MITIGATION_POLICY)8, &sigPolicy, sizeof(sigPolicy))) {
			CodeWallStatus |= eCW_CIG;
		}
	}

	// 2. Enable Arbitrary Code Guard (ACG)
	if (pSetProcessMitigationPolicy && (CodeWallStatus & eCW_ACG) == 0) {
		...
		if (pSetProcessMitigationPolicy((PROCESS_MITIGATION_POLICY)2, &dynamicCodePolicy, sizeof(dynamicCodePolicy))) {
			CodeWallStatus |= eCW_ACG;
		}
	}

	return CodeWallStatus;
}

int WinAPI::InitializeCodeWallExternal() {
	...
	if (hAdvapi32) {
		...
		if (pSetSecurityInfo && (CodeWallStatus & eCW_DACL) == 0) {
			// "D:" specifies a DACL. 
