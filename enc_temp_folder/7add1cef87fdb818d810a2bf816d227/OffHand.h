...
{
	struct Timer_FinishOffHandAction;
	struct Timer_FinishGrenadeAction;
	struct Timer_EnableCollisionsWithOwner;
...
public:
	...
	void DumpFallbackConstants(IEntity* pPlayerEnt, const char* tpParentJoint);
	...
	bool IsHeldObjectBroken(EntityId entityId) const;
...
private:
	...
	{
		bool ok = false;
		Vec3 leftLocal = ZERO;   // one-hand: use this
		Vec3 rightLocal = ZERO;  // one-hand: duplicate leftLocal (for safety)
		float widthWS = 0.0f;    // one-hand: 0
	};
...
public:
	...
	SGripHitLocal ComputeGripAnchorOneHand_PhysOnly(CActor* pOwner, IEntity* pObject, float extraMargin, float ttlSeconds);
	...
	void AttachObjectToHand_original(bool attach, EntityId objectId, bool throwObject);
	...
	bool IsEnableCollisionsTimerActive();
...
public:
	void ToggleHeldEntityClientFlag();
	void DebugLogInfo();

	void TickCarryPhysics(IEntity* pObject);

	struct SHoldData {
		EntityId ownerId{};
		string   attachmentName{ "held_object_attachment" };
		Quat     rotOffset = IDENTITY; // from your rotationOffset
		Vec3     posOffset = ZERO;     // from your positionOffset
		Vec3     comLocal = ZERO;     // object-local COM (once)
		bool     twoHand = false;
		bool     active = false;
		EntityId entityId = 0;
		IPhysicalEntity* pPhys = nullptr;
	};

	SHoldData m_hold; // member

	// --- Physics-thread integration state ---
public:

	void CacheHandPose_GameThread(IAttachment* att);

	int  OnPhysPostStep_Instance(const EventPhys* pBase);

	// The physics-thread implementation (no attachment queries inside)
	void TickCarryPhysics_Thread(IPhysicalEntity* pe, const QuatT& handWT, float dt);

	// Register/unregister the physics PostStep callback for mode==2
	void EnablePhysPostStep(bool enable);

private:
	// Thread-safe hand pose cache (written on game thread, read on physics thread)
	std::mutex m_handMtx;
	QuatT m_handWT_cached = QuatT(IDENTITY, ZERO);
	bool  m_handPoseValid = false;

	// Physics-thread kinematics history (used only inside TickCarryPhysics_Thread)
	bool     m_threadPrevValid = false;
	Vec3     m_threadPrev_p_pivot = ZERO;
	Vec3     m_threadPrev_v_pivot = ZERO;
	Quat     m_threadPrev_q_pivot = IDENTITY;
	Vec3     m_threadPrev_w_hand = ZERO;
	EntityId m_threadPrevObjId = 0;

	// Optional toggle: place COM exactly at the attachment pivot
	bool m_kCOMAtAttachment = false;

	bool m_postStepRegistered = false;
