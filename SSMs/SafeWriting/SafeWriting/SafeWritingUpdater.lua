...
SfwUpdates["2.1.1"].Update = function(self)
    ...
    if file then
        for line in file:lines() do
            local name, ip, host, profile, reason, bantime, bannedBy = string.match(line,
                "name:\"(.*)\" ip:\"(.*)\" host:\"(.*)\" profile:\"(.*)\" reason:\"(.*)\" time:\"(.*)\" bannedby:\"(.*)\"");
        ...
        if (file) then
            for i, v in ipairs(bans) do
                ...
                local line = SpecialFormat(
                    "name:%s\t ip:%s\t host:%s\t profile:%s\t reason:%s\t time:%s\t bannedby:%s\t expire:0", name, ip,
                    host, profile, reason, bantime, bannedBy);
                file:write(line .. "\n");
            end
            file:close();
        end
...
SfwUpdates["2.2.0"].Update = function(self)
    ...
    if (file) then
        for line in file:lines() do
            local name, ip, host, profile, reason, bantime, bannedBy, expire = string.match(line,
                "name:(.*)\t ip:(.*)\t host:(.*)\t profile:(.*)\t reason:(.*)\t time:(.*)\t bannedby:(.*)\t expire:(.*)");
