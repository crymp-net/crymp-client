...
SafeWriting.FuncContainer:AddCategs({"OnTimerTick", "PrepareAll", "CheckPlayer", "OnKill", "ProcessBulletDamage",
									 ...
									 "OnActorHit", "OnItemPickedUp", "OnItemDropped", "EquipPlayer", "CanEnterVehicle",
									 "OnRename", "OnLeaveVehicleSeat", "UpdatePlayer", "OnEnterBuyZone",
...
function GetGameRules()
	local result = MakePluginEvent("GetGameRules")
	if result == nil then
		return { name = g_gameRules.class, teams = g_gameRules.class == "PowerStruggle" and 2 or 1 }
	else
		return result
	end
end
...
function InitializeAllScripts(hookFunc)
	...
	printf("Loaded %d new scripts (actual scripts: %d, error scripts: %d, total scripts: %d)", LOADED_SCRIPTS,
		TOTAL_SCRIPTS - LOADED_SCRIPTS, #ERROR_SCRIPTS, TOTAL_SCRIPTS)
...
function ExecuteCommand(cmdname, player, msg)
	...
	if (se.UseCommandsSession and g_gameRules.class == "PowerStruggle") then
		...
		if ((staff) and se.CommandsSessionFlags == SessionFlags.AdminsModerators) then
			...
			if ((_time - (player.LastSession or (se.SessionExpiry + 1))) > se.SessionExpiry and se.CommandsSessionFlags ==
	...
	if (player) then
		...
		if (_time - lastuse < timeout and lastuse ~= 0) then
			if (sendtext == true) then
				Chat:SendToTarget(nil, player, "[[USABLE_ONLY_N_SECONDS]]", cmdname, tostring(timeout),
...
function CanSee(player, target, bone, thr)
	...
	local hitData = {}
	local hits = Physics.RayWorldIntersection(pos, dir, CalcDistance3D(pos, target:GetWorldPos()) + 10, ent_all, skip,
		nil, hitData)
	local entsBefore = 0
	if (hits > 0) then
...
function ConnectHTTP(host, url, method, port, http11, timeout, alive)
	...
	if (IsDllLoaded() or IsDllLoaded100()) and CPPAPI then
		...
		method = method:upper()
		tmp_ret = CPPAPI.ConnectWebsite(host, url, port or 80, http11 or false, timeout,
			method == "GET" and true or false, alive or false)
	else
...
function AsyncConnectHTTP(host, url, method, port, http11, timeout, func)
	...
	if CPPAPI ~= nil and CPPAPI.Request ~= nil then
		...
		AsyncCreateId(CPPAPI.AsyncConnectWebsite(host, url, port or 80, http11 or false, timeout,
			method == "GET" and true or false, false), func)
	end
end
...
function SmartHTTP(method, host, url, func)
	...
	return AsyncConnectHTTP(host, url, method, 80, true, 5000, function(ret, error)
		if error ~= nil then
			func(error, true)
...
function SmartHTTPS(method, host, url, func)
	...
	return AsyncConnectHTTP(host, url, method, 443, true, 5000, function(ret, error)
		if error ~= nil then
			func(error, true)
		elseif ret:sub(1, 8) == "\\\\Error:" then
			func(ret:sub(3), true)
	...
	player.last_team_change = _time
		...
		if (newTeamID ~= 0) then
			g_gameRules.Server.RequestSpawnGroup(g_gameRules, player.id,
				g_gameRules.game:GetTeamDefaultSpawnGroup(newTeamID) or NULL_ENTITY, true)
...
function ParseTime(s)
	...
	local y, mo, d, h, m, s = parts.y or 0, parts.mo or 0, parts.d or 0, parts.h or 0, parts.m or parts.min or 0,
		parts.s or 0
...
function PermaBanPlayer(player, reason, bannedBy, isCheater, expire)
	...
	SafeWriting.Bans[#SafeWriting.Bans + 1] = {name, ip, host, profile, reason, bantime, bannedBy, expire or "0",
											   player.hwid}
	...
	player.wasForceDisconnected = true
	local exists = System.GetEntity(player.id)
	if exists ~= nil then
		CryAction.BanPlayer(player.id, reason)
		if KICK_REMOVE_ENTITY then
			System.RemoveEntity(player.id)
...
function SetPlayerProperty(player, giver, prop, undo, reason, expire)
	...
	local name, ip, host, profile, reason, bannedBy = xmlfixstring(SanitizeName(player)), player.ip, player.host,
		player.profile, reason or "property assigned", giver:GetName()
	local bantime = os.date("%d.%m.%Y %H:%M:%S", time)
	expire = expire or 0
	SafeWriting.Bans[#SafeWriting.Bans + 1] = {name, ip, host, profile, reason, bantime, bannedBy, tostring(expire),
											   player.hwid, prop, undo}
	...
	player.assignedProperties[prop] = {
		["undo"] = undo,
		["expire"] = SafeWriting.Bans[#SafeWriting.Bans][8]
	}
	local fn = _G[prop]
	fn(player)
end
