...
if not SafeWritingGameRules.UpdatePings then
	...
	SafeWritingGameRules.Server.OnClientConnect = function(self, channelId, reset, name)
		...
		if (not reset) then
			...
			if (specMode == 0 or teamId ~= 0) then
				...
				self.Server.RequestSpawnGroup(self, player.id,
					self.game:GetTeamDefaultSpawnGroup(teamId) or NULL_ENTITY, true);
		...
		if (SafeWriting.Settings.EnableStatistics) then
			if (player) then
				...
				if (player.statsidx) then
					pli[player.statsidx].pltime = tostring(pli[player.statsidx].pltime + (_time - player.connecttime) -
															   (player.rOffsetTime or 0));
					pli[player.statsidx].kills = ((pli[player.statsidx].kills or 0) + (player.rKills or 0)) -
													 (player.rOffsetKills or 0);
					pli[player.statsidx].deaths = ((pli[player.statsidx].deaths or 0) + (player.rDeaths or 0)) -
													  (player.rOffsetDeaths or 0);
	...
	SafeWritingGameRules.Server.OnTimer = function(self, timerId, msec)
		...
		elseif (timerId == self.NEXTLEVEL_TIMERID) then
			...
			if (SafeWriting.GlobalStorage.ForceNextMap) then
				...
				SafeWriting.GlobalStorage.NextMap = nil;
				if map.gameRules then
						System.ExecuteCommand("sv_gamerules " .. map.gameRules)
				end
				System.ExecuteCommand("map " .. map.map);
			else
				self.game:NextLevel();
			end
		end
	...
	SafeWritingGameRules.Server.OnFreeze = function(self, targetId, shooterId, weaponId, value)
		...
		if shooter and GetProtectionKind(target) == 1 then
			return false;
		end
		if g_gameRules.class == "PowerStruggle" then
	...
	SafeWritingGameRules.Server.SvBuy = function(self, playerId, itemName)
		...
		if (se.UseCommandsSession) then
			...
			if (result) then
				Console:SendToTarget(player, "${t:SessionActive|3}Session successfuly updated, expires in %d seconds",
					se.SessionExpiry);
			end
		end
		if player then
	...
	SafeWritingGameRules.DoBuyAmmo = function(self, playerId, name)
		...
		if (def.level and def.level > 0 and def.level > level) then
			self.game:SendTextMessage(TextMessageError, "@mp_AlienEnergyRequired", TextMessageToClient, playerId,
				def.name);
			return false;
		end
		...
		if (ammo and ammo.ammo) then
			...
			if (vehicleId) then
				if (alive) then
					...
					if (self:IsInServiceZone(playerId) and (price == 0 or self:EnoughPP(playerId, nil, price)) and
						self:VehicleCanUseAmmo(vehicle, name)) then
						...
						if not player.wasForceDisconnected then
							if player.waitingForAuth and _time - player.waitingForAuth > 25 and master then
								if (player.isSfwCl) or SafeWriting.Settings.StrictProfilePolicy then
									printf("player client: %s, profile: %s, gsprofile: %s",
										(player.isSfwCl and "sfwcl" or "else"), tostring(player.profile),
										...
										if (gd.AvgPingComplete) then
											if ((ping - gd.AveragePing) > se.HighPingThreshold) then
												if (not se.WarnForHighPing) then
													...
													if ((_time - (player.LastHPWarning or 0) >
														(se.WarningInterval or 10))) then
														if ((player.HighPingWarnings or 1) > (se.WarningsCount or 3)) then
															player.CanGetKickedForHP = true;
														else
															Chat:SendToTarget(nil, player, "[[WARNING]] " ..
																(player.HighPingWarnings or 1) .. "/" ..
																(se.WarningsCount or 3) .. ": [[PING_TOO_HIGH]]!");
														end
												...
												if (player.CanGetKickedForHP) then
													Chat:SendToAll(nil, "Player " .. player:GetName() ..
	...
	SafeWritingGameRules.RevivePlayer = function(self, channelId, player, keepEquip)
			...
			if (spawnGroup and spawnGroup.vehicle) then -- spawn group is a vehicle, and the vehicle has some free seats then
				...
				if not (spawnGroup.lockowner and spawnGroup.lockowner ~= player.profile) then
					for i, seat in pairs(spawnGroup.Seats) do
						if ((not seat.seat:IsDriver()) and (not seat.seat:IsGunner()) and (not seat.seat:IsLocked()) and
							(seat.seat:IsFree())) then
		...
		elseif (self.USE_SPAWN_GROUPS) then
			Log("Failed to spawn %s! teamId: %d  groupId: %s  groupTeamId: %d", player:GetName(),
				self.game:GetTeam(player.id), tostring(groupId), self.game:GetTeam(groupId or NULL_ENTITY));
			return false;
		end
		if (not result) then
			...
			if (self.USE_SPAWN_GROUPS or (not player.death_time) or (not player.death_pos)) then
				spawnId, zoffset = self.game:GetSpawnLocation(player.id, ignoreTeam, includeNeutral,
					groupId or NULL_ENTITY);
			else
				spawnId, zoffset = self.game:GetSpawnLocation(player.id, ignoreTeam, includeNeutral,
					groupId or NULL_ENTITY, 50, player.death_pos);
			end
			local pos, angles;
			if (spawnId) then
	...
	SafeWritingGameRules.Server.OnHit = function(self, hit)
		...
		local shooter = hit.shooter;
		if target then
			if target.id == g_gameRules.id then
				return;
			end
		end
		if shooter then
			if shooter.id == g_gameRules.id then
				return;
			end
		end
		...
		if (headshot) then
			if ((AI.GetGroupOf(target.id) == 0 and target.AI and target.AI.theVehicle) or
				(target.AI and target.AI.curSuitMode and target.AI.curSuitMode == BasicAI.SuitMode.SUIT_ARMOR) or
				(target.Properties and target.Properties.bNanoSuit == 1)) then
				headshot = false;
		...
		if (self:IsMultiplayer() or ((not hit.target.actor) or (not hit.target.actor:IsPlayer()))) then
			...
			hit.damage = math.floor(0.5 +
										self:CalcDamage(material_type, hit.damage,
					self:GetDamageTable(hit.shooter, hit.target), hit.assistance));
		end
	...
	SafeWritingGameRules.BuyVehicle = function(self, playerId, itemName)
		...
		if (factory) then
			...
			if (not limitOk) then
				if (teamCheck) then
					self.game:SendTextMessage(TextMessageError, "@mp_TeamItemLimit", TextMessageToClient, playerId,
						self:GetItemName(itemName));
				else
					self.game:SendTextMessage(TextMessageError, "@mp_GlobalItemLimit", TextMessageToClient, playerId,
						self:GetItemName(itemName));
				end
				return false;
			end
	...
	SafeWritingGameRules.IsTeamLocked = function(self, teamId, playerId)
		...
		if (currentTeam~=0) then -- changing team
			local cCount=#GetPlayersByTeamId(currentTeam)-1;
			local nCount=#GetPlayersByTeamId(teamId)+1;
			...
			for i,v in pairs(self.teamId) do
				local vCount=#GetPlayersByTeamId(v);

				for k,j in pairs(self.teamId) do
					if (k~=i) then
						local jCount=#GetPlayersByTeamId(j);
	...
	SafeWritingGameRules.OnChatMessage = function(self, mType, mSourceId, mTargetId, mMsg)
		...
		if SafeWriting.NumVersion >= 218 and source and (not source.IsMuted) then
			...
			if te and source.id == te.id then
				if not IsDllLoaded100() then
					show = false;
				else
					logCon = false;
				end
			...
			if echo and source.host then
				...
				if source.actor then
					isspect = source.actor:GetSpectatorMode() ~= 0;
				end
				local changed = false;
		...
		if (se.CensoreBadWords) then
			for i, word in pairs(se.BadWords) do
				if type(word) == "table" then
					...
					mMsg = string.gsub(mMsg, word, word:sub(1, 1) ..
						string.rep(se.CensoreCharacter or "*", string.len(word) - 2) .. word:sub(-1));
				end
			end
			if (mMsg ~= oMsg) then
				sendfeedback = true;
				...
				if (tgtname == "ALL") then
					g_gameRules.game:SendTextMessage(TextMessageConsole,
						SpecialFormat(
							...
							srcname, tgtname, logMsg), TextMessageToAll);
				elseif (tgtname == "TEAM" or tgtname == "Team") then
					local players = g_gameRules.game:GetPlayers();
					for i, player in ipairs(players) do
						if (g_gameRules.game:GetTeam(player.id) == g_gameRules.game:GetTeam(source.id) or
							(se.LogChatToAdmins and IsAdmin(player))) then
							g_gameRules.game:SendTextMessage(TextMessageConsole,
								SpecialFormat(
									"${t:ChatMsg1|5}[CHAT] ${t:ChatMsg2|8}%s ${t:ChatMsg3|6}to ${t:ChatMsg4|8}%s: ${t:ChatMsg5|6}%s",
									srcname, tgtname, logMsg), TextMessageToClient, player.id);
						end
					end
				else
					g_gameRules.game:SendTextMessage(TextMessageConsole,
	...
	SafeWritingGameRules.OnPlayerRename = function(self, playerid, newname, isUserData)
		...
		if ignoreRename == 0 then
			...
			if player.DisallowRenaming then
				canrename = false;
			end
			local protected = {};
			if SafeWriting.Settings.ProtectNames then
				local lookup = newname:gsub("^[\\$][0-9]", ""):gsub("([^a-zA-Z0-9])", ""):lower();
				local a, m, p, o = SafeWriting.Settings.Admins, SafeWriting.Settings.Moderators,
					SafeWriting.Settings.Premiums, SafeWriting.Settings.Others;
				a = a or {};
				m = m or {};
				p = p or {};
				o = o or {};
						...
						if (seat) then
							...
							for i = 1, wc do
								...
								if (wpnid) then
									...
									if (wpnclass == "Asian50Cal" or wpnclass == "USCoaxialGun" or wpnclass ==
										"AvengerCannon" or wpnclass == "SideWinder") then -- exception for machine guns
										return;
									end
								end
							end
		...
		if (se.DetectRapidFire and _time - player.lastshoot >= 1 and weaponclass ~= "Fists") then
			if (player.totalshoots > ((math.ceil(expected) + 4))) then
				if (not se.RapidFireLogDetails) then
					...
					AntiCheat:DealWithPlayer(player, "rapid-fire hack", true,
						"Shoots per second detected: " .. player.totalshoots, "Using weapon: " .. weaponclass,
						"Fire rate: " .. fireRate, "Is on vehicle: " .. isonveh, "Vehicle class: " .. vehicleclass,
						"Ping: " .. ping, "Latency: " .. (_time - SafeWriting.lastUpdate));
				end
			end
			player.lastshoot = _time;
			player.totalshoots = 0;
		...
		if (work and work.active) then
			...
			if (entity) then
				...
				if (work.type == "repair") then
					...
					elseif (entity.item and (entity.class == "AutoTurret" or entity.class == "AutoTurretAA") and
						(not entity.item:IsDestroyed())) then
						entity.Server.OnHit(entity, hit);
	...
	SafeWritingGameRules.AwardKillPP = function(self, hit)
		...
		MakePluginEvent("OnKillAward", award)
		pp = award.pp
		self:AwardPPCount(playerId, pp);

		if (pp < 0) then -- negative points are assumed to be a teamkill here
