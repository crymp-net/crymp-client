SafeWriting.Settings = {
    ...
    AllowMasterServer = Yes, 			-- allows reporting to master-server + allows users connected without GS using SafeWritingClient
    ...
    Caching = false,					-- enables caching of frequently used functions (optimizes performance)
    ...
    HashSeed = 0x1F0, 					-- this is seed for hashing algorithe, set it to any random number that is on your mind and is higher than 127
    ...
    InitJailOnStart = false, 			-- creates jail on random position on round start
    ...
    JcukenChatCommands = false, 		-- if enabled, commands mistakenly written in JCUKEN layout as if it was QWERTY are translated to QWERTY
    ...
    DisableTranslations = false, 		-- set this to true, to use english language for all players, but Translations.lua will be still required!
    FixOverpoweredAA = true, 			-- set this to true, to fix overpowered AACannon on 5767
    ...
    OtherWelcomeMessages = {"[[SpawnInfo2]]", "[[SpawnInfo3]]", "[[SpawnInfo4]]", "[[SpawnInfo5]]"},
    UsePersistantScores = true, 			-- if enabled and player reconnects, his score is same as before

    ResetScoreOnSpectatorSwitch = false,	-- if enabled, resets score of spectators
    ForbiddenAreaDisabled = true,			-- if enabled, deletes all forbidden areas
	SmoothGameStart = false,				-- if enabled, there is smooth transition from pre-game to in-game without restart

    UseCustomEquipment = true,
    BasicEquipment = { 						-- use: {WeaponName,attachments...}
    {"SCAR", "LAMRifle", "Reflex", "Silencer"}, {"FragGrenade"}, {"FragGrenade"}, {"Parachute"}},
    ...
    PlayerNames = {							-- when player connects as Nomad it renames him to random name from those you define here
		"Hydrogenium", "Helium", "Lithium", "Berylium", "Borum", "Carboneum", "Nitrogenium", "Oxygenum", "Fluorum", "Neon",
		"Natrium", "Magnesium", "Aluminium", "Silicium", "Phosphorus", "Sulphur", "Chlorum", "Argon", "Kalium", "Calcium",
		"Titanium", "Chromium", "Ferrum", "Cuprum", "Zincum", "Selenium", "Stibium", "Telurium", "Iodum", "Xenon",
		"Lanthanum", "Actinium"
	},
    ...
    BadWords = { 							-- Here you put bad words
		"fuck", "bitch", "whore", "cunt", "nigger", "nigga", "kurva", "kokot", "jebe", "jebat", "jeban", "jebn", "kurven",
		"kurvy", "jebl"
	},
